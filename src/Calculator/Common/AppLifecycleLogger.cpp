...
AppLifecycleLogger::AppLifecycleLogger()
    : m_appLifecycleProvider(
          ...
          GUID{0xef00584a, 0x2655, 0x462c, 0xbc, 0x24, 0xe7, 0xde, 0x63, 0xe,
               0x7f, 0xbf}) // Unique provider ID
...
void AppLifecycleLogger::LogAppLifecycleEvent(
    hstring const &eventName, LoggingFields const &fields) const {
  m_appLifecycleProvider.LogEvent(
      eventName, fields, LoggingLevel::Information,
      LoggingOptions(MICROSOFT_KEYWORD_TELEMETRY |
                     WINEVENT_KEYWORD_RESPONSE_TIME));
}
#pragma endregion
