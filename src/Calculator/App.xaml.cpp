...
App::App() {
  ...
  // Currently this is bugged so the property is only respected from
  // code-behind.
  this->HighContrastAdjustment = ApplicationHighContrastAdjustment::None;

#if _DEBUG
  this->DebugSettings->IsBindingTracingEnabled = true;
  this->DebugSettings->BindingFailed += ref new BindingFailedEventHandler(
      [](_In_ Object ^ /*sender*/, _In_ BindingFailedEventArgs ^ e) {
...
/// </summary>
bool App::IsAnimationEnabled() { return App::m_isAnimationEnabled; }
...
void App::RemoveWindowFromMap(int viewId) {
  ...
  assert(iter != m_secondaryWindows.end() &&
         "Window does not exist in the list");
...
task<void> App::HandleViewReleaseAndRemoveWindowFromMap(
...
task<void> App::SetupJumpList() {
  try {
    ...
    for (NavCategory ^ option : calculatorOptions->Categories) {
      ...
      auto item = JumpListItem::CreateWithArguments(
          ((int)mode).ToString(),
          L"ms-resource:///Resources/" + NavCategory::GetNameResourceKey(mode));
      item->Description =
          L"ms-resource:///Resources/" + NavCategory::GetNameResourceKey(mode);
      item->GroupName =
          L"ms-resource:///Resources/" +
          NavCategoryGroup::GetHeaderResourceKey(calculatorOptions->GroupType);
...
/// entry points will be used when the application is launched to open a
...
void App::OnAppLaunch(IActivatedEventArgs ^ args, String ^ argument) {
  ...
  TraceLogger::GetInstance().LogOnAppLaunch(
      previousExecutionState.ToString()->Data());
  ...
  args->SplashScreen->Dismissed +=
      ref new TypedEventHandler<SplashScreen ^, Object ^>(
          this, &App::DismissedEventHandler);

  auto rootFrame = dynamic_cast<Frame ^>(Window::Current->Content);
  WeakReference weak(this);

  float minWindowWidth = static_cast<float>(static_cast<double>(
      this->Resources->Lookup(ApplicationResourceKeys::AppMinWindowWidth)));
  float minWindowHeight = static_cast<float>(static_cast<double>(
      this->Resources->Lookup(ApplicationResourceKeys::AppMinWindowHeight)));
  Size minWindowSize =
      SizeHelper::FromDimensions(minWindowWidth, minWindowHeight);

  ApplicationView ^ appView = ApplicationView::GetForCurrentView();
  ApplicationDataContainer ^ localSettings =
      ApplicationData::Current->LocalSettings;
  // For very first launch, set the size of the calc as size of the default
  // standard mode
  ...
  if (rootFrame == nullptr) {
    if (!Windows::Foundation::Metadata::ApiInformation::IsTypePresent(
            "Windows.Phone.UI.Input.HardwareButtons")) // PC Family
    {
      // Disable the system view activation policy during the first launch of
      // the app only for PC family devices and not for phone family devices
      try {
    ...
    AddWindowToMap(WindowFrameService::CreateNewWindowFrameService(
        rootFrame, false, weak));
  } else {
    // For first launch, LaunchStart is logged in constructor, this is for
    // subsequent launches.
    TraceLogger::GetInstance().LogAppLaunchStart();

    // !Phone check is required because even in continuum mode user interaction
    // mode is Mouse not Touch
    if ((UIViewSettings::GetForCurrentView()->UserInteractionMode ==
         UserInteractionMode::Mouse) &&
        (!Windows::Foundation::Metadata::ApiInformation::IsTypePresent(
            "Windows.Phone.UI.Input.HardwareButtons"))) {
      // If the pre-launch hasn't happened then allow for the new window/view
      // creation
      if (!m_preLaunched) {
        auto newCoreAppView = CoreApplication::CreateNewView();
        newCoreAppView->Dispatcher->RunAsync(
            CoreDispatcherPriority::Normal,
            ref new DispatchedHandler([args, argument, minWindowSize, weak]() {
              ...
              if (that != nullptr) {
                ...
                if (!rootFrame->Navigate(MainPage::typeid, argument)) {
                  // We couldn't navigate to the main page, kill the app so we
                  // have a good stack to debug
                  throw std::bad_exception();
                }

                auto frameService =
                    WindowFrameService::CreateNewWindowFrameService(rootFrame,
                                                                    true, weak);
                that->AddWindowToMap(frameService);

                auto dispatcher = CoreWindow::GetForCurrentThread()->Dispatcher;
                auto safeFrameServiceCreation =
                    std::make_shared<SafeFrameWindowCreation>(frameService,
                                                              that);
                int newWindowId =
                    ApplicationView::GetApplicationViewIdForWindow(
                        CoreWindow::GetForCurrentThread());
                ...
                auto activateEventArgs =
                    dynamic_cast<IViewSwitcherProvider ^>(args);
                ...
                if (activationViewSwitcher != nullptr) {
                  activationViewSwitcher->ShowAsStandaloneAsync(
                      newWindowId, ViewSizePreference::Default);
                  safeFrameServiceCreation->SetOperationSuccess(true);
                } else {
                  auto activatedEventArgs =
                      dynamic_cast<IApplicationViewActivatedEventArgs ^>(args);
                    ...
                    create_task(
                        ...
                        .then(
                            [safeFrameServiceCreation](bool viewShown) {
                              // SafeFrameServiceCreation is used to
                              // automatically remove the frame from the list of
                              // frames if something goes bad.
                              safeFrameServiceCreation->SetOperationSuccess(
                                  viewShown);
                            },
    ...
    } else // for touch devices
    {
      ...
      if (!Windows::Foundation::Metadata::ApiInformation::IsTypePresent(
              "Windows.Phone.UI.Input.HardwareButtons")) {
        // for tablet mode: since system view activation policy is disabled so
        // do ShowAsStandaloneAsync if activationViewSwitcher exists in
        // activationArgs
        ActivationViewSwitcher ^ activationViewSwitcher;
        auto activateEventArgs = dynamic_cast<IViewSwitcherProvider ^>(args);
        if (activateEventArgs != nullptr) {
        ...
        if (activationViewSwitcher != nullptr) {
          auto viewId = safe_cast<IApplicationViewActivatedEventArgs ^>(args)
