...
Calculator::Calculator()
    : m_doAnimate(false), m_isLastAnimatedInScientific(false),
      m_isLastAnimatedInProgrammer(false), m_resultAnimate(false) {
  ...
  m_displayFlyout =
      static_cast<MenuFlyout ^>(Resources->Lookup(L"DisplayContextMenu"));
...
void Calculator::LoadResourceStrings() {
  ...
  m_openMemoryFlyoutAutomationName =
      resProvider.GetResourceString(L"MemoryButton_Open");
  m_closeMemoryFlyoutAutomationName =
      resProvider.GetResourceString(L"MemoryButton_Close");
  m_openHistoryFlyoutAutomationName =
      resProvider.GetResourceString(L"HistoryButton_Open");
  m_closeHistoryFlyoutAutomationName =
      resProvider.GetResourceString(L"HistoryButton_Close");
  AutomationProperties::SetName(MemoryButton, m_openMemoryFlyoutAutomationName);
  AutomationProperties::SetName(HistoryButton,
                                m_openHistoryFlyoutAutomationName);
}
...
void Calculator::InitializeHistoryView(_In_ HistoryViewModel ^ historyVM) {
  if (m_historyList == nullptr) {
    ...
    historyVM->HideHistoryClicked +=
        ref new ViewModel::HideHistoryClickedHandler(
            this, &Calculator::OnHideHistoryClicked);
    historyVM->HistoryItemClicked +=
        ref new ViewModel::HistoryItemClickedHandler(
...
void Calculator::SetFontSizeResources() {
  ...
  while (currentItem->numericSystem.compare(std::wstring(L"Default")) != 0 &&
         currentItem->numericSystem.compare(formatter->NumeralSystem->Data()) !=
             0) {
    currentItem++;
  }

  this->Resources->Insert(StringReference(L"ResultFullFontSize"),
                          currentItem->fullFont);
  this->Resources->Insert(StringReference(L"ResultFullMinFontSize"),
                          currentItem->fullFontMin);
  this->Resources->Insert(StringReference(L"ResultPortraitMinFontSize"),
                          currentItem->portraitMin);
  this->Resources->Insert(StringReference(L"ResultSnapFontSize"),
                          currentItem->snapFont);
  this->Resources->Insert(StringReference(L"CalcButtonCaptionSizeOverride"),
                          currentItem->fullNumPadFont);
  this->Resources->Insert(
      StringReference(L"CalcButtonScientificSnapCaptionSizeOverride"),
      currentItem->snapScientificNumPadFont);
  this->Resources->Insert(
      StringReference(L"CalcButtonScientificPortraitCaptionSizeOverride"),
...
void Calculator::OnLoaded(_In_ Object ^, _In_ RoutedEventArgs ^) {
  Model->PropertyChanged += ref new PropertyChangedEventHandler(
      this, &Calculator::OnCalcPropertyChanged);
  Model->HideMemoryClicked +=
      ref new HideMemoryClickedHandler(this, &Calculator::OnHideMemoryClicked);

  InitializeHistoryView(Model->HistoryVM);
  String ^ historyPaneName =
      AppResourceProvider::GetInstance().GetResourceString(L"HistoryPane");
  HistoryFlyout->FlyoutPresenterStyle->Setters->Append(
      ref new Setter(AutomationProperties::NameProperty, historyPaneName));
  String ^ memoryPaneName =
      AppResourceProvider::GetInstance().GetResourceString(L"MemoryPane");
  MemoryFlyout->FlyoutPresenterStyle->Setters->Append(
      ref new Setter(AutomationProperties::NameProperty, memoryPaneName));

  if (Windows::Foundation::Metadata::ApiInformation::IsEventPresent(
          L"Windows.UI.Xaml.Controls.Primitives.FlyoutBase", L"Closing")) {
    HistoryFlyout->Closing +=
        ref new TypedEventHandler<FlyoutBase ^, FlyoutBaseClosingEventArgs ^>(
            this, &Calculator::HistoryFlyout_Closing);
    MemoryFlyout->Closing +=
        ref new TypedEventHandler<FlyoutBase ^, FlyoutBaseClosingEventArgs ^>(
            this, &Calculator::OnMemoryFlyoutClosing);
  }
...
void Calculator::UpdateViewState() {
  ...
  VisualStateManager::GoToState(this, ref new String(state.c_str()),
                                true /*useTransitions*/);
}
...
void Calculator::SetResultStyles() {
  ...
  if (window) {
    ...
    } else if ((IsProgrammer && curHeight >= 640) ||
               (IsScientific && curHeight >= 544) || IsStandard) {
...
void Calculator::AnimateCalculator(bool resultAnimate) {
  if (App::IsAnimationEnabled()) {
    ...
    if (((m_isLastAnimatedInScientific && IsScientific) ||
      ...
      // It's because if last animation was in standard, then go to unit
      // converter, then comes back to standard The state for the calculator
      // does not change and the animation would not get run.
      this->OnStoryboardCompleted(nullptr, nullptr);
    }
  }
}

void Calculator::OnContextRequested(UIElement ^ sender,
...
void Calculator::OnIsStandardPropertyChanged(bool /*oldValue*/,
                                             bool /*newValue*/) {
  UpdateViewState();
  UpdatePanelViewState();
}

void Calculator::OnIsScientificPropertyChanged(bool /*oldValue*/,
                                               bool newValue) {
  if (newValue) {
    EnsureScientific();
  }
...
void Calculator::OnIsProgrammerPropertyChanged(bool /*oldValue*/,
                                               bool newValue) {
  if (newValue) {
    EnsureProgrammer();
    m_pivotItem = static_cast<Windows::UI::Xaml::Controls::PivotItem ^>(
        DockPivot->Items->GetAt(0));
...
void Calculator::OnCalcPropertyChanged(_In_ Object ^ sender,
                                       _In_ PropertyChangedEventArgs ^ e) {
...
void Calculator::SetChildAsMemory() { DockMemoryHolder->Child = GetMemory(); }
...
void Calculator::OnHideHistoryClicked() { ToggleHistoryFlyout(nullptr); }
...
void Calculator::HistoryFlyout_Opened(_In_ Object ^ sender,
                                      _In_ Object ^ args) {
  ...
  AutomationProperties::SetName(HistoryButton,
                                m_closeHistoryFlyoutAutomationName);
  TraceLogger::GetInstance().LogHistoryFlyoutOpenEnd(
      Model->HistoryVM->ItemSize);
  TraceLogger::GetInstance().LogHistoryBodyOpened();
}

void Calculator::HistoryFlyout_Closing(_In_ FlyoutBase ^ sender,
                                       _In_ FlyoutBaseClosingEventArgs ^ args) {
  // Set in the Closing event so the new name is available when the Flyout has
  // Closed.
  AutomationProperties::SetName(HistoryButton,
                                m_openHistoryFlyoutAutomationName);
}

void Calculator::HistoryFlyout_Closed(_In_ Object ^ sender,
                                      _In_ Object ^ args) {
  // Ideally, this would be renamed in the Closing event because the Closed
  // event is too late. Closing is not available until RS1+ so we set the name
  // again here for TH2 support.
  AutomationProperties::SetName(HistoryButton,
                                m_openHistoryFlyoutAutomationName);
...
void Calculator::ToggleHistoryFlyout(Object ^ /*parameter*/) {
  String ^ viewState = App::GetAppViewState();
  // If app starts correctly in snap mode and shortcut is used for history then
  // we need to load history if not yet initialized.
  if (viewState != ViewState::DockedView) {
    if (m_fIsHistoryFlyoutOpen) {
      HistoryFlyout->Hide();
    } else {
      TraceLogger::GetInstance().LogHistoryFlyoutOpenBegin(
          Model->HistoryVM->ItemSize);
...
void Calculator::ToggleMemoryFlyout() {
  ...
  if (viewState != ViewState::DockedView) {
    if (m_fIsMemoryFlyoutOpen) {
      ...
      TraceLogger::GetInstance().LogMemoryFlyoutOpenBegin(
          Model->MemorizedNumbers->Size);
...
void Calculator::OnMemoryFlyoutOpened(_In_ Object ^ sender,
                                      _In_ Object ^ args) {
  TraceLogger::GetInstance().LogMemoryFlyoutOpenEnd(
      Model->MemorizedNumbers->Size);
  m_IsLastFlyoutMemory = true;
  m_IsLastFlyoutHistory = false;
  m_fIsMemoryFlyoutOpen = true;
  AutomationProperties::SetName(MemoryButton,
                                m_closeMemoryFlyoutAutomationName);
  EnableControls(false);
  TraceLogger::GetInstance().LogMemoryBodyOpened();
}

void Calculator::OnMemoryFlyoutClosing(_In_ FlyoutBase ^ sender,
                                       _In_ FlyoutBaseClosingEventArgs ^ args) {
  // Set in the Closing event so the new name is available when the Flyout has
  // Closed.
  AutomationProperties::SetName(MemoryButton, m_openMemoryFlyoutAutomationName);
...
void Calculator::OnMemoryFlyoutClosed(_In_ Object ^ sender,
                                      _In_ Object ^ args) {
  // Ideally, this would be renamed in the Closing event because the Closed
  // event is too late. Closing is not available until RS1+ so we set the name
  // again here for TH2 support.
...
Memory ^ Calculator::GetMemory() {
  if (m_memory == nullptr) {
    ...
    VisualStateManager::GoToState(
        m_memory, ref new String(GetCurrentLayoutState().c_str()),
        true /*useTransitions*/);
  }
...
void Calculator::OnMemoryFlyOutTapped(_In_ Object ^ sender,
                                      _In_ TappedRoutedEventArgs ^ e) {
...
void Calculator::OnHistoryFlyOutTapped(_In_ Object ^ sender,
                                       _In_ TappedRoutedEventArgs ^ e) {
  Grid ^ grid = safe_cast<Grid ^>(sender);
  Point point = e->GetPosition(nullptr);
...
void Calculator::expressionContainer_LayoutUpdated(_In_ Object ^ sender,
                                                   _In_ Object ^ e) {
...
void Calculator::OnHistoryAccessKeyInvoked(_In_ UIElement ^ sender,
                                           _In_ AccessKeyInvokedEventArgs ^
                                               args) {
  DockPivot->SelectedItem = HistoryPivotItem;
}

void Calculator::OnMemoryAccessKeyInvoked(_In_ UIElement ^ sender,
                                          _In_ AccessKeyInvokedEventArgs ^
                                              args) {
  DockPivot->SelectedItem = MemoryPivotItem;
}

void CalculatorApp::Calculator::DockPivot_SelectionChanged(
