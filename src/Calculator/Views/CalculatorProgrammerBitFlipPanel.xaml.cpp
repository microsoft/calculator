...
CalculatorProgrammerBitFlipPanel::CalculatorProgrammerBitFlipPanel()
  ...
  InitializeComponent();
  auto booleanToVisibilityConverter =
      ref new Converters::BooleanToVisibilityConverter;
  SetVisibilityBinding(BitFlipPanel, L"IsBinaryBitFlippingEnabled",
                       booleanToVisibilityConverter);

  AssignFlipButtons();
}

void CalculatorProgrammerBitFlipPanel::OnLoaded(Object ^ sender,
                                                RoutedEventArgs ^ e) {
  UnsubscribePropertyChanged();
  SubscribePropertyChanged();
}

void CalculatorProgrammerBitFlipPanel::OnUnloaded(Object ^ sender,
                                                  RoutedEventArgs ^ e) {
  UnsubscribePropertyChanged();
}
  ...
  if (Model != nullptr) {
    m_propertyChangedToken = Model->PropertyChanged +=
        ref new PropertyChangedEventHandler(
            this, &CalculatorProgrammerBitFlipPanel::OnPropertyChanged);
...
void CalculatorProgrammerBitFlipPanel::OnPropertyChanged(
    Object ^ sender, PropertyChangedEventArgs ^ e) {
  if (e->PropertyName ==
      StandardCalculatorViewModel::BinaryDisplayValuePropertyName) {
    UpdateCheckedStates();
  }
}
...
void CalculatorProgrammerBitFlipPanel::SetVisibilityBinding(
    _In_ FrameworkElement ^ element, _In_ String ^ path,
    _In_ IValueConverter ^ converter) {
...
void CalculatorProgrammerBitFlipPanel::OnBitToggled(_In_ Object ^ sender,
  ...
  // Any input from the Numpad may also result in toggling the bit as their
  // state is bound to the BinaryDisplayValue. Also, if the mode is switched to
  // other Calculator modes when the BitFlip panel is open, a race condition
  // exists in which the IsProgrammerMode property is still true and the
  // UpdatePrimaryResult() is called, which continuously alters the Display
  // Value and the state of the Bit Flip buttons.
...
void CalculatorProgrammerBitFlipPanel::UpdateCheckedStates() {
  ...
  static const wchar_t ch0 =
      LocalizationSettings::GetInstance().GetDigitSymbolFromEnUsDigit(L'0');

  // Filter any unwanted characters from the displayed string.
  static constexpr array<wchar_t, 4> unwantedChars = {L' ', Utils::LRE,
                                                      Utils::PDF, Utils::LRO};

  wstringstream stream;
  wstring displayValue = Model->BinaryDisplayValue->Data();
  for (const wchar_t &c : displayValue) {
    if (find(begin(unwantedChars), end(unwantedChars), c) ==
        unwantedChars.end()) {
      stream << c;
    }
  }
