...
UnitConverter::UnitConverter()
    : m_layoutDirection(::FlowDirection::LeftToRight),
      m_meteredConnectionOverride(false), m_isAnimationEnabled(false) {
  InitializeComponent();

  // adding ESC key shortcut binding to clear button
  clearEntryButtonPos0->SetValue(
      Common::KeyboardShortcutManager::VirtualKeyProperty,
      Common::MyVirtualKey::Escape);
  ...
  bool preferRight =
      LocalizationSettings::GetInstance().GetCurrencySymbolPrecedence() == 0;
  VisualStateManager::GoToState(this,
                                preferRight ? "CurrencySymbolRightState"
                                            : "CurrencySymbolLeftState",
                                false);
  ...
  m_resultsFlyout = static_cast<MenuFlyout ^>(
      Resources->Lookup(L"CalculationResultContextMenu"));
...
void UnitConverter::OnPropertyChanged(_In_ Object ^ sender,
                                      _In_ PropertyChangedEventArgs ^ e) {
  String ^ propertyName = e->PropertyName;
  if (propertyName == UnitConverterViewModel::NetworkBehaviorPropertyName ||
      propertyName ==
          UnitConverterViewModel::CurrencyDataLoadFailedPropertyName) {
    OnNetworkBehaviorChanged();
  } else if (propertyName ==
             UnitConverterViewModel::CurrencyDataIsWeekOldPropertyName) {
    SetCurrencyTimestampFontWeight();
  } else if (propertyName ==
             UnitConverterViewModel::IsCurrencyLoadingVisiblePropertyName) {
...
void UnitConverter::InitializeOfflineStatusTextBlock() {
  ...
  std::wstring offlineStatusHyperlinkText =
      static_cast<String ^>(
          resProvider.GetResourceString(L"OfflineStatusHyperlinkText"))
          ->Data();

  // The resource string has the 'NetworkSettings' hyperlink wrapped with
  // '%HL%'. Break the string and assign pieces appropriately.
  ...
  size_t secondSplitPosition =
      offlineStatusHyperlinkText.find(delimiter, firstSplitPosition + 1);
  assert(secondSplitPosition != std::wstring::npos);
  size_t hyperlinkTextLength =
      secondSplitPosition - (firstSplitPosition + delimiterLength);

  // Assign pieces.
  auto offlineStatusTextBeforeHyperlink = ref new String(
      offlineStatusHyperlinkText.substr(0, firstSplitPosition).c_str());
  auto offlineStatusTextLink = ref new String(
      offlineStatusHyperlinkText
          .substr(firstSplitPosition + delimiterLength, hyperlinkTextLength)
          .c_str());
  auto offlineStatusTextAfterHyperlink = ref new String(
      offlineStatusHyperlinkText.substr(secondSplitPosition + delimiterLength)
          .c_str());
...
void UnitConverter::OnValueKeyDown(
...
void UnitConverter::OnContextRequested(UIElement ^ sender,
...
void UnitConverter::OnCopyMenuItemClicked(_In_ Object ^ sender,
...
void UnitConverter::OnPasteMenuItemClicked(_In_ Object ^ sender,
                                           _In_ RoutedEventArgs ^ e) {
  CopyPasteManager::GetStringToPaste(Model->Mode, CategoryGroupType::Converter)
      .then([this](String ^ pastedString) {
...
void UnitConverter::OnValueSelected(_In_ Platform::Object ^ sender) {
  ...
  safe_cast<UnitConverterViewModel ^>(this->DataContext)
      ->OnValueActivated(AsActivatable(value));
}

void UnitConverter::UpdateDropDownState(_In_ Platform::Object ^ sender,
                                        _In_ Platform::Object ^ e) {
  safe_cast<UnitConverterViewModel ^>(this->DataContext)->IsDropDownOpen =
      (Units1->IsDropDownOpen) || (Units2->IsDropDownOpen);
...
void UnitConverter::SetDefaultFocus() {
  const std::vector<Control ^> focusPrecedence = {
      Value1, CurrencyRefreshBlockControl, OfflineBlock, clearEntryButtonPos0};
...
void UnitConverter::CurrencyRefreshButton_Click(_In_ Object ^ /*sender*/,
                                                _In_ RoutedEventArgs ^
                                                /*e*/) {
...
void UnitConverter::OnDataContextChanged(_In_ FrameworkElement ^ sender,
                                         _In_ DataContextChangedEventArgs ^
                                             args) {
  Model->PropertyChanged -= m_propertyChangedToken;

  m_propertyChangedToken = Model->PropertyChanged +=
      ref new PropertyChangedEventHandler(this,
                                          &UnitConverter::OnPropertyChanged);

  OnNetworkBehaviorChanged();
}

void UnitConverter::Units1_IsEnabledChanged(Object ^ sender,
...
void UnitConverter::OnIsDisplayVisibleChanged() {
  if (Model->IsCurrencyLoadingVisible) {
    ...
    if (m_isAnimationEnabled && Model->IsCurrencyCurrentCategory &&
        !Model->CurrencyTimestamp->IsEmpty()) {
...
void UnitConverter::StartProgressRingWithDelay() {
  ...
  m_delayTimer->Tick +=
      ref new EventHandler<Object ^>(this, &UnitConverter::OnDelayTimerTick);
...
// The function will make sure the UI will have enough space to display
// supplementary results and currency information
void CalculatorApp::UnitConverter::SupplementaryResultsPanelInGrid_SizeChanged(
    Platform::Object ^ sender, Windows::UI::Xaml::SizeChangedEventArgs ^ e) {
  // We add 0.01 to be sure to not create an infinite loop with SizeChanged
  // events cascading due to float approximation
