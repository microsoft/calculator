...
// returns automationName in form "Nth bit Value zero/one" depending if bit is
// set/unset at corresponding index of binary display
Object ^ BitFlipAutomationNameConverter::Convert(_In_ Object ^ value,
                                                 TypeName targetType,
                                                 _In_ Object ^ parameter,
                                                 _In_ String ^ language) {
  ...
  String ^ indexName =
      resourceLoader.GetResourceString(static_cast<String ^>(parameter));
  ...
  String ^ zero =
      resourceLoader.GetResourceString(L"BinaryZeroValueAutomationName");
  String ^ one =
      resourceLoader.GetResourceString(L"BinaryOneValueAutomationName");
  if ((value != nullptr) && (parameter != nullptr)) {
    ...
    // remove all the characters except 0 and 1 from the array.
        ...
        // if index is more than the length of binary display return automation
        // name with zero
...
Object ^ BitFlipAutomationNameConverter::ConvertBack(_In_ Object ^ value,
                                                     TypeName targetType,
                                                     _In_ Object ^ parameter,
                                                     _In_ String ^ language) {
  return value;
