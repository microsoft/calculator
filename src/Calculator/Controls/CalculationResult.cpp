...
DEPENDENCY_PROPERTY_INITIALIZATION(CalculationResult,
                                   MaxExpressionHistoryCharacters);
...
CalculationResult::CalculationResult()
    : m_startingFontSize(0.0), m_isScalingText(false),
...
void CalculationResult::OnApplyTemplate() {
  assert((m_scrollLeft == nullptr && m_scrollRight == nullptr) ||
         (m_scrollLeft != nullptr && m_scrollRight != nullptr));
  if (m_textContainer) {
    m_textContainer->LayoutUpdated -= m_textContainerLayoutChangedToken;
  }
  m_textContainer =
      dynamic_cast<ScrollViewer ^>(GetTemplateChild("textContainer"));
  if (m_textContainer) {
    m_textContainer->SizeChanged += ref new SizeChangedEventHandler(
        this, &CalculationResult::TextContainerSizeChanged);
    ...
    m_textContainerLayoutChangedToken = m_textContainer->LayoutUpdated +=
        ref new EventHandler<Object ^>(
            this, &CalculationResult::OnTextContainerLayoutUpdated);

    m_textContainer->ChangeView(m_textContainer->ExtentWidth -
                                    m_textContainer->ViewportWidth,
                                nullptr, nullptr);
    m_scrollLeft =
        dynamic_cast<HyperlinkButton ^>(GetTemplateChild("scrollLeft"));
    m_scrollRight =
        dynamic_cast<HyperlinkButton ^>(GetTemplateChild("scrollRight"));
    auto borderContainer =
        dynamic_cast<UIElement ^>(GetTemplateChild("border"));
    if (m_scrollLeft && m_scrollRight) {
      m_scrollLeft->Click +=
          ref new RoutedEventHandler(this, &CalculationResult::OnScrollClick);
      m_scrollRight->Click +=
          ref new RoutedEventHandler(this, &CalculationResult::OnScrollClick);
      borderContainer->PointerEntered += ref new PointerEventHandler(
          this, &CalculationResult::OnPointerEntered);
      borderContainer->PointerExited += ref new PointerEventHandler(
          this, &CalculationResult::OnPointerExited);
    }
    m_textBlock =
        dynamic_cast<TextBlock ^>(m_textContainer->FindName("normalOutput"));
    if (m_textBlock) {
...
void CalculationResult::OnTextContainerLayoutUpdated(Object ^ /*sender*/,
                                                     Object ^
                                                     /*e*/) {
  if (m_isScalingText) {
    UpdateTextState();
...
void CalculationResult::TextContainerSizeChanged(Object ^ /*sender*/,
                                                 SizeChangedEventArgs ^
                                                 /*e*/) {
  UpdateTextState();
}

void CalculationResult::OnIsActivePropertyChanged(bool /*oldValue*/,
                                                  bool /*newValue */) {
  UpdateVisualState();
}

void CalculationResult::OnAccentColorPropertyChanged(Brush ^ /*oldValue*/,
                                                     Brush ^
                                                     /*newValue*/) {
...
void CalculationResult::OnDisplayValuePropertyChanged(String ^ /*oldValue*/,
                                                      String ^
                                                      /*newValue*/) {
  UpdateTextState();
}

void CalculationResult::OnIsInErrorPropertyChanged(bool /*oldValue*/,
                                                   bool newValue) {
  ...
  if (newValue) {
    ...
    // We use UIText as the font type because this is the most common font type
    // to use
    m_textBlock->FontFamily =
        LocalizationService::GetInstance()->GetLanguageFontFamilyForType(
            LanguageFontType::UIText);
  } else {
...
void CalculationResult::UpdateTextState() {
  ...
  // UpdateLayout will keep calling us until we make it through the below 2
  // if-statements
  ...
  if (containerSize > 0) {
    ...
    if (widthDiff > WIDTHCUTOFF) {
      fontSizeChange = min<double>(
          max<double>(floor(WIDTHTOFONTSCALAR * widthDiff) - WIDTHTOFONTOFFSET,
                      INCREMENTOFFSET),
          MAXFONTINCREMENT);
    }
    if (m_textBlock->ActualWidth < containerSize &&
        abs(m_textBlock->FontSize - m_startingFontSize) > FONTTOLERANCE &&
        !m_haveCalculatedMax) {
    ...
    if (m_textBlock->ActualWidth >= containerSize &&
        abs(m_textBlock->FontSize - MinFontSize) > FONTTOLERANCE) {
      ModifyFontAndMargin(m_textBlock, -1 * fontSizeChange);
      m_textBlock->InvalidateArrange();
      return;
    }
    assert(m_textBlock->FontSize >= MinFontSize &&
           m_textBlock->FontSize <= m_startingFontSize);
...
void CalculationResult::ScrollLeft() {
  if (m_textContainer->HorizontalOffset > 0) {
    double offset = m_textContainer->HorizontalOffset -
                    (scrollRatio * m_textContainer->ViewportWidth);
...
void CalculationResult::ScrollRight() {
  if (m_textContainer->HorizontalOffset <
      m_textContainer->ExtentWidth - m_textContainer->ViewportWidth) {
    double offset = m_textContainer->HorizontalOffset +
                    (scrollRatio * m_textContainer->ViewportWidth);
    m_textContainer->ChangeView(offset, nullptr, nullptr);
    m_textContainer->UpdateLayout();
...
void CalculationResult::OnScrollClick(Object ^ sender, RoutedEventArgs ^
                                      /*e*/) {
...
void CalculationResult::OnPointerEntered(Platform::Object ^ sender,
                                         PointerRoutedEventArgs ^ e) {
  if (e->Pointer->PointerDeviceType == PointerDeviceType::Mouse &&
      m_textBlock->ActualWidth >= m_textContainer->ActualWidth) {
    UpdateScrollButtons();
  }
}

void CalculationResult::ShowHideScrollButtons(::Visibility vLeft,
                                              ::Visibility vRight) {
...
void CalculationResult::UpdateScrollButtons() {
  ...
  // We have more number on both side. Show both arrows
           ...
           m_textContainer->HorizontalOffset <
               (m_textContainer->ExtentWidth -
                m_textContainer->ViewportWidth)) {
    ShowHideScrollButtons(::Visibility::Visible, ::Visibility::Visible);
  }
  // Width is larger than the container and left most part of the number is
  // shown. Should be able to scroll left.
  else if (m_textContainer->HorizontalOffset == 0) {
    ShowHideScrollButtons(::Visibility::Collapsed, ::Visibility::Visible);
  } else // Width is larger than the container and right most part of the number
         // is shown. Should be able to scroll left.
  {
    ShowHideScrollButtons(::Visibility::Visible, ::Visibility::Collapsed);
  }
}

void CalculationResult::OnPointerExited(Platform::Object ^ sender,
                                        PointerRoutedEventArgs ^ e) {
  if (e->Pointer->PointerDeviceType == PointerDeviceType::Mouse) {
    UpdateScrollButtons();
  }
...
void CalculationResult::ModifyFontAndMargin(TextBlock ^ textBox,
                                            double fontChange) {
...
void CalculationResult::ProgrammaticSelect() { RaiseSelectedEvent(); }

void CalculationResult::RaiseSelectedEvent() { Selected(this); }
