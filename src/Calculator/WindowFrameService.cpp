...
WindowFrameService ^ WindowFrameService::CreateNewWindowFrameService(
                         _In_ Frame ^ viewFrame, bool createdByUs,
...
WindowFrameService::WindowFrameService(_In_ Frame ^ frame, WeakReference parent)
    : m_currentWindow(CoreWindow::GetForCurrentThread()),
      m_coreDispatcher(m_currentWindow->Dispatcher), m_frame(frame),
      m_parent(parent), m_viewId(ApplicationView::GetApplicationViewIdForWindow(
                            m_currentWindow.Get())) {}

void WindowFrameService::InitializeFrameService(bool createdByUs) {
  assert(createdByUs == (!CoreApplication::GetCurrentView()->IsHosted &&
                         !CoreApplication::GetCurrentView()->IsMain));
  if (createdByUs) {
    ApplicationView::GetForCurrentView()->Consolidated +=
        ref new TypedEventHandler<ApplicationView ^,
                                  ApplicationViewConsolidatedEventArgs ^>(
            this, &WindowFrameService::OnConsolidated);
  } else {
    CoreWindow::GetForCurrentThread()->Closed +=
        ref new TypedEventHandler<CoreWindow ^, CoreWindowEventArgs ^>(
            this, &WindowFrameService::OnClosed);
  }
}
...
int WindowFrameService::GetViewId() { return m_viewId; }

Frame ^ WindowFrameService::GetFrame() { return m_frame; }

Page ^ WindowFrameService::GetCurrentPage() {
  return dynamic_cast<Page ^>(m_frame->Content);
}

void WindowFrameService::SetNewFrame(_In_ Windows::UI::Xaml::Controls::Frame ^
                                     frame) {
  assert(frame->BackStackDepth == 0);
  m_frame = frame;
}

void WindowFrameService::RegisterOnWindowClosingHandler(
    function<void()> onWindowClosingHandler) {
...
task<void> WindowFrameService::HandleViewRelease() {
  ...
  m_coreDispatcher->RunAsync(
      ...
      ref new DispatchedHandler([that, closingHandlersCompletedEvent]() {
        KeyboardShortcutManager::OnWindowClosed(that->m_viewId);
        Window::Current->Content = nullptr;
        that->InvokeWindowClosingHandlers();
        // This is to ensure InvokeWindowClosingHandlers is be done before
        // RemoveWindowFromMap If InvokeWindowClosingHandlers throws any
        // exception we want it to crash the application so we are OK not
        // setting closingHandlersCompletedEvent in that case
...
void WindowFrameService::OnConsolidated(
    _In_ ApplicationView ^ sender,
    _In_ ApplicationViewConsolidatedEventArgs ^ e) {
...
void WindowFrameService::OnClosed(_In_ CoreWindow ^ sender,
...
void WindowFrameService::LogOnViewClosed(_In_ CoreWindow ^ coreWindow) {
  if (coreWindow) {
    TraceLogger::GetInstance().LogViewClosingTelemetry(
        ApplicationView::GetApplicationViewIdForWindow(coreWindow));
  } else {
    TraceLogger::GetInstance().LogCoreWindowWasNull();
  }
}

void WindowFrameService::RegisterRuntimeWindowService(TypeName serviceId,
                                                      _In_opt_ Object ^
                                                          service) {
...
_Ret_maybenull_ Object ^
    WindowFrameService::TryResolveRuntimeWindowService(TypeName serviceId) {
