...
StringReference CalculationResultAutomationNamePropertyName(
    L"CalculationResultAutomationName");
} // namespace
...
StringReference CalculatorResults_DecimalSeparator_Announced(
    L"Format_CalculatorResults_Decimal");
...
StringReference LeftParenthesisAutomationFormat(
    L"Format_OpenParenthesisAutomationNamePrefix");
StringReference OpenParenthesisCountAutomationFormat(
    L"Format_OpenParenthesisCountAutomationNamePrefix");
...
StringReference
    ButtonPressFeedbackFormat(L"Format_ButtonPressAuditoryFeedback");
...
StandardCalculatorViewModel::StandardCalculatorViewModel()
    : m_DisplayValue(L"0"), m_DecimalDisplayValue(L"0"),
      m_HexDisplayValue(L"0"), m_BinaryDisplayValue(L"0"),
      ...
      m_IsMemoryEmpty(true), m_IsFToEChecked(false), m_isShiftChecked(false),
      m_IsShiftProgrammerChecked(false), m_IsQwordEnabled(true),
      m_IsDwordEnabled(true), m_IsWordEnabled(true), m_IsByteEnabled(true),
      m_isBitFlipChecked(false), m_isBinaryBitFlippingEnabled(false),
      m_CurrentRadixType(RADIX_TYPE::DEC_RADIX),
      m_CurrentAngleType(NumbersAndOperatorsEnum::Degree),
      m_OpenParenthesisCount(L""), m_Announcement(nullptr),
      m_feedbackForButtonPress(nullptr), m_isRtlLanguage(false),
  ...
  m_expressionAutomationNameFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          CalculatorResourceKeys::CalculatorExpression);
  m_localizedCalculationResultAutomationFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          CalculatorResourceKeys::CalculatorResults);
  m_localizedCalculationResultDecimalAutomationFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          CalculatorResourceKeys::CalculatorResults_DecimalSeparator_Announced);
  m_localizedHexaDecimalAutomationFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          CalculatorResourceKeys::HexButton);
  m_localizedDecimalAutomationFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          CalculatorResourceKeys::DecButton);
  m_localizedOctalAutomationFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          CalculatorResourceKeys::OctButton);
  m_localizedBinaryAutomationFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          CalculatorResourceKeys::BinButton);
  m_leftParenthesisAutomationFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          CalculatorResourceKeys::LeftParenthesisAutomationFormat);

  // Initialize the Automation Name
  CalculationResultAutomationName = GetLocalizedStringFormat(
      m_localizedCalculationResultAutomationFormat, m_DisplayValue);
  CalculationExpressionAutomationName =
      GetLocalizedStringFormat(m_expressionAutomationNameFormat, L"");
...
String ^ StandardCalculatorViewModel::LocalizeDisplayValue(
             _In_ wstring const &displayValue, _In_ bool isError) {
  wstring result(displayValue);

  LocalizationSettings::GetInstance().LocalizeDisplayValue(&result);

  // WINBLUE: 440747 - In BiDi languages, error messages need to be wrapped in
  // LRE/PDF
...
String ^ StandardCalculatorViewModel::CalculateNarratorDisplayValue(
             _In_ wstring const &displayValue,
             _In_ String ^ localizedDisplayValue, _In_ bool isError) {
  ...
  if (Utils::IsLastCharacterTarget(displayValue, m_decimalSeparator)) {
    ...
    localizedValue = LocalizeDisplayValue(
        displayValue.substr(0, displayValue.length() - 1), isError);
  ...
  // In Programmer modes using non-base10, we want the strings to be read as
  // literal digits.
...
String ^ StandardCalculatorViewModel::GetNarratorStringReadRawNumbers(
             _In_ String ^ localizedDisplayValue) {
  wstringstream wss;
  auto &locSettings = LocalizationSettings::GetInstance();

  // Insert a space after each digit in the string, to force Narrator to read
...
void StandardCalculatorViewModel::SetPrimaryDisplay(
    _In_ wstring const &displayStringValue, _In_ bool isError) {
  String ^ localizedDisplayStringValue =
      LocalizeDisplayValue(displayStringValue, isError);

  // Set this variable before the DisplayValue is modified, Otherwise the
  // DisplayValue will not match what the narrator is saying
  m_CalculationResultAutomationName = CalculateNarratorDisplayValue(
      displayStringValue, localizedDisplayStringValue, isError);
...
void StandardCalculatorViewModel::SetParenthesisCount(
    _In_ const wstring &parenthesisCount) {
...
void StandardCalculatorViewModel::
  ...
  String ^ parenthesisCount =
      ((m_OpenParenthesisCount == nullptr) ? "0" : m_OpenParenthesisCount);
  wstring localizedParenthesisCount = parenthesisCount->Data();
  LocalizationSettings::GetInstance().LocalizeDisplayValue(
      &localizedParenthesisCount);

  String ^ announcement =
      LocalizationStringUtil::GetLocalizedNarratorAnnouncement(
  ...
  Announcement =
      CalculatorAnnouncement::GetOpenParenthesisCountChangedAnnouncement(
          announcement);
}
...
void StandardCalculatorViewModel::SetNoParenAddedNarratorAnnouncement() {
  ...
  Announcement = CalculatorAnnouncement::GetNoRightParenthesisAddedAnnouncement(
      announcement);
}

void StandardCalculatorViewModel::DisableButtons(
    CommandType selectedExpressionCommandType) {
...
String ^ StandardCalculatorViewModel::GetLeftParenthesisAutomationName() {
  ...
  return GetLocalizedStringFormat(
      m_leftParenthesisAutomationFormat,
      ref new String(localizedParenthesisCount.c_str()));
}

void StandardCalculatorViewModel::SetExpressionDisplay(
    _Inout_ shared_ptr<CalculatorVector<pair<wstring, int>>> const &tokens,
    _Inout_ shared_ptr<CalculatorVector<shared_ptr<IExpressionCommand>>> const
        &commands) {
  m_tokens = tokens;
  m_commands = commands;
  if (!IsEditingEnabled) {
...
void StandardCalculatorViewModel::SetHistoryExpressionDisplay(
  ...
  m_commands =
      make_shared<CalculatorVector<shared_ptr<IExpressionCommand>>>(*commands);
  IsEditingEnabled = false;

  // Setting the History Item Load Mode so that UI does not get updated with
  // recalculation of every token
...
void StandardCalculatorViewModel::SetTokens(
    _Inout_ shared_ptr<CalculatorVector<pair<wstring, int>>> const &tokens) {
  ...
  for (unsigned int i = 0; i < nTokens; ++i) {
    if (SUCCEEDED(tokens->GetAt(i, &currentToken))) {
      ...
      if (!isEditable) {
        ...
        IFTPlatformException(m_commands->GetAt(
            static_cast<unsigned int>(currentToken.second), &command));
      ...
      DisplayExpressionToken ^ expressionToken = ref new DisplayExpressionToken(
          ref new String(currentToken.first.c_str()), i, isEditable, type);
...
void StandardCalculatorViewModel::SetMemorizedNumbers(
    const vector<wstring> &newMemorizedNumbers) {
  const auto &localizer = LocalizationSettings::GetInstance();
  ...
  // A new value is added to the memory
    ...
    while (newMemorizedNumbers.size() > MemorizedNumbers->Size) {
      size_t newValuePosition =
          newMemorizedNumbers.size() - MemorizedNumbers->Size - 1;
      ...
      memorySlot->Value =
          Utils::LRO + ref new String(stringValue.c_str()) + Utils::PDF;
  ...
  } else if (newMemorizedNumbers.size() ==
             MemorizedNumbers->Size) // Either M+ or M-
  {
    for (unsigned int i = 0; i < MemorizedNumbers->Size; i++) {
      ...
      if (MemorizedNumbers->GetAt(i)->Value !=
          StringReference(newStringValue.c_str())) {
        MemorizedNumbers->GetAt(i)->Value =
            Utils::LRO + ref new String(newStringValue.c_str()) + Utils::PDF;
      }
    }
  }
}
...
void StandardCalculatorViewModel::HandleUpdatedOperandData(Command cmdenum) {
  DisplayExpressionToken ^ displayExpressionToken =
      ExpressionTokens->GetAt(m_tokenPosition);
  if (displayExpressionToken == nullptr) {
    return;
  }
  if ((displayExpressionToken->Token == nullptr) ||
      (displayExpressionToken->Token->Length() == 0)) {
...
bool StandardCalculatorViewModel::IsOperator(Command cmdenum) {
  if ((cmdenum >= Command::Command0 && cmdenum <= Command::Command9) ||
      (cmdenum == Command::CommandPNT) || (cmdenum == Command::CommandBACK) ||
      (cmdenum == Command::CommandEXP) || (cmdenum == Command::CommandFE) ||
      (cmdenum == Command::ModeBasic) || (cmdenum == Command::ModeProgrammer) ||
      (cmdenum == Command::ModeScientific) ||
      (cmdenum == Command::CommandINV) || (cmdenum == Command::CommandCENTR) ||
      (cmdenum == Command::CommandDEG) || (cmdenum == Command::CommandRAD) ||
      (cmdenum == Command::CommandGRAD) ||
      ((cmdenum >= Command::CommandBINEDITSTART) &&
       (cmdenum <= Command::CommandBINEDITEND))) {
    return false;
  }
  return true;
}
  ...
  if (IsEditingEnabled &&
    ...
    if (numOpEnum == NumbersAndOperatorsEnum::Memory) {
      ...
      if (numOpEnum == NumbersAndOperatorsEnum::Clear ||
          ...
          numOpEnum == NumbersAndOperatorsEnum::IsStandardMode ||
          numOpEnum == NumbersAndOperatorsEnum::IsProgrammerMode) {
        // On Clear('C') the F-E button needs to be UnChecked if it in Checked
        // state. Also, the Primary Display Value should not show in exponential
        // format. Hence the check below to ensure parity with Desktop
        // Calculator. Clear the FE mode if the switching to StandardMode, since
        // 'C'/'CE' in StandardMode doesn't honor the FE button.
        if (IsFToEChecked) {
          IsFToEChecked = false;
        }
      }
      ...
      if ((cmdenum >= Command::Command0 && cmdenum <= Command::Command9) ||
          (cmdenum == Command::CommandPNT) ||
          (cmdenum == Command::CommandBACK) ||
...
void StandardCalculatorViewModel::OnCopyCommand(Object ^ parameter) {
  ...
  String ^ announcement = AppResourceProvider::GetInstance().GetResourceString(
      CalculatorResourceKeys::DisplayCopied);
  Announcement =
      CalculatorAnnouncement::GetDisplayCopiedAnnouncement(announcement);
}
...
void StandardCalculatorViewModel::OnPasteCommand(Object ^ parameter) {
  ...
  CopyPasteManager::GetStringToPaste(mode, NavCategory::GetGroupType(mode),
                                     NumberBase, bitLengthType)
      .then(
          [this, mode](String ^ pastedString) { OnPaste(pastedString, mode); },
          concurrency::task_continuation_context::use_current());
}

CalculationManager::Command StandardCalculatorViewModel::ConvertToOperatorsEnum(
...
void StandardCalculatorViewModel::OnPaste(String ^ pastedString,
                                          ViewMode mode) {
  // If pastedString is invalid("NoOp") then display pasteError else process the
  // string
  ...
  // Iterate through each character pasted, and if it's valid, send it to the
  // model.
  ...
  while (it != pastedString->End()) {
    ...
    NumbersAndOperatorsEnum mappedNumOp =
        MapCharacterToButtonId(*it, canSendNegate);
    ...
    if (mappedNumOp != NumbersAndOperatorsEnum::None) {
      switch (mappedNumOp) {
        // Opening parenthesis starts a new expression and pushes negation state
        // onto the stack
      case NumbersAndOperatorsEnum::OpenParenthesis:
        ...
        // Closing parenthesis pops the negation state off the stack and sends
        // it down to the calc engine
      case NumbersAndOperatorsEnum::CloseParenthesis:
        if (!negateStack.empty()) {
          ...
          // Don't send a closing parenthesis if a matching opening parenthesis
      ...
      if (sendCommand) {
        ...
        // The CalcEngine state machine won't allow the negate command to be
        // sent before any other digits, so instead a flag is set and the
        // command is sent after the first appropriate command.
        if (sendNegate) {
          if (canSendNegate) {
            Command cmdNegate =
                ConvertToOperatorsEnum(NumbersAndOperatorsEnum::Negate);
            m_standardCalculatorManager.SendCommand(cmdNegate);
          }

          // Can't send negate on a leading zero, so wait until the appropriate
          // time to send it.
          if (NumbersAndOperatorsEnum::Zero != mappedNumOp &&
              NumbersAndOperatorsEnum::Decimal != mappedNumOp) {
            sendNegate = false;
          }
        }
      }
    ...
    if (mappedNumOp == NumbersAndOperatorsEnum::Exp) {
      ++it;
      if (!(MapCharacterToButtonId(*it, canSendNegate) ==
...
void StandardCalculatorViewModel::OnClearMemoryCommand(Object ^ parameter) {
  ...
  String ^ announcement =
      LocalizationStringUtil::GetLocalizedNarratorAnnouncement(
          CalculatorResourceKeys::MemoryCleared, m_localizedMemoryCleared);
  Announcement =
      CalculatorAnnouncement::GetMemoryClearedAnnouncement(announcement);
}
...
StandardCalculatorViewModel::MapCharacterToButtonId(const wchar_t ch,
                                                    bool &canSendNegate) {
  ...
  case '9':
    mappedValue = NumbersAndOperatorsEnum::Zero +
                  static_cast<NumbersAndOperatorsEnum>(ch - L'0');
    canSendNegate = true;
    break;
  ...
  if (mappedValue == NumbersAndOperatorsEnum::None) {
    if (LocalizationSettings::GetInstance().IsLocalizedDigit(ch)) {
      mappedValue =
          ...
          static_cast<NumbersAndOperatorsEnum>(
              ch -
              LocalizationSettings::GetInstance().GetDigitSymbolFromEnUsDigit(
                  '0'));
      canSendNegate = true;
    }
  }
...
void StandardCalculatorViewModel::OnMemoryButtonPressed() {
  ...
  int windowId = Utils::GetWindowId();
  TraceLogger::GetInstance().InsertIntoMemoryMap(windowId, IsStandard,
                                                 IsScientific, IsProgrammer);

  String ^ announcement =
      LocalizationStringUtil::GetLocalizedNarratorAnnouncement(
          CalculatorResourceKeys::MemorySave,
          m_localizedMemorySavedAutomationFormat, m_DisplayValue->Data());

  Announcement =
      CalculatorAnnouncement::GetMemoryItemAddedAnnouncement(announcement);
}

void StandardCalculatorViewModel::OnMemoryItemChanged(
    unsigned int indexOfMemory) {
  if (indexOfMemory < MemorizedNumbers->Size) {
    ...
    Announcement =
        CalculatorAnnouncement::GetMemoryItemChangedAnnouncement(announcement);
  }
}

void StandardCalculatorViewModel::OnMemoryItemPressed(Object ^
                                                      memoryItemPosition) {
  if (MemorizedNumbers && MemorizedNumbers->Size > 0) {
    ...
    HideMemoryClicked();
    int windowId = Utils::GetWindowId();
    TraceLogger::GetInstance().LogMemoryUsed(
        windowId, boxedPosition->Value, IsStandard, IsScientific, IsProgrammer,
        MemorizedNumbers->Size);
  }
}
...
void StandardCalculatorViewModel::OnMemoryAdd(Object ^ memoryItemPosition) {
  ...
  if (MemorizedNumbers) {
    ...
    if (MemorizedNumbers->Size > 0) {
      TraceLogger::GetInstance().LogMemoryUsed(
          windowId, boxedPosition->Value, IsStandard, IsScientific,
          IsProgrammer, MemorizedNumbers->Size);
      TraceLogger::GetInstance().UpdateMemoryMap(windowId, boxedPosition->Value,
                                                 IsStandard, IsScientific,
                                                 IsProgrammer);
    } else {
      TraceLogger::GetInstance().InsertIntoMemoryMap(
          windowId, IsStandard, IsScientific, IsProgrammer);
    }
    m_standardCalculatorManager.MemorizedNumberAdd(boxedPosition->Value);
  }
}

void StandardCalculatorViewModel::OnMemorySubtract(Object ^
                                                   memoryItemPosition) {
  int windowId = Utils::GetWindowId();

  // M- will add negative of displayed number to memorylist if memory list is
  // empty.
  if (MemorizedNumbers) {
    auto boxedPosition = safe_cast<Box<int> ^>(memoryItemPosition);
    if (MemorizedNumbers->Size > 0) {
...
void StandardCalculatorViewModel::OnMemoryClear(_In_ Object ^
                                                memoryItemPosition) {
  if (MemorizedNumbers && MemorizedNumbers->Size > 0) {
    int windowId = Utils::GetWindowId();
    auto boxedPosition = safe_cast<Box<int> ^>(memoryItemPosition);
    ...
    if (boxedPosition->Value >= 0) {
      unsigned int unsignedPosition =
          safe_cast<unsigned int>(boxedPosition->Value);
      ...
      TraceLogger::GetInstance().LogMemoryUsed(
          windowId, boxedPosition->Value, IsStandard, IsScientific,
          IsProgrammer, MemorizedNumbers->Size);
      TraceLogger::GetInstance().DeleteFromMemoryMap(windowId,
                                                     boxedPosition->Value);
...
Array<unsigned char> ^ StandardCalculatorViewModel::Serialize() {
  ...
  writer->WriteUInt32(static_cast<UINT32>(
      m_standardCalculatorManager.SerializeSavedDegreeMode()));
  ...
  vector<long> serializedPrimaryDisplay =
      m_standardCalculatorManager.GetSerializedPrimaryDisplay();
  ...
  vector<unsigned char> serializedCommand =
      m_standardCalculatorManager.SerializeCommands();
  writer->WriteUInt32(static_cast<UINT32>(serializedCommand.size()));
  writer->WriteBytes(ref new Array<unsigned char>(
      serializedCommand.data(),
      static_cast<unsigned int>(serializedCommand.size())));
  ...
  Platform::Array<unsigned char> ^ viewModelDataAsBytes =
      ref new Array<unsigned char>(buffer->Length);
...
void StandardCalculatorViewModel::Deserialize(Array<unsigned char> ^ state) {
  ...
  if (buffer->Length != 0) {
    DataReader ^ reader = DataReader::FromBuffer(buffer);
    m_CurrentAngleType =
        ConvertIntegerToNumbersAndOperatorsEnum(reader->ReadUInt32());
    ...
    m_standardCalculatorManager.DeSerializePrimaryDisplay(
        serializedPrimaryDisplay);

    CurrentRadixType = reader->ReadUInt32();
    // Read command data and Deserialize
    UINT32 modeldatalength = reader->ReadUInt32();
    Array<unsigned char> ^ modelDataAsBytes =
        ref new Array<unsigned char>(modeldatalength);
    reader->ReadBytes(modelDataAsBytes);
    m_standardCalculatorManager.DeSerializeCommands(vector<unsigned char>(
        modelDataAsBytes->begin(), modelDataAsBytes->end()));

    // After recalculation. If there is an error then
    // IsInError should be set synchronously.
    if (IsInError) {
      shared_ptr<CalculatorVector<shared_ptr<IExpressionCommand>>>
          commandVector = Utils::DeserializeCommands(reader);
      shared_ptr<CalculatorVector<pair<wstring, int>>> tokenVector =
          Utils::DeserializeTokens(reader);
...
// Given a format string, returns a string with the input display value
// inserted.
//     'format' is a localized string containing a %1 formatting mark where the
//     display value should be inserted. 'displayValue' is a localized string
//     containing a numerical value to be displayed to the user.
String ^ StandardCalculatorViewModel::GetLocalizedStringFormat(
             String ^ format, String ^ displayValue) {
  String ^ localizedString =
      ref new String(LocalizationStringUtil::GetLocalizedString(
                         format->Data(), displayValue->Data())
                         .c_str());
...
void StandardCalculatorViewModel::SwitchProgrammerModeBase(
...
void StandardCalculatorViewModel::SaveEditedCommand(
    _In_ unsigned int tokenPosition, _In_ Command command) {
  ...
  if (IsUnaryOp(nOpCode) && command != Command::CommandSIGN) {
    int angleCmd =
        static_cast<int>(m_standardCalculatorManager.GetCurrentDegreeMode());
    ANGLE_TYPE angleType =
        GetAngleTypeFromCommand(static_cast<Command>(angleCmd));

    if (IsTrigOp(nOpCode)) {
      shared_ptr<IUnaryCommand> spUnaryCommand =
          dynamic_pointer_cast<IUnaryCommand>(tokenCommand);
    ...
    case static_cast<int>(Command::CommandASIN):
      updatedToken = CCalcEngine::OpCodeToUnaryString(
          static_cast<int>(Command::CommandSIN), true, angleType);
      break;
    case static_cast<int>(Command::CommandACOS):
      updatedToken = CCalcEngine::OpCodeToUnaryString(
          static_cast<int>(Command::CommandCOS), true, angleType);
      break;
    case static_cast<int>(Command::CommandATAN):
      updatedToken = CCalcEngine::OpCodeToUnaryString(
          static_cast<int>(Command::CommandTAN), true, angleType);
      break;
    case static_cast<int>(Command::CommandASINH):
      updatedToken = CCalcEngine::OpCodeToUnaryString(
          static_cast<int>(Command::CommandSINH), true, angleType);
      break;
    case static_cast<int>(Command::CommandACOSH):
      updatedToken = CCalcEngine::OpCodeToUnaryString(
          static_cast<int>(Command::CommandCOSH), true, angleType);
      break;
    case static_cast<int>(Command::CommandATANH):
      updatedToken = CCalcEngine::OpCodeToUnaryString(
          static_cast<int>(Command::CommandTANH), true, angleType);
      break;
    case static_cast<int>(Command::CommandPOWE):
      updatedToken = CCalcEngine::OpCodeToUnaryString(
          static_cast<int>(Command::CommandLN), true, angleType);
      break;
    default:
      updatedToken =
          CCalcEngine::OpCodeToUnaryString(nOpCode, false, angleType);
    }
    if ((token.first.length() > 0) &&
        (token.first[token.first.length() - 1] == L'(')) {
      wstring chOpenBrace = L"(";
      updatedToken.append(chOpenBrace);
    }
  } else if (IsBinOp(nOpCode)) {
    shared_ptr<IBinaryCommand> spBinaryCommand =
        dynamic_pointer_cast<IBinaryCommand>(tokenCommand);
    ...
    if (tokenCommand->GetCommandType() == CommandType::UnaryCommand) {
      shared_ptr<IExpressionCommand> spSignCommand =
          make_shared<CUnaryCommand>(nOpCode);
      IFTPlatformException(
          m_commands->InsertAt(tokenCommandIndex + 1, spSignCommand));
    } else {
      shared_ptr<IOpndCommand> spOpndCommand =
          dynamic_pointer_cast<IOpndCommand>(tokenCommand);
      spOpndCommand->ToggleSign();
      updatedToken = spOpndCommand->GetToken(
          m_standardCalculatorManager.DecimalSeparator());
    }
    IsOperandUpdatedUsingViewModel = true;
  }
  ...
  if (!handleOperand) {
    ...
    DisplayExpressionToken ^ displayExpressionToken =
        ExpressionTokens->GetAt(tokenPosition);
    displayExpressionToken->Token =
        ref new Platform::String(updatedToken.c_str());

    // Special casing
    if (command == Command::CommandSIGN &&
        tokenCommand->GetCommandType() == CommandType::UnaryCommand) {
      IsEditingEnabled = false;
      Recalculate();
    }
  }
...
void StandardCalculatorViewModel::Recalculate(bool fromHistory) {
  // Recalculate
  Command currentDegreeMode =
      m_standardCalculatorManager.GetCurrentDegreeMode();
  shared_ptr<CalculatorVector<shared_ptr<IExpressionCommand>>> savedCommands =
  ...
  for (unsigned int i = 0; i < commandListCount; i++) {
    ...
    if (commandType == CommandType::UnaryCommand) {
      shared_ptr<IUnaryCommand> spCommand =
          dynamic_pointer_cast<IUnaryCommand>(command);
      shared_ptr<CalculatorVector<int>> unaryCommands =
          spCommand->GetCommands();
    ...
    if (commandType == CommandType::BinaryCommand) {
      shared_ptr<IBinaryCommand> spCommand =
          dynamic_pointer_cast<IBinaryCommand>(command);
      currentCommands.push_back(spCommand->GetCommand());
    }

    if (commandType == CommandType::Parentheses) {
      shared_ptr<IParenthesisCommand> spCommand =
          dynamic_pointer_cast<IParenthesisCommand>(command);
      currentCommands.push_back(spCommand->GetCommand());
    }

    if (commandType == CommandType::OperandCommand) {
      shared_ptr<IOpndCommand> spCommand =
          dynamic_pointer_cast<IOpndCommand>(command);
      ...
      for (unsigned int j = 0; j < opndCommandCount; ++j) {
        ...
        if (fNeedIDCSign && nOCode != IDC_0) {
          currentCommands.push_back(
              static_cast<int>(CalculationManager::Command::CommandSIGN));
          fNeedIDCSign = false;
        }
      }
    }
  }
  shared_ptr<CalculatorVector<pair<wstring, int>>> savedTokens =
      make_shared<CalculatorVector<pair<wstring, int>>>();
  ...
  for (size_t i = 0; i < currentCommandsSize; i++) {
    m_standardCalculatorManager.SendCommand(
        static_cast<CalculationManager::Command>(currentCommands[i]));
  }

  if (fromHistory) // This is for the cases where the expression is loaded from
                   // history
  {
    // To maintain F-E state of the engine, as the last operand hasn't reached
    // engine by now
    m_standardCalculatorManager.SendCommand(Command::CommandFE);
    m_standardCalculatorManager.SendCommand(Command::CommandFE);
  }
...
CommandType StandardCalculatorViewModel::GetSelectedTokenType(
    _In_ unsigned int tokenPosition) {
  pair<wstring, int> token;
  shared_ptr<IExpressionCommand> tokenCommand;
  IFTPlatformException(m_tokens->GetAt(tokenPosition, &token));
...
bool StandardCalculatorViewModel::IsOpnd(int nOpCode) {
  ...
  static Command opnd[] = {
      Command::Command0, Command::Command1,  Command::Command2,
      Command::Command3, Command::Command4,  Command::Command5,
      Command::Command6, Command::Command7,  Command::Command8,
      Command::Command9, Command::CommandPNT};
...
bool StandardCalculatorViewModel::IsUnaryOp(int nOpCode) {
  static Command unaryOp[] = {
      Command::CommandSQRT,  Command::CommandFAC,   Command::CommandSQR,
      Command::CommandLOG,   Command::CommandPOW10, Command::CommandPOWE,
      Command::CommandLN,    Command::CommandREC,   Command::CommandSIGN,
      Command::CommandSINH,  Command::CommandASINH, Command::CommandCOSH,
      Command::CommandACOSH, Command::CommandTANH,  Command::CommandATANH,
      Command::CommandCUB};
...
bool StandardCalculatorViewModel::IsTrigOp(int nOpCode) {
  static Command trigOp[] = {Command::CommandSIN,  Command::CommandCOS,
                             Command::CommandTAN,  Command::CommandASIN,
                             Command::CommandACOS, Command::CommandATAN};
...
bool StandardCalculatorViewModel::IsBinOp(int nOpCode) {
  static Command binOp[] = {Command::CommandADD, Command::CommandSUB,
                            Command::CommandMUL, Command::CommandDIV,
                            Command::CommandEXP, Command::CommandROOT,
                            Command::CommandMOD, Command::CommandPWR};
...
bool StandardCalculatorViewModel::IsRecoverableCommand(int nOpCode) {
  ...
  static Command recoverableCommands[] = {Command::CommandA, Command::CommandB,
                                          Command::CommandC, Command::CommandD,
...
wstring StandardCalculatorViewModel::AddPadding(wstring binaryString) {
  if (LocalizationSettings::GetInstance().GetEnglishValueFromLocalizedDigits(
          binaryString) == L"0") {
    return binaryString;
  }
...
void StandardCalculatorViewModel::UpdateProgrammerPanelDisplay() {
  ...
  if (!IsInError) {
    // we want the precision to be set to maximum value so that the
    // autoconversions result as desired
    ...
    if (m_standardCalculatorManager.GetResultForRadix(16, precision) == L"") {
      ...
      hexDisplayString =
          m_standardCalculatorManager.GetResultForRadix(16, precision);
      decimalDisplayString =
          m_standardCalculatorManager.GetResultForRadix(10, precision);
      octalDisplayString =
          m_standardCalculatorManager.GetResultForRadix(8, precision);
      binaryDisplayString =
          m_standardCalculatorManager.GetResultForRadix(2, precision);
    }
  }
  const auto &localizer = LocalizationSettings::GetInstance();
  ...
  HexDisplayValue = Utils::LRO +
                    ref new Platform::String(hexDisplayString.c_str()) +
                    Utils::PDF;
  DecimalDisplayValue = Utils::LRO +
                        ref new Platform::String(decimalDisplayString.c_str()) +
                        Utils::PDF;
  OctalDisplayValue = Utils::LRO +
                      ref new Platform::String(octalDisplayString.c_str()) +
                      Utils::PDF;
  BinaryDisplayValue = Utils::LRO +
                       ref new Platform::String(binaryDisplayString.c_str()) +
                       Utils::PDF;
  HexDisplayValue_AutomationName = GetLocalizedStringFormat(
      m_localizedHexaDecimalAutomationFormat,
      GetNarratorStringReadRawNumbers(HexDisplayValue));
  DecDisplayValue_AutomationName = GetLocalizedStringFormat(
      m_localizedDecimalAutomationFormat, DecimalDisplayValue);
  OctDisplayValue_AutomationName = GetLocalizedStringFormat(
      m_localizedOctalAutomationFormat,
      GetNarratorStringReadRawNumbers(OctalDisplayValue));
  BinDisplayValue_AutomationName = GetLocalizedStringFormat(
      m_localizedBinaryAutomationFormat,
      GetNarratorStringReadRawNumbers(BinaryDisplayValue));
}
...
NumbersAndOperatorsEnum
StandardCalculatorViewModel::ConvertIntegerToNumbersAndOperatorsEnum(
...
void StandardCalculatorViewModel::UpdateOperand(int pos, String ^ text) {
  ...
  String ^ englishString =
      LocalizationSettings::GetInstance().GetEnglishValueFromLocalizedDigits(
          text->Data());
  ...
  if (operandCommand != nullptr) {
    shared_ptr<CalculatorVector<int>> commands =
        make_shared<CalculatorVector<int>>();
    ...
    if (length > 0) {
      ...
      for (unsigned int i = 0; i < length; ++i) {
        if (p.first[i] == L'.') {
          ...
          if (i == 0) {
            shared_ptr<IOpndCommand> spOpndCommand =
                dynamic_pointer_cast<IOpndCommand>(exprCmd);
...
void StandardCalculatorViewModel::UpdatecommandsInRecordingMode() {
  vector<unsigned char> savedCommands =
      m_standardCalculatorManager.GetSavedCommands();
  shared_ptr<CalculatorVector<int>> commands =
      make_shared<CalculatorVector<int>>();
  ...
  if (size > 0) {
    shared_ptr<IOpndCommand> sp =
        make_shared<COpndCommand>(commands, isNegative, isDecimal, isExpMode);
...
void StandardCalculatorViewModel::OnMaxDigitsReached() {
  ...
  Announcement =
      CalculatorAnnouncement::GetMaxDigitsReachedAnnouncement(announcement);
}
...
NarratorAnnouncement ^
    StandardCalculatorViewModel::GetDisplayUpdatedNarratorAnnouncement() {
  String ^ announcement;
  if (m_feedbackForButtonPress == nullptr ||
      m_feedbackForButtonPress->IsEmpty()) {
