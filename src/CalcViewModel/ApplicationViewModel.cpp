...
ApplicationViewModel::ApplicationViewModel()
    : m_CalculatorViewModel(nullptr), m_DateCalcViewModel(nullptr),
      m_ConverterViewModel(nullptr), m_PreviousMode(ViewMode::None),
      m_mode(ViewMode::None), m_categories(nullptr) {
  SetMenuCategories();
}
...
void ApplicationViewModel::Categories::set(
    IObservableVector<NavCategoryGroup ^> ^ value) {
...
bool ApplicationViewModel::TryRecoverFromNavigationModeFailure() {
  // Here we are simply trying to recover from being unable to navigate to a
  // mode. Try falling back to standard mode and if there are *any* exceptions,
  // we should fail because something is seriously wrong.
  try {
    Mode = ViewMode::Standard;
    return true;
  } catch (...) {
...
void ApplicationViewModel::OnModeChanged() {
  ...
  TraceLogger::GetInstance().LogModeChangeBegin(
      m_PreviousMode, m_mode,
      ApplicationView::GetApplicationViewIdForWindow(
          CoreWindow::GetForCurrentThread()));
  if (NavCategory::IsCalculatorViewMode(m_mode)) {
    TraceLogger::GetInstance().LogCalculatorModeViewed(
        m_mode, ApplicationView::GetApplicationViewIdForWindow(
                    CoreWindow::GetForCurrentThread()));
    ...
    TraceLogger::GetInstance().LogDateCalculatorModeViewed(
        m_mode, ApplicationView::GetApplicationViewIdForWindow(
                    CoreWindow::GetForCurrentThread()));
  ...
  CategoryName =
      resProvider.GetResourceString(NavCategory::GetNameResourceKey(m_mode));
  ...
  // Don't save until after we have adjusted to the new mode, so we don't save a
  // mode that fails to load.
  ApplicationData::Current->LocalSettings->Values->Insert(
      ModePropertyName, NavCategory::Serialize(m_mode));

  TraceLogger::GetInstance().LogModeChangeEnd(
      m_mode, ApplicationView::GetApplicationViewIdForWindow(
                  CoreWindow::GetForCurrentThread()));
