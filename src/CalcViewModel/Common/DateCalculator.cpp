...
bool DateCalculationEngine::AddDuration(_In_ DateTime startDate,
                                        _In_ const DateDifference &duration,
                                        _Out_ DateTime *endDate) {
  try {
...
// Returns: True if function succeeds to calculate the date else returns False
bool DateCalculationEngine::SubtractDuration(
    _In_ DateTime startDate, _In_ const DateDifference &duration,
    _Out_ DateTime *endDate) {
  // For Subtract the Algorithm is different than Add. Here the smaller units
  // are subtracted first and then the larger units.
  try {
    m_calendar->SetDateTime(startDate);
...
void DateCalculationEngine::GetDateDifference(
    _In_ DateTime date1, _In_ DateTime date2, _In_ DateUnit outputFormat,
  ...
  if (static_cast<int>(outputFormat) & 7) {
    ...
    // If we're unable to calculate the days-in-month or days-in-year, we'll
    ...
    if (TryGetCalendarDaysInMonth(startDate, daysInMonth) &&
      ...
      UINT daysIn[c_unitsOfDate] = {approximateDaysInYear, daysInMonth,
                                    c_daysInWeek, 1};
      ...
      for (int unitIndex = 0; unitIndex < c_unitsGreaterThanDays; unitIndex++) {
        ...
        if (static_cast<int>(outputFormat & dateUnit)) {
          ...
          if (differenceInDates[unitIndex] != 0) {
            try {
              pivotDate = AdjustCalendarDate(
                  pivotDate, dateUnit,
                  static_cast<int>(differenceInDates[unitIndex]));
          ...
          do {
            ...
            if (tempDaysDiff < 0) {
              // pivotDate has gone over the end date; start from the beginning
              // of this unit
              ...
              if (isEndDateHit) {
                // This is the closest the pivot can get to the end date for
                // this unit
                break;
              }
          ...
          tempPivotDate = AdjustCalendarDate(
              tempPivotDate, dateUnit,
              static_cast<int>(differenceInDates[unitIndex]));
          pivotDate = tempPivotDate;
...
bool DateCalculationEngine::TryGetCalendarDaysInMonth(_In_ DateTime date,
                                                      _Out_ UINT &daysInMonth) {
  bool result = false;
...
// Returns true if successful, false otherwise.
bool DateCalculationEngine::TryGetCalendarDaysInYear(_In_ DateTime date,
                                                     _Out_ UINT &daysInYear) {
  bool result = false;
  UINT days = 0;
...
DateCalculationEngine::AdjustCalendarDate(Windows::Foundation::DateTime date,
  ...
  case DateUnit::Year: {
    ...
    // It is not guaranteed that adding 1 year will always add 365 days in the
    // Japanese Calendar. To work around this quirk, we will change the calendar
    // system to Gregorian before adding 1 year in the Japanese Calendar case
    // only. We will then return the calendar system back to the Japanese
    // Calendar.
