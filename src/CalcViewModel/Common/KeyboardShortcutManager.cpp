...
DEPENDENCY_PROPERTY_INITIALIZATION(KeyboardShortcutManager,
...
DEPENDENCY_PROPERTY_INITIALIZATION(KeyboardShortcutManager,
                                   VirtualKeyControlInverseChord);

static multimap<int, multimap<wchar_t, WeakReference>> s_CharacterForButtons;
static multimap<int, multimap<MyVirtualKey, WeakReference>>
    s_VirtualKeysForButtons;
static multimap<int, multimap<MyVirtualKey, WeakReference>>
    s_VirtualKeyControlChordsForButtons;
static multimap<int, multimap<MyVirtualKey, WeakReference>>
    s_VirtualKeyShiftChordsForButtons;
static multimap<int, multimap<MyVirtualKey, WeakReference>>
    s_VirtualKeyAltChordsForButtons;
static multimap<int, multimap<MyVirtualKey, WeakReference>>
    s_VirtualKeyControlShiftChordsForButtons;
static multimap<int, multimap<MyVirtualKey, WeakReference>>
    s_VirtualKeyInverseChordsForButtons;
static multimap<int, multimap<MyVirtualKey, WeakReference>>
    s_VirtualKeyControlInverseChordsForButtons;
...
template <typename T> void LightUpButtons(const T &buttons) {
...
template <typename T> void RunFirstEnabledButtonCommand(const T &buttons) {
...
void KeyboardShortcutManager::OnCharacterPropertyChanged(
    DependencyObject ^ target, String ^ oldValue, String ^ newValue) {
  // Writer lock for the static maps
  reader_writer_lock::scoped_lock lock(s_keyboardShortcutMapLock);
  ...
  if (iterViewMap != s_CharacterForButtons.end()) {
    ...
    if (newValue) {
      if (newValue == L".") {
        ...
        iterViewMap->second.insert(
            std::make_pair(decSep, WeakReference(button)));
      } else {
        iterViewMap->second.insert(
            std::make_pair(newValue->Data()[0], WeakReference(button)));
      }
    }
  } else {
    s_CharacterForButtons.insert(
        std::make_pair(viewId, std::multimap<wchar_t, WeakReference>()));

    if (newValue == L".") {
      wchar_t decSep =
          LocalizationSettings::GetInstance().GetDecimalSeparator();
      s_CharacterForButtons.find(viewId)->second.insert(
          std::make_pair(decSep, WeakReference(button)));
    } else {
      s_CharacterForButtons.find(viewId)->second.insert(
          std::make_pair(newValue->Data()[0], WeakReference(button)));
...
void KeyboardShortcutManager::OnVirtualKeyPropertyChanged(
    DependencyObject ^ target, MyVirtualKey /*oldValue*/,
  ...
  if (iterViewMap != s_VirtualKeysForButtons.end()) {
    ...
    // If the View Id is not already registered, then register it and make the
    // entry
    s_VirtualKeysForButtons.insert(
        std::make_pair(viewId, std::multimap<MyVirtualKey, WeakReference>()));
    s_VirtualKeysForButtons.find(viewId)->second.insert(
        std::make_pair(newValue, WeakReference(button)));
  }
}
...
void KeyboardShortcutManager::OnVirtualKeyControlChordPropertyChanged(
  ...
  if (control == nullptr) {
    // Handling Ctrl+E shortcut for Date Calc, target would be NavigationView^
    // in that case
  ...
  if (iterViewMap != s_VirtualKeyControlChordsForButtons.end()) {
    iterViewMap->second.insert(
        std::make_pair(newValue, WeakReference(control)));
  } else {
    // If the View Id is not already registered, then register it and make the
    // entry
    s_VirtualKeyControlChordsForButtons.insert(
        std::make_pair(viewId, std::multimap<MyVirtualKey, WeakReference>()));
    s_VirtualKeyControlChordsForButtons.find(viewId)->second.insert(
        std::make_pair(newValue, WeakReference(control)));
  }
}
...
void KeyboardShortcutManager::OnVirtualKeyShiftChordPropertyChanged(
  ...
  if (iterViewMap != s_VirtualKeyShiftChordsForButtons.end()) {
    ...
    // If the View Id is not already registered, then register it and make the
    // entry
    s_VirtualKeyShiftChordsForButtons.insert(
        std::make_pair(viewId, std::multimap<MyVirtualKey, WeakReference>()));
    s_VirtualKeyShiftChordsForButtons.find(viewId)->second.insert(
        std::make_pair(newValue, WeakReference(button)));
  }
}
...
void KeyboardShortcutManager::OnVirtualKeyAltChordPropertyChanged(
  ...
  if (iterViewMap != s_VirtualKeyAltChordsForButtons.end()) {
    iterViewMap->second.insert(
        std::make_pair(newValue, WeakReference(navView)));
  } else {
    // If the View Id is not already registered, then register it and make the
    // entry
    s_VirtualKeyAltChordsForButtons.insert(
        std::make_pair(viewId, std::multimap<MyVirtualKey, WeakReference>()));
    s_VirtualKeyAltChordsForButtons.find(viewId)->second.insert(
        std::make_pair(newValue, WeakReference(navView)));
  }
}
...
void KeyboardShortcutManager::OnVirtualKeyControlShiftChordPropertyChanged(
  ...
  if (iterViewMap != s_VirtualKeyControlShiftChordsForButtons.end()) {
    ...
    // If the View Id is not already registered, then register it and make the
    // entry
    s_VirtualKeyControlShiftChordsForButtons.insert(
        std::make_pair(viewId, std::multimap<MyVirtualKey, WeakReference>()));
    s_VirtualKeyControlShiftChordsForButtons.find(viewId)->second.insert(
        std::make_pair(newValue, WeakReference(button)));
  }
}
...
void KeyboardShortcutManager::OnVirtualKeyInverseChordPropertyChanged(
  ...
  if (iterViewMap != s_VirtualKeyInverseChordsForButtons.end()) {
    ...
    // If the View Id is not already registered, then register it and make the
    // entry
    s_VirtualKeyInverseChordsForButtons.insert(
        std::make_pair(viewId, std::multimap<MyVirtualKey, WeakReference>()));
    s_VirtualKeyInverseChordsForButtons.find(viewId)->second.insert(
        std::make_pair(newValue, WeakReference(button)));
  }
}
...
void KeyboardShortcutManager::OnVirtualKeyControlInverseChordPropertyChanged(
  ...
  if (iterViewMap != s_VirtualKeyControlInverseChordsForButtons.end()) {
    ...
    // If the View Id is not already registered, then register it and make the
    // entry
    s_VirtualKeyControlInverseChordsForButtons.insert(
        std::make_pair(viewId, std::multimap<MyVirtualKey, WeakReference>()));
    s_VirtualKeyControlInverseChordsForButtons.find(viewId)->second.insert(
        std::make_pair(newValue, WeakReference(button)));
  }
}
...
void KeyboardShortcutManager::OnCharacterReceivedHandler(
  ...
  if (currentHonorShortcuts != s_fHonorShortcuts.end()) {
    if (currentHonorShortcuts->second) {
      ...
      auto buttons =
          s_CharacterForButtons.find(viewId)->second.equal_range(character);
...
const std::multimap<MyVirtualKey, WeakReference> &
GetCurrentKeyDictionary(MyVirtualKey key, bool altPressed = false) {
  int viewId = Utils::GetWindowId();
  ...
  if (altPressed) {
    ...
    if ((Window::Current->CoreWindow->GetKeyState(VirtualKey::Control) &
         CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down) {
      auto iterViewMap =
          s_VirtualKeyControlInverseChordsForButtons.find(viewId);
      if (iterViewMap != s_VirtualKeyControlInverseChordsForButtons.end()) {
        for (auto iterator = iterViewMap->second.begin();
             iterator != iterViewMap->second.end(); ++iterator) {
          if (key == iterator->first) {
            return s_VirtualKeyControlInverseChordsForButtons.find(viewId)
                ->second;
          }
        }
      }
...
void KeyboardShortcutManager::OnKeyDownHandler(CoreWindow ^ sender,
                                               KeyEventArgs ^ args) {
  ...
  if (!args->Handled) {
    ...
    bool isControlKeyPressed =
        (currentControlKeyPressed != s_ControlKeyPressed.end()) &&
        (currentControlKeyPressed->second);
    bool isShiftKeyPressed =
        (currentShiftKeyPressed != s_ShiftKeyPressed.end()) &&
        (currentShiftKeyPressed->second);

    // Handle Ctrl + E for DateCalculator
    if ((key == VirtualKey::E) && isControlKeyPressed && !isShiftKeyPressed) {
      const auto &lookupMap =
          GetCurrentKeyDictionary(static_cast<MyVirtualKey>(key));
      ...
      auto appViewModel =
          safe_cast<ApplicationViewModel ^>(navView->DataContext);
      appViewModel->Mode = ViewMode::Date;
      auto categoryName = AppResourceProvider::GetInstance().GetResourceString(
          L"DateCalculationModeText");
      appViewModel->CategoryName = categoryName;

      auto menuItems =
          static_cast<IObservableVector<Object ^> ^>(navView->MenuItemsSource);
    ...
    if (currentHonorShortcuts != s_fHonorShortcuts.end()) {
      if (currentHonorShortcuts->second) {
        ...
        // Ctrl+C and Ctrl+V shifts focus to some button because of which enter
        // doesn't work after copy/paste. So don't shift focus if Ctrl+C or
        // Ctrl+V is pressed. When drop down is open, pressing escape shifts
        // focus to clear button. So dont's shift focus if drop down is open.
        // Ctrl+Insert is equivalent to Ctrl+C and Shift+Insert is equivalent to
        // Ctrl+V
        if (currentIsDropDownOpen != s_IsDropDownOpen.end() &&
            !currentIsDropDownOpen->second) {
          // Do not Light Up Buttons when Ctrl+C, Ctrl+V, Ctrl+Insert or
          // Shift+Insert is pressed
          if (!(isControlKeyPressed &&
                (key == VirtualKey::C || key == VirtualKey::V ||
...
void KeyboardShortcutManager::OnKeyUpHandler(CoreWindow ^ sender,
                                             KeyEventArgs ^ args) {
  int viewId = Utils::GetWindowId();
  auto key = args->VirtualKey;
...
void KeyboardShortcutManager::OnAcceleratorKeyActivated(
    CoreDispatcher ^, AcceleratorKeyEventArgs ^ args) {
  if (args->KeyStatus.IsKeyReleased) {
    auto key = args->VirtualKey;
    bool altPressed = args->KeyStatus.IsMenuKeyDown;

    // If the Alt/Menu key is not pressed then we don't care about the key
    // anymore
    if (!altPressed) {
      return;
    }
    ...
    const auto &lookupMap =
        GetCurrentKeyDictionary(static_cast<MyVirtualKey>(key), altPressed);
    auto listItems = lookupMap.equal_range(static_cast<MyVirtualKey>(key));
    for (auto listIterator = listItems.first; listIterator != listItems.second;
         ++listIterator) {
      auto item = listIterator->second.Resolve<MUXC::NavigationView>();
      if (item != nullptr) {
        auto navView = safe_cast<MUXC::NavigationView ^>(item);

        auto menuItems = static_cast<IObservableVector<Object ^> ^>(
            navView->MenuItemsSource);
        if (menuItems != nullptr) {
          auto vm = safe_cast<ApplicationViewModel ^>(navView->DataContext);
          if (nullptr != vm) {
            ViewMode toMode = NavCategory::GetViewModeForVirtualKey(
                static_cast<MyVirtualKey>(key));
            if (NavCategory::IsValidViewMode(toMode)) {
              vm->Mode = toMode;
              navView->SelectedItem =
                  menuItems->GetAt(NavCategory::GetFlatIndex(toMode));
            }
          }
        }
        break;
  ...
  if (args->VirtualKey == VirtualKey::Escape) {
    ...
    if ((iterViewMap != s_AboutFlyout.end()) &&
        (iterViewMap->second != nullptr)) {
...
void KeyboardShortcutManager::Initialize() {
  ...
  coreWindow->CharacterReceived +=
      ref new TypedEventHandler<CoreWindow ^, CharacterReceivedEventArgs ^>(
          &KeyboardShortcutManager::OnCharacterReceivedHandler);
  coreWindow->KeyDown +=
      ref new TypedEventHandler<CoreWindow ^, KeyEventArgs ^>(
          &KeyboardShortcutManager::OnKeyDownHandler);
  coreWindow->KeyUp += ref new TypedEventHandler<CoreWindow ^, KeyEventArgs ^>(
      &KeyboardShortcutManager::OnKeyUpHandler);
  coreWindow->Dispatcher->AcceleratorKeyActivated +=
      ref new TypedEventHandler<CoreDispatcher ^, AcceleratorKeyEventArgs ^>(
          &KeyboardShortcutManager::OnAcceleratorKeyActivated);
...
void KeyboardShortcutManager::RegisterNewAppViewId() {
  ...
  if (s_CharacterForButtons.find(appViewId) == s_CharacterForButtons.end()) {
    s_CharacterForButtons.insert(
        std::make_pair(appViewId, std::multimap<wchar_t, WeakReference>()));
  }

  if (s_VirtualKeysForButtons.find(appViewId) ==
      s_VirtualKeysForButtons.end()) {
    s_VirtualKeysForButtons.insert(std::make_pair(
        appViewId, std::multimap<MyVirtualKey, WeakReference>()));
  }

  if (s_VirtualKeyControlChordsForButtons.find(appViewId) ==
      s_VirtualKeyControlChordsForButtons.end()) {
    s_VirtualKeyControlChordsForButtons.insert(std::make_pair(
        appViewId, std::multimap<MyVirtualKey, WeakReference>()));
  }

  if (s_VirtualKeyShiftChordsForButtons.find(appViewId) ==
      s_VirtualKeyShiftChordsForButtons.end()) {
    s_VirtualKeyShiftChordsForButtons.insert(std::make_pair(
        appViewId, std::multimap<MyVirtualKey, WeakReference>()));
  }

  if (s_VirtualKeyAltChordsForButtons.find(appViewId) ==
      s_VirtualKeyAltChordsForButtons.end()) {
    s_VirtualKeyAltChordsForButtons.insert(std::make_pair(
        appViewId, std::multimap<MyVirtualKey, WeakReference>()));
  }

  if (s_VirtualKeyControlShiftChordsForButtons.find(appViewId) ==
      s_VirtualKeyControlShiftChordsForButtons.end()) {
    s_VirtualKeyControlShiftChordsForButtons.insert(std::make_pair(
        appViewId, std::multimap<MyVirtualKey, WeakReference>()));
  }

  if (s_VirtualKeyInverseChordsForButtons.find(appViewId) ==
      s_VirtualKeyInverseChordsForButtons.end()) {
    s_VirtualKeyInverseChordsForButtons.insert(std::make_pair(
        appViewId, std::multimap<MyVirtualKey, WeakReference>()));
  }

  if (s_VirtualKeyControlInverseChordsForButtons.find(appViewId) ==
      s_VirtualKeyControlInverseChordsForButtons.end()) {
    s_VirtualKeyControlInverseChordsForButtons.insert(std::make_pair(
        appViewId, std::multimap<MyVirtualKey, WeakReference>()));
  }
