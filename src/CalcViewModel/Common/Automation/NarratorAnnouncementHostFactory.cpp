...
// This static variable is used only to call the initialization function, to
// initialize the other static variables.
int NarratorAnnouncementHostFactory::s_init =
    NarratorAnnouncementHostFactory::Initialize();
...
// if not.  The availability of the host depends on the version of the OS the
// app is running on. When the app switches to min version RS3, the
// LiveRegionHost can be removed and we will always use NotificationHost.
// TODO - MSFT 12735088
void NarratorAnnouncementHostFactory::RegisterHosts() {
  ...
  NarratorAnnouncementHostFactory::s_hosts = {ref new NotificationHost(),
                                              ref new LiveRegionHost()};
}

INarratorAnnouncementHost ^ NarratorAnnouncementHostFactory::GetHostProducer() {
  for (INarratorAnnouncementHost ^ host :
