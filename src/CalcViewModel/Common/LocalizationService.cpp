...
// Resources for the engine use numbers as keys. It's inconvenient, but also
// difficult to change given that the engine heavily relies on perfect ordering
// of certain elements. The key for open parenthesis, '(', is "48".
...
LocalizationService::LocalizationService() {
  ...
  m_flowDirection =
      ResourceContext::GetForCurrentView()->QualifierValues->Lookup(
  ...
  m_fontFamilyOverride =
      resourceLoader.GetResourceString(L"LocalizedFontFamilyOverride");

  String ^ reserved = L"RESERVED_FOR_FONTLOC";

  m_overrideFontApiValues =
      ((m_fontFamilyOverride != nullptr) && (m_fontFamilyOverride != reserved));
  if (m_overrideFontApiValues) {
    String ^ localizedUICaptionFontSizeFactorOverride =
        resourceLoader.GetResourceString(
    ...
    String ^ localizedFontWeightOverride =
        resourceLoader.GetResourceString(L"LocalizedFontWeightOverride");

    // If any of the font overrides are modified then all of them need to be
    // modified
    ...
    m_uiTextFontScaleFactorOverride =
        _wtof(localizedUITextFontSizeFactorOverride->Data());
    m_uiCaptionFontScaleFactorOverride =
        _wtof(localizedUICaptionFontSizeFactorOverride->Data());
  }
...
String ^ LocalizationService::GetLanguage() { return m_language; }

bool LocalizationService::GetOverrideFontApiValues() {
  return m_overrideFontApiValues;
}

FontFamily ^ LocalizationService::GetLanguageFontFamilyForType(
                 LanguageFontType fontType) {
  if (m_overrideFontApiValues) {
...
double
LocalizationService::GetFontScaleFactorOverride(LanguageFontType fontType) {
  assert(m_overrideFontApiValues);
...
void LocalizationService::OnFontTypePropertyChanged(
    DependencyObject ^ target, LanguageFontType /*oldValue*/,
    LanguageFontType /*newValue*/) {
  UpdateFontFamilyAndSize(target);
}

void LocalizationService::OnFontWeightPropertyChanged(DependencyObject ^ target,
                                                      FontWeight /*oldValue*/,
                                                      FontWeight /*newValue*/) {
  UpdateFontFamilyAndSize(target);
}

void LocalizationService::OnFontSizePropertyChanged(DependencyObject ^ target,
                                                    double /*oldValue*/,
                                                    double /*newValue*/) {
  UpdateFontFamilyAndSize(target);
}
...
// If successful, returns a formatter that respects the user's regional format
// settings, as configured by running intl.cpl.
DecimalFormatter ^
    LocalizationService::GetRegionalSettingsAwareDecimalFormatter() {
  IIterable<String ^> ^ languageIdentifiers =
      LocalizationService::GetLanguageIdentifiers();
  if (languageIdentifiers != nullptr) {
    return ref new DecimalFormatter(
        languageIdentifiers, GlobalizationPreferences::HomeGeographicRegion);
  }
...
DateTimeFormatter ^
    LocalizationService::GetRegionalSettingsAwareDateTimeFormatter(_In_ String ^
                                                                   format) {
  IIterable<String ^> ^ languageIdentifiers =
      LocalizationService::GetLanguageIdentifiers();
  if (languageIdentifiers == nullptr) {
...
DateTimeFormatter ^
    LocalizationService::GetRegionalSettingsAwareDateTimeFormatter(
        _In_ String ^ format, _In_ String ^ calendarIdentifier,
  ...
  return ref new DateTimeFormatter(
      format, languageIdentifiers,
      GlobalizationPreferences::HomeGeographicRegion, calendarIdentifier,
      clockIdentifier);
}
...
IIterable<String ^> ^ LocalizationService::GetLanguageIdentifiers() {
  ...
  if (result != 0) {
    // GetUserDefaultLocaleName may return an invalid bcp47 language tag with
    // trailing non-BCP47 friendly characters, which if present would start with
    // an underscore, for example sort order (see
    // https://msdn.microsoft.com/en-us/library/windows/desktop/dd373814(v=vs.85).aspx).
    // Therefore, if there is an underscore in the locale name, trim all
    // characters from the underscore onwards.
...
LocalizationService::GetTokenToReadableNameMap() {
  // Resources for the engine use numbers as keys. It's inconvenient, but also
  // difficult to change given that the engine heavily relies on perfect
  // ordering of certain elements. To compromise, we'll declare a map from
  // engine resource key to automation name from the standard project resources.
  ...
  static const wstring openParen =
      resProvider.GetCEngineString(StringReference(s_openParenResourceKey))
          ->Data();

  for (const auto &keyPair : s_parenEngineKeyResourceMap) {
    wstring engineStr =
        resProvider.GetCEngineString(StringReference(keyPair.first.c_str()))
            ->Data();
    wstring automationName =
        resProvider.GetResourceString(StringReference(keyPair.second.c_str()))
            ->Data();
...
String ^ LocalizationService::GetNarratorReadableToken(String ^ rawToken) {
  static unordered_map<wstring, wstring> s_tokenToReadableNameMap =
      GetTokenToReadableNameMap();
  ...
  if (itr == s_tokenToReadableNameMap.end()) {
    ...
    static const String ^ openParen =
        AppResourceProvider::GetInstance().GetCEngineString(
            StringReference(s_openParenResourceKey));
...
String ^ LocalizationService::GetNarratorReadableString(String ^ rawString) {
  ...
  for (const auto &c : asWstring) {
    readableString
        << LocalizationService::GetNarratorReadableToken(L"" + c)->Data();
  }
