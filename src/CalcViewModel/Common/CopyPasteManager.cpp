...
// Support digit separators ` (WinDbg/MASM), ' (C++), and _ (C# and other
// languages)
static const wstring c_hexProgrammerChars =
    L"([a-f]|[A-F]|\\d)+((_|'|`)([a-f]|[A-F]|\\d)+)*";
...
static const array<wregex, 2> scientificModePatterns = {
    wregex(L"(" + c_wspc + L"[-+]?)|(" + c_wspcLParenSigned + L")" +
           c_signedDecFloat + c_wspcRParens),
    wregex(L"(" + c_wspc + L"[-+]?)|(" + c_wspcLParenSigned + L")" +
           c_signedDecFloat + L"[e]([+]|[-])+\\d+" + c_wspcRParens)};
static const array<array<wregex, 5>, 4> programmerModePatterns = {
    {// Hex numbers like 5F, 4A0C, 0xa9, 0xFFull, 47CDh
     {wregex(c_wspcLParens + L"(0[xX])?" + c_hexProgrammerChars +
             c_uIntSuffixes + c_wspcRParens),
      wregex(c_wspcLParens + c_hexProgrammerChars + L"[hH]?" + c_wspcRParens)},
     // Decimal numbers like -145, 145, 0n145, 123ull etc
     {wregex(c_wspcLParens + L"[-+]?" + c_decProgrammerChars + L"[lL]{0,2}" +
             c_wspcRParens),
      wregex(c_wspcLParens + L"(0[nN])?" + c_decProgrammerChars +
             c_uIntSuffixes + c_wspcRParens)},
     // Octal numbers like 06, 010, 0t77, 0o77, 077ull etc
     {wregex(c_wspcLParens + L"(0[otOT])?" + c_octProgrammerChars +
             c_uIntSuffixes + c_wspcRParens)},
     // Binary numbers like 011010110, 0010110, 10101001, 1001b, 0b1001, 0y1001,
     // 0b1001ull
     {wregex(c_wspcLParens + L"(0[byBY])?" + c_binProgrammerChars +
             c_uIntSuffixes + c_wspcRParens),
      wregex(c_wspcLParens + c_binProgrammerChars + L"[bB]?" +
             c_wspcRParens)}}};
...
task<String ^> CopyPasteManager::GetStringToPaste(ViewMode mode,
  ...
  //-- add support to avoid pasting of expressions like 12 34(as of now we allow
  // 1234)
  //-- add support to allow pasting for expressions like .2 , -.2
  //-- add support to allow pasting for expressions like 1.3e12(as of now we
  // allow 1.3e+12)

  return create_task(
             (dataPackageView->GetTextAsync(::StandardDataFormats::Text)))
      .then(
          [mode, modeType, programmerNumberBase,
           bitLengthType](String ^ pastedText) {
            return ValidatePasteExpression(pastedText, mode, modeType,
                                           programmerNumberBase, bitLengthType);
          },
...
String ^ CopyPasteManager::ValidatePasteExpression(String ^ pastedText,
                                                   ViewMode mode,
                                                   int programmerNumberBase,
                                                   int bitLengthType) {
  return CopyPasteManager::ValidatePasteExpression(
      pastedText, mode, NavCategory::GetGroupType(mode), programmerNumberBase,
      bitLengthType);
}
...
String ^ CopyPasteManager::ValidatePasteExpression(String ^ pastedText,
  ...
  if (pastedText->Length() > MaxPasteableLength) {
    ...
    TraceLogger::GetInstance().LogInvalidPastedInputOccurred(
        L"PastedExpressionSizeGreaterThanMaxAllowed", mode,
        programmerNumberBase, bitLengthType);
  ...
  // Removing the spaces, comma separator from the pasteExpression to allow
  // pasting of expressions like 1  +     2+1,333
  pasteExpression = RemoveUnwantedCharsFromWstring(englishString->Data());

  // If the last character is an = sign, remove it from the pasteExpression to
  // allow evaluating the result on paste.
  if (!pasteExpression.empty() && pasteExpression.back() == L'=') {
    pasteExpression = pasteExpression.substr(0, pasteExpression.length() - 1);
  }

  // Extract operands from the expression to make regex comparison easy and
  // quick. For whole expression it was taking too much of time. Operands vector
  // will have the list of operands in the pasteExpression
  vector<wstring> operands = ExtractOperands(
      pasteExpression, mode, programmerNumberBase, bitLengthType);
  if (operands.empty()) {
    // return NoOp to indicate don't paste anything.
    return StringReference(PasteErrorString);
  }
  ...
  if (!ExpressionRegExMatch(operands, mode, modeType, programmerNumberBase,
                            bitLengthType)) {
    TraceLogger::GetInstance().LogInvalidPastedInputOccurred(
        L"InvalidExpressionForPresentMode", mode, programmerNumberBase,
        bitLengthType);
    return StringReference(PasteErrorString);
  }
...
vector<wstring>
CopyPasteManager::ExtractOperands(const wstring &pasteExpression, ViewMode mode,
  ...
  for (size_t i = 0; i < pasteExpression.length(); i++) {
    ...
    if (operands.size() >= MaxOperandCount) {
      TraceLogger::GetInstance().LogInvalidPastedInputOccurred(
          L"OperandCountGreaterThanMaxCount", mode, programmerNumberBase,
          bitLengthType);
      operands.clear();
      return operands;
    }
    ...
    if (startExpCounting) {
      if ((pasteExpression.at(i) >= L'0') && (pasteExpression.at(i) <= L'9')) {
        ...
        // to disallow pasting of 1e+12345 as 1e+1234, max exponent that can be
        // pasted is 9999.
        if (expLength > MaxExponentLength) {
          TraceLogger::GetInstance().LogInvalidPastedInputOccurred(
              L"ExponentLengthGreaterThanMaxLength", mode, programmerNumberBase,
              bitLengthType);
          operands.clear();
          return operands;
        }
    ...
    if (((pasteExpression.at(i) == L'+') || (pasteExpression.at(i) == L'-') ||
         (pasteExpression.at(i) == L'*') || (pasteExpression.at(i) == L'/'))) {
      if ((pasteExpression.at(i) == L'+') || (pasteExpression.at(i) == L'-')) {
        // don't break the expression into operands if the encountered character
        // corresponds to sign command(+-)
        if (isPreviousOpenParen || startOfExpression || isPreviousOperator ||
            ((mode != ViewMode::Programmer) &&
             !((i != 0) && (pasteExpression.at(i - 1) != L'e')))) {
  ...
  if (!haveOperator) {
    ...
    operands.push_back(
        pasteExpression.substr(lastIndex, pasteExpression.length() - 1));
  }

  return operands;
}

bool CopyPasteManager::ExpressionRegExMatch(vector<wstring> operands,
                                            ViewMode mode,
                                            CategoryGroupType modeType,
                                            int programmerNumberBase,
                                            int bitLengthType) {
  ...
  pair<size_t, uint64_t> operandLimits = GetMaxOperandLengthAndValue(
      mode, modeType, programmerNumberBase, bitLengthType);
  ...
  if (mode == ViewMode::Standard) {
    ...
    patterns.assign(scientificModePatterns.begin(),
                    scientificModePatterns.end());
  } else if (mode == ViewMode::Programmer) {
    patterns.assign(
        programmerModePatterns[programmerNumberBase - HexBase].begin(),
        programmerModePatterns[programmerNumberBase - HexBase].end());
  ...
  for (const wstring &operand : operands) {
    ...
    if (operandMatched) {
      // Remove characters that are valid in the expression but we do not want
      // to include in length calculations or which will break conversion from
      // string-to-ULL.
      wstring operandValue = SanitizeOperand(operand);

      // If an operand exceeds the maximum length allowed, break and return.
      if (OperandLength(operandValue, mode, modeType, programmerNumberBase) >
          maxOperandLength) {
        expMatched = false;
        break;
      }

      // If maxOperandValue is set and the operandValue exceeds it, break and
      // return.
      if (maxOperandValue != 0) {
        unsigned long long int operandAsULL = 0;
        if (!TryOperandToULL(operandValue, programmerNumberBase,
                             operandAsULL)) {
          // Operand was empty, received invalid_argument, or received
          // out_of_range. Input is invalid.
          expMatched = false;
          break;
        }
...
pair<size_t, uint64_t> CopyPasteManager::GetMaxOperandLengthAndValue(
    ViewMode mode, CategoryGroupType modeType, int programmerNumberBase,
    int bitLengthType) {
  size_t maxLength = 0;
  uint64_t maxValue = 0;
...
wstring CopyPasteManager::SanitizeOperand(const wstring &operand) {
  ...
  return Utils::RemoveUnwantedCharsFromWstring(
      operand, unWantedChars, static_cast<int>(size(unWantedChars)));
}

bool CopyPasteManager::TryOperandToULL(const wstring &operand, int numberBase,
...
size_t CopyPasteManager::OperandLength(wstring operand, ViewMode mode,
                                       CategoryGroupType modeType,
                                       int programmerNumberBase) {
  size_t len = 0;
...
size_t CopyPasteManager::ProgrammerOperandLength(const wstring &operand,
                                                 int numberBase) {
  ...
  transform(operandUpper.begin(), operandUpper.end(), operandUpper.begin(),
            towupper);

  // Detect if there is a suffix and subtract its length
  // Check suffixes first to allow e.g. "0b" to result in length 1 (value 0),
  // rather than length 0 (no value).
  for (const wstring &suffix : suffixes) {
    if (len < suffix.length()) {
      continue;
    }

    if (operandUpper.compare(operandUpper.length() - suffix.length(),
...
// return wstring after removing characters like space, comma, double quotes,
// and monetary prefix currency symbols supported by the Windows keyboard: yen
// or yuan(¥) - 165 unspecified currency sign(¤) - 164 Ghanaian cedi(₵) - 8373
// dollar or peso($) - 36
// colón(₡) - 8353
// won(₩) - 8361
// shekel(₪) - 8362
...
wstring CopyPasteManager::RemoveUnwantedCharsFromWstring(const wstring &input) {
  wchar_t unWantedChars[] = {L' ', L',', L'"', 165,  164,  8373,
                             36,   8353, 8361, 8362, 8358, 8377,
                             163,  8364, 8234, 8235, 8236, 8237};
