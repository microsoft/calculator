...
static constexpr array<const wchar_t *const, 9> s_programmerType{
    L"N/A",     L"QwordType", L"DwordType", L"WordType", L"ByteType",
...
constexpr auto EVENT_NAME_PRELAUNCHED_APP_ACTIVATED_BY_USER =
    L"PrelaunchedAppActivatedByUser";
...
constexpr auto EVENT_NAME_PREVIOUS_STATE_WINDOW_ON_CREATION =
    L"PreviousStateOnWindowCreation";
constexpr auto EVENT_NAME_SIZE_ON_SUSPENSION = L"CalculatorSizeOnSuspension";
constexpr auto EVENT_NAME_CALCULATOR_VIEWED_IN_SESSION =
    L"CalculatorViewedInSession";
constexpr auto EVENT_NAME_DATE_CALCULATOR_VIEWED_IN_SESSION =
    L"DateCalculatorViewedInSession";
constexpr auto EVENT_NAME_CONVERTER_VIEWED_IN_SESSION =
    L"ConverterViewedInSession";
...
constexpr auto EVENT_NAME_INVALID_PASTED_INPUT_OCCURRED =
    L"InvalidPastedInputOccurred";
...
constexpr auto EVENT_NAME_WINDOW_LAUNCHED_PROTOCOL =
    L"WindowActivatedThroughProtocol";
constexpr auto EVENT_NAME_WINDOW_LAUNCHED_TILESEARCH =
    L"WindowLaunchedThroughTile";
...
// c.f. WINEVENT_KEYWORD_RESERVED_63-56 0xFF00000000000000 // Bits 63-56 -
// channel keywords c.f. WINEVENT_KEYWORD_*              0x00FF000000000000 //
// Bits 55-48 - system-reserved keywords
constexpr int64_t MICROSOFT_KEYWORD_CRITICAL_DATA =
    0x0000800000000000;                                             // Bit 47
...
constexpr int64_t MICROSOFT_KEYWORD_RESERVED_44 =
    0x0000100000000000; // Bit 44 (reserved for future assignment)
#else
...
TraceLogger::TraceLogger()
    : g_calculatorProvider(
          ...
          LoggingChannelOptions(GUID{0x4f50731a, 0x89cf, 0x4782, 0xb3, 0xe0,
                                     0xdc, 0xe8, 0xc9, 0x4, 0x76,
                                     0xba}), // Microsoft Telemetry group
          GUID{0x905ca09, 0x610e, 0x401e, 0xb6, 0x50, 0x2f, 0x21, 0x29, 0x80,
               0xb9, 0xe0}), // Unique providerID
                             // {0905CA09-610E-401E-B650-2F212980B9E0}
...
void TraceLogger::LogTelemetryEvent(wstring_view eventName,
                                    LoggingFields fields) const {
  g_calculatorProvider.LogEvent(eventName, fields, LoggingLevel::Verbose,
                                LoggingOptions(MICROSOFT_KEYWORD_TELEMETRY));
}

void TraceLogger::LogMeasureEvent(wstring_view eventName,
                                  LoggingFields fields) const {
  g_calculatorProvider.LogEvent(eventName, fields, LoggingLevel::Verbose,
                                LoggingOptions(MICROSOFT_KEYWORD_MEASURES));
}

void TraceLogger::LogCriticalDataEvent(wstring_view eventName,
                                       LoggingFields fields) const {
  g_calculatorProvider.LogEvent(
      eventName, fields, LoggingLevel::Verbose,
      LoggingOptions(MICROSOFT_KEYWORD_CRITICAL_DATA));
}

void TraceLogger::LogPerformanceEvent(wstring_view eventName,
                                      LoggingFields fields) const {
  g_calculatorProvider.LogEvent(eventName, fields, LoggingLevel::Verbose,
                                LoggingOptions(WINEVENT_KEYWORD_RESPONSE_TIME));
}

void TraceLogger::LogInfoEvent(wstring_view eventName,
                               LoggingFields fields) const {
  g_calculatorProvider.LogEvent(eventName, fields, LoggingLevel::Information);
}

unique_ptr<TraceActivity>
TraceLogger::CreateTraceActivity(wstring_view eventName,
                                 LoggingFields fields) const {
  return make_unique<TraceActivity>(g_calculatorProvider, eventName, fields);
}
#pragma endregion

void TraceLogger::InsertIntoMemoryMap(int windowId, bool isStandard,
                                      bool isScientific, bool isProgrammer) {
...
void TraceLogger::UpdateMemoryMap(int windowId, int memoryPosition,
                                  bool isStandard, bool isScientific,
                                  bool isProgrammer) {
  // Writer lock for the static resources
  reader_writer_lock::scoped_lock lock(s_traceLoggerLock);
...
void TraceLogger::LogCalculatorModeViewed(ViewMode mode, int windowId) {
  ...
  // store windowId in windowIdLog which says we have logged mode for the
  // present windowId.
  if (windowIdLog.find(windowId) == windowIdLog.end()) {
  ...
  if (currentMode != mode) {
    ...
    fields.AddString(L"CalculatorMode",
                     NavCategory::GetFriendlyName(mode)->Data());
...
void TraceLogger::LogDateCalculatorModeViewed(ViewMode mode, int windowId) {
  ...
  if (currentMode != mode) {
    ...
    LoggingFields fields{};
    fields.AddString(L"DateCalculatorMode",
                     NavCategory::GetFriendlyName(mode)->Data());
    fields.AddUInt32(L"WindowId", windowId);
...
void TraceLogger::LogConverterModeViewed(ViewMode mode, int windowId) {
  ...
  if (currentMode != mode) {
    ...
    LoggingFields fields{};
    fields.AddString(L"ConverterMode",
                     NavCategory::GetFriendlyName(mode)->Data());
    fields.AddUInt32(L"WindowId", windowId);
...
void TraceLogger::LogSharedMemoryUsed(wstring_view fromMode,
                                      wstring_view toMode,
...
void TraceLogger::LogAppLaunchStart() {
  ...
  if (!isAppLaunchBeginLogged) {
    m_appLaunchActivity = g_calculatorProvider.StartActivity(
        EVENT_NAME_APP_LAUNCH_BEGIN, nullptr, LoggingLevel::Verbose,
        LoggingOptions(MICROSOFT_KEYWORD_TELEMETRY));
...
void TraceLogger::LogHistoryFlyoutOpenBegin(
    unsigned int historyItemCount) const {
  if (!GetTraceLoggingProviderEnabled())
    return;
...
// If calculator is launched in any mode other than standard then this call will
// come which is not intended. But there is no way to avoid it. So don't use
// this function to analyze the count of mode change in session instead use
// CalculatorViewedInSession and ConverterViewedInSession to do that. Use of
// this function is to analyze perf of mode change.
void TraceLogger::LogModeChangeBegin(ViewMode fromMode, ViewMode toMode,
                                     int windowId) {
  if (!GetTraceLoggingProviderEnabled())
    return;

  if (NavCategory::IsValidViewMode(fromMode) &&
      NavCategory::IsValidViewMode(toMode)) {
    LoggingFields fields{};
    fields.AddString(L"FromMode",
                     NavCategory::GetFriendlyName(fromMode)->Data());
...
void TraceLogger::LogMemoryUsed(int windowId, unsigned int slotPosition,
                                bool isStandard, bool isScientific,
                                bool isProgrammer,
                                unsigned int memorySize) const {
  if (!GetTraceLoggingProviderEnabled())
  ...
  if (iterMap != s_memoryMap.end()) {
    // if current mode is not equal to mode of memoryMap[slotPosition] then
    // LogSharedMemoryUsed
    if (isStandard && (iterMap->second[slotPosition] != L"Standard")) {
      LogSharedMemoryUsed(iterMap->second[slotPosition], L"Standard",
                          memorySize);
    } else if (isScientific &&
               (iterMap->second[slotPosition] != L"Scientific")) {
      LogSharedMemoryUsed(iterMap->second[slotPosition], L"Scientific",
                          memorySize);
    } else if (isProgrammer &&
               (iterMap->second[slotPosition] != L"Programmer")) {
      LogSharedMemoryUsed(iterMap->second[slotPosition], L"Programmer",
                          memorySize);
    }
  }
}

void TraceLogger::LogMultipleMemoryUsed(unsigned int slotPosition,
                                        unsigned int memorySize) const {
  if (!GetTraceLoggingProviderEnabled())
    return;
...
void TraceLogger::LogInvalidPastedInputOccurred(wstring_view reason,
                                                ViewMode mode,
  ...
  fields.AddString(L"ProgrammerNumberBase",
                   GetProgrammerType(programmerNumberBase).c_str());
...
void TraceLogger::LogStandardException(wstring_view functionName,
...
void TraceLogger::LogWinRTException(wstring_view functionName,
                                    hresult_error const &e) const {
  if (!GetTraceLoggingProviderEnabled())
    return;
...
void TraceLogger::LogPlatformException(wstring_view functionName,
                                       Platform::Exception ^ e) const {
  if (!GetTraceLoggingProviderEnabled())
    return;
...
void TraceLogger::InitFunctionLogArray() {
  ...
  // update the functionCount with total function count which we are tracking
  // through tracelog.
  functionCount = i;
}
...
void TraceLogger::UpdateWindowCount(size_t windowCount) {
  maxWindowCount =
      (maxWindowCount > windowCount) ? maxWindowCount : windowCount;
  windowLaunchCount++;
}
...
void TraceLogger::LogBitLengthButtonUsed(int windowId) {
  ...
  if ((bitLengthButtonUsage[windowId] == 5) &&
      !bitLengthButtonLoggedInSession) {
    LoggingFields fields{};
    fields.AddUInt32(L"WindowId", windowId);
...
void TraceLogger::LogDateAddSubtractModeUsed(int windowId, bool isAddMode) {
  ...
  auto isLoggedInSession = isAddMode ? &m_dateAddUsageLoggedInSession
                                     : &m_dateSubtractUsageLoggedInSession;
  ...
  // Ignore first 3 calls during the initialization of the combo box selected
  // items for Add mode
...
void TraceLogger::LogDateClippedTimeDifferenceFound(
    Calendar const &today, DateTime const &clippedTime) const {
  if (!GetTraceLoggingProviderEnabled())
    return;
...
void TraceLogger::LogConversionResult(wstring_view fromValue,
                                      wstring_view fromUnit,
                                      wstring_view toValue,
                                      wstring_view toUnit) const {
  if (!GetTraceLoggingProviderEnabled())
    return;
