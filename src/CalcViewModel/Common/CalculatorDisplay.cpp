...
// This class provides the concrete implementation for the ICalcDisplay
// interface that is declared in the Calculation Manager Library.
...
void CalculatorDisplay::SetHistoryCallback(
    Platform::WeakReference callbackReference) {
  m_historyCallbackReference = callbackReference;
}

void CalculatorDisplay::SetPrimaryDisplay(
    _In_ const wstring &displayStringValue, _In_ bool isError) {
  if (m_callbackReference) {
    if (auto calcVM = m_callbackReference
                          .Resolve<ViewModel::StandardCalculatorViewModel>()) {
      calcVM->SetPrimaryDisplay(displayStringValue, isError);
    }
  }
}

void CalculatorDisplay::SetParenDisplayText(
    _In_ const std::wstring &parenthesisCount) {
...
void CalculatorDisplay::SetMemorizedNumbers(
    _In_ const vector<std::wstring> &newMemorizedNumbers) {
  if (m_callbackReference != nullptr) {
    if (auto calcVM = m_callbackReference
                          .Resolve<ViewModel::StandardCalculatorViewModel>()) {
...
void CalculatorDisplay::OnHistoryItemAdded(_In_ unsigned int addedItemIndex) {
  if (m_historyCallbackReference != nullptr) {
    if (auto historyVM =
            m_historyCallbackReference.Resolve<ViewModel::HistoryViewModel>()) {
