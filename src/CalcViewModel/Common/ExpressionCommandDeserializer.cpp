...
std::shared_ptr<IExpressionCommand>
CommandDeserializer::Deserialize(_In_ CalculationManager::CommandType cmdType) {
  switch (cmdType) {
  ...
  default:
    throw ref new Platform::Exception(
        E_INVALIDARG, ref new Platform::String(L"Unknown command type"));
  }
}
...
COpndCommand CommandDeserializer::DeserializeOperand() {
  ...
  std::shared_ptr<CalculatorVector<int>> cmdVector =
