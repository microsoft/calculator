...
void Utils::RunOnUIThreadNonblocking(std::function<void()> &&function,
                                     _In_ CoreDispatcher ^ currentDispatcher) {
  if (currentDispatcher != nullptr) {
    auto task = create_task(currentDispatcher->RunAsync(
        CoreDispatcherPriority::Normal,
        ref new DispatchedHandler([function]() { function(); })));
  }
}

// Returns if the last character of a wstring is the target wchar_t
bool Utils::IsLastCharacterTarget(_In_ wstring const &input,
                                  _In_ wchar_t target) {
...
wstring Utils::RemoveUnwantedCharsFromWstring(wstring input,
                                              wchar_t *unwantedChars,
                                              unsigned int size) {
  for (unsigned int i = 0; i < size; ++i) {
    input.erase(std::remove(input.begin(), input.end(), unwantedChars[i]),
                input.end());
  }
  return input;
}
...
void Utils::SerializeCommandsAndTokens(
    ...
    _In_ shared_ptr<CalculatorVector<shared_ptr<IExpressionCommand>>> const
        &commands,
    DataWriter ^ writer) {
...
const shared_ptr<CalculatorVector<shared_ptr<IExpressionCommand>>>
Utils::DeserializeCommands(DataReader ^ reader) {
  shared_ptr<CalculatorVector<shared_ptr<IExpressionCommand>>> commandVector =
  ...
  for (unsigned int i = 0; i < commandVectorSize; ++i) {
    ...
    CalculationManager::CommandType commandType =
        static_cast<CalculationManager::CommandType>(commandTypeInt);
    shared_ptr<IExpressionCommand> exprCmd =
        cmdDeserializer.Deserialize(commandType);
...
const shared_ptr<CalculatorVector<pair<wstring, int>>>
Utils::DeserializeTokens(DataReader ^ reader) {
...
task<void> Utils::WriteFileToFolder(IStorageFolder ^ folder, String ^ fileName,
                                    String ^ contents,
  ...
  StorageFile ^ file =
      co_await folder->CreateFileAsync(fileName, collisionOption);
  if (file == nullptr) {
    co_return;
  }
...
task<String ^> Utils::ReadFileFromFolder(IStorageFolder ^ folder,
                                         String ^ fileName) {
  if (folder == nullptr) {
    co_return nullptr;
  }
