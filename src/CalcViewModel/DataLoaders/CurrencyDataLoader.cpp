...
// (1 interval/100 nanosecond)(100 nanosecond/1e-7 s)(60 s/1 min)(60 min/1
// hr)(24 hr/1 day) = (interval/day)
...
static constexpr auto STATIC_DATA_FILENAME =
    L"CURRENCY_CONVERTER_STATIC_DATA.txt";
static constexpr array<wstring_view, 5> STATIC_DATA_PROPERTIES = {
    wstring_view{L"CountryCode", 11}, wstring_view{L"CountryName", 11},
    wstring_view{L"CurrencyCode", 12}, wstring_view{L"CurrencyName", 12},
    wstring_view{L"CurrencySymbol", 14}};

static constexpr auto ALL_RATIOS_DATA_FILENAME =
    L"CURRENCY_CONVERTER_ALL_RATIOS_DATA.txt";
...
static constexpr array<wstring_view, 2> ALL_RATIOS_DATA_PROPERTIES = {
    wstring_view{RATIO_KEY, 2}, wstring_view{CURRENCY_CODE_KEY, 2}};

static constexpr auto DEFAULT_FROM_TO_CURRENCY_FILE_URI =
    L"ms-appx:///DataLoaders/DefaultFromToCurrency.json";
...
CurrencyDataLoader::CurrencyDataLoader(
    _In_ unique_ptr<ICurrencyHttpClient> client)
    : m_client(move(client)), m_loadStatus(CurrencyLoadStatus::NotLoaded),
      m_responseLanguage(L"en-US"), m_ratioFormat(L""), m_timestampFormat(L""),
      m_networkManager(ref new NetworkManager()), m_meteredOverrideSet(false) {
  if (GlobalizationPreferences::Languages->Size > 0) {
    m_responseLanguage = GlobalizationPreferences::Languages->GetAt(0);
  }

  if (m_client != nullptr) {
    m_client->SetSourceCurrencyCode(
  ...
  m_ratioFormat = AppResourceProvider::GetInstance()
                      .GetResourceString(L"CurrencyFromToRatioFormat")
                      ->Data();
  m_timestampFormat = AppResourceProvider::GetInstance()
                          .GetResourceString(L"CurrencyTimestampFormat")
                          ->Data();
}
...
void CurrencyDataLoader::RegisterForNetworkBehaviorChanges() {
  ...
  m_networkBehaviorToken = m_networkManager->NetworkBehaviorChanged +=
      ref new NetworkBehaviorChangedHandler(
          [this](NetworkAccessBehavior newBehavior) {
...
void CurrencyDataLoader::OnNetworkBehaviorChanged(
    NetworkAccessBehavior newBehavior) {
  m_networkAccessBehavior = newBehavior;
  if (m_vmCallback != nullptr) {
    m_vmCallback->NetworkBehaviorChanged(
        static_cast<int>(m_networkAccessBehavior));
  }
}
...
vector<UCM::Unit>
CurrencyDataLoader::LoadOrderedUnits(const UCM::Category & /*category*/) {
  lock_guard<mutex> lock(m_currencyUnitsMutex);
  return m_currencyUnits;
}

unordered_map<UCM::Unit, UCM::ConversionData, UCM::UnitHash>
CurrencyDataLoader::LoadOrderedRatios(const UCM::Unit &unit) {
  lock_guard<mutex> lock(m_currencyUnitsMutex);
...
void CurrencyDataLoader::SetViewModelCallback(
    const shared_ptr<UCM::IViewModelCurrencyCallback> &callback) {
  m_vmCallback = callback;
  OnNetworkBehaviorChanged(m_networkAccessBehavior);
}

pair<wstring, wstring>
CurrencyDataLoader::GetCurrencySymbols(const UCM::Unit &unit1,
                                       const UCM::Unit &unit2) {
  lock_guard<mutex> lock(m_currencyUnitsMutex);
...
pair<wstring, wstring>
CurrencyDataLoader::GetCurrencyRatioEquality(_In_ const UCM::Unit &unit1,
                                             _In_ const UCM::Unit &unit2) {
  try {
    auto iter = m_currencyRatioMap.find(unit1);
    if (iter != m_currencyRatioMap.end()) {
      unordered_map<UCM::Unit, UCM::ConversionData, UCM::UnitHash> ratioMap =
          iter->second;
      ...
      if (iter2 != ratioMap.end()) {
        ...
        double rounded =
            static_cast<int>(ratio * static_cast<int>(scale)) / scale;

        wstring digitSymbol = wstring{
        ...
        wstring ratioString = LocalizationStringUtil::GetLocalizedString(
            m_ratioFormat.c_str(), digitSymbol.c_str(),
            unit1.abbreviation.c_str(), roundedFormat.c_str(),
            unit2.abbreviation.c_str());

        wstring accessibleRatioString =
            LocalizationStringUtil::GetLocalizedString(
                m_ratioFormat.c_str(), digitSymbol.c_str(),
                unit1.accessibleName.c_str(), roundedFormat.c_str(),
...
task<bool> CurrencyDataLoader::TryLoadDataFromCacheAsync() {
  try {
    ...
    if (localSettings == nullptr ||
        !localSettings->Values->HasKey(CacheTimestampKey)) {
      co_return false;
    }

    bool loadComplete = false;
    m_cacheTimestamp =
        static_cast<DateTime>(localSettings->Values->Lookup(CacheTimestampKey));
...
task<bool> CurrencyDataLoader::TryFinishLoadFromCacheAsync() {
  ...
  if (!localSettings->Values->HasKey(CacheLangcodeKey) ||
      !static_cast<String ^>(localSettings->Values->Lookup(CacheLangcodeKey))
           ->Equals(m_responseLanguage)) {
    co_return false;
  }
  ...
  String ^ staticDataResponse =
      co_await Utils::ReadFileFromFolder(localCacheFolder, StaticDataFilename);
  String ^ allRatiosResponse = co_await Utils::ReadFileFromFolder(
      localCacheFolder, AllRatiosDataFilename);
...
task<bool> CurrencyDataLoader::TryLoadDataFromWebAsync() {
  try {
    ...
    if (m_networkAccessBehavior == NetworkAccessBehavior::Offline ||
        (m_networkAccessBehavior == NetworkAccessBehavior::OptIn &&
         !m_meteredOverrideSet)) {
      co_return false;
    }
    ...
    try {
      ...
      for (const auto &fileInfo : cachedFiles) {
        co_await Utils::WriteFileToFolder(
            localCacheFolder, fileInfo.first, fileInfo.second,
...
bool CurrencyDataLoader::TryParseWebResponses(
    _In_ String ^ staticDataJson, _In_ String ^ allRatiosJson,
...
bool CurrencyDataLoader::TryParseStaticData(
    _In_ String ^ rawJson,
    _Inout_ vector<UCM::CurrencyStaticData> &staticData) {
  ...
  vector<wstring *> values = {&countryCode, &countryName, &currencyCode,
  ...
  for (unsigned int i = 0; i < data->Size; i++) {
    ...
    for (size_t j = 0; j < values.size(); j++) {
      (*values[j]) =
          obj->GetNamedString(StringReference(STATIC_DATA_PROPERTIES[j].data()))
              ->Data();
    }

    staticData[i] = CurrencyStaticData{countryCode, countryName, currencyCode,
                                       currencyName, currencySymbol};
  }

  // TODO - MSFT 8533667: this sort will be replaced by a WinRT call to sort
  // localized strings
  sort(begin(staticData), end(staticData),
       [](CurrencyStaticData unit1, CurrencyStaticData unit2) {
...
bool CurrencyDataLoader::TryParseAllRatiosData(
    _In_ String ^ rawJson, _Inout_ CurrencyRatioMap &allRatios) {
  JsonArray ^ data = nullptr;
  if (!JsonArray::TryParse(rawJson, &data)) {
    return false;
  }
  ...
  for (unsigned int i = 0; i < data->Size; i++) {
    ...
    wstring targetCurrencyCode =
        obj->GetNamedString(StringReference(CURRENCY_CODE_KEY))->Data();

    allRatios.emplace(
        targetCurrencyCode,
        CurrencyRatio{relativeRatio, sourceCurrencyCode, targetCurrencyCode});
  }
...
// FinalizeUnits
//
// There are a few ways we can get the data needed for Currency Converter,
// including from cache or from web. This function accepts the data from any
// source, and acts as a 'last-steps' for the converter to be ready. This
// includes identifying which units will be selected and building the map of
// currency ratios.
#pragma optimize("", off) // Turn off optimizations to work around DevDiv 393321
task<void> CurrencyDataLoader::FinalizeUnits(
    _In_ const vector<UCM::CurrencyStaticData> &staticData,
    _In_ const CurrencyRatioMap &ratioMap) {
...

  {
    ...
    for (const UCM::CurrencyStaticData &currencyUnit : staticData) {
      ...
      if (itr != ratioMap.end() && (itr->second).ratio > 0) {
        ...
        m_currencyMetadata.emplace(
            unit, CurrencyUnitMetadata{currencyUnit.currencySymbol});
        idToUnit.insert(pair<int, pair<UCM::Unit, double>>(
            unit.id, pair<UCM::Unit, double>(unit, (itr->second).ratio)));
        i++;
      }
    }
...
void CurrencyDataLoader::UpdateDisplayedTimestamp() {
  if (m_vmCallback != nullptr) {
    ...
    bool isWeekOld =
        Utils::IsDateTimeOlderThan(m_cacheTimestamp, WEEK_DURATION);
...
wstring CurrencyDataLoader::GetCurrencyTimestamp() {
  ...
  if (m_cacheTimestamp.UniversalTime != epoch.UniversalTime) {
    DateTimeFormatter ^ dateFormatter = ref new DateTimeFormatter(
        L"{month.abbreviated} {day.integer}, {year.full}");
    ...
    timestamp = LocalizationStringUtil::GetLocalizedString(
        m_timestampFormat.c_str(), date.c_str(), time.c_str());
  }
...
task<SelectedUnits> CurrencyDataLoader::GetDefaultFromToCurrency() {
  ...
  bool foundInLocalSettings =
      TryGetLastUsedCurrenciesFromLocalSettings(&fromCurrency, &toCurrency);
  if (!foundInLocalSettings) {
    try {
      // Second, see if the current locale has preset defaults in
      // DefaultFromToCurrency.json.
      Uri ^ fileUri =
          ref new Uri(StringReference(DEFAULT_FROM_TO_CURRENCY_FILE_URI));
      StorageFile ^ defaultFromToCurrencyFile =
          co_await StorageFile::GetFileFromApplicationUriAsync(fileUri);
      if (defaultFromToCurrencyFile != nullptr) {
        String ^ fileContents =
            co_await FileIO::ReadTextAsync(defaultFromToCurrencyFile);
        JsonObject ^ fromToObject = JsonObject::Parse(fileContents);
        JsonObject ^ regionalDefaults =
            fromToObject->GetNamedObject(m_responseLanguage);

        // Get both values before assignment in-case either fails.
        String ^ selectedFrom =
            regionalDefaults->GetNamedString(StringReference(FROM_KEY));
        String ^ selectedTo =
            regionalDefaults->GetNamedString(StringReference(TO_KEY));
...
bool CurrencyDataLoader::TryGetLastUsedCurrenciesFromLocalSettings(
...
void CurrencyDataLoader::SaveSelectedUnitsToLocalSettings(
    _In_ const SelectedUnits &selectedUnits) {
  String ^ fromKey = UnitConverterResourceKeys::CurrencyUnitFromKey;
  String ^ toKey = UnitConverterResourceKeys::CurrencyUnitToKey;
  ApplicationDataContainer ^ localSettings =
      ApplicationData::Current->LocalSettings;
