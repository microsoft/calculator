...
HistoryItemViewModel::HistoryItemViewModel(
    ...
    _In_ const shared_ptr<CalculatorVector<shared_ptr<IExpressionCommand>>>
        &spCommands)
    : m_expression(expression), m_result(result), m_spTokens(spTokens),
      m_spCommands(spCommands) {
  // updating accessibility names for expression and result
  m_accExpression = HistoryItemViewModel::GetAccessibleExpressionFromTokens(
      spTokens, m_expression);
...
String ^
  ...
  if (SUCCEEDED(hr)) {
    ...
    for (unsigned int i = 0; i < nTokens; i++) {
      ...
      accExpression << LocalizationService::GetNarratorReadableToken(
                           StringReference(token.c_str()))
                           ->Data();
    }
  }
