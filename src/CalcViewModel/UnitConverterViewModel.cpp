...
StringReference
    CurrencySymbolVisibilityPropertyName(L"CurrencySymbolVisibility");
...
UnitConverterViewModel::UnitConverterViewModel(
    const shared_ptr<UCM::IUnitConverter> &model)
    : m_model(model), m_resettingTimer(false),
      m_value1cp(ConversionParameter::Source), m_Value1Active(true),
      m_Value2Active(false), m_Value1("0"), m_Value2("0"),
      m_valueToUnlocalized(L"0"), m_valueFromUnlocalized(L"0"),
      ...
      m_IsDropDownOpen(false), m_IsDropDownEnabled(true),
      m_IsCurrencyLoadingVisible(false), m_isCurrencyDataLoaded(false),
      m_lastAnnouncedFrom(L""), m_lastAnnouncedTo(L""),
      m_lastAnnouncedConversionResult(L""), m_isValue1Updating(false),
      m_isValue2Updating(false), m_Announcement(nullptr),
      m_Mode(ViewMode::None), m_CurrencySymbol1(L""), m_CurrencySymbol2(L""),
      m_IsCurrencyCurrentCategory(false), m_CurrencyRatioEquality(L""),
      m_CurrencyRatioEqualityAutomationName(L""), m_isInputBlocked(false),
      m_CurrencyDataLoadFailed(false) {
  m_model->SetViewModelCallback(make_shared<UnitConverterVMCallback>(this));
  m_model->SetViewModelCurrencyCallback(
      make_shared<ViewModelCurrencyCallback>(this));
  m_decimalFormatter =
  ...
  m_currencyFormatter =
      LocalizationService::GetRegionalSettingsAwareCurrencyFormatter();
  m_currencyFormatter->IsGrouped = true;
  m_currencyFormatter->Mode = CurrencyFormatterMode::UseCurrencyCode;
  m_currencyFormatter->ApplyRoundingForCurrency(
      RoundingAlgorithm::RoundHalfDown);
  m_currencyMaxFractionDigits = m_currencyFormatter->FractionDigits;

  auto resourceLoader = AppResourceProvider::GetInstance();
  m_localizedValueFromFormat = resourceLoader.GetResourceString(
      UnitConverterResourceKeys::ValueFromFormat);
  m_localizedValueToFormat = resourceLoader.GetResourceString(
      UnitConverterResourceKeys::ValueToFormat);
  m_localizedConversionResultFormat = resourceLoader.GetResourceString(
      UnitConverterResourceKeys::ConversionResultFormat);
  m_localizedValueFromDecimalFormat = resourceLoader.GetResourceString(
      UnitConverterResourceKeys::ValueFromDecimalFormat);
  m_localizedInputUnitName = resourceLoader.GetResourceString(
      UnitConverterResourceKeys::InputUnit_Name);
  m_localizedOutputUnitName = resourceLoader.GetResourceString(
      UnitConverterResourceKeys::OutputUnit_Name);
...
void UnitConverterViewModel::PopulateData() { InitializeView(); }
...
void UnitConverterViewModel::OnCategoryChanged(Object ^ parameter) {
  ...
  IsCurrencyCurrentCategory =
      currentCategory.id == NavCategory::Serialize(ViewMode::Currency);
  ...
  IsCurrencyLoadingVisible =
      m_IsCurrencyCurrentCategory && !m_isCurrencyDataLoaded;
...
void UnitConverterViewModel::SetSelectedUnits() {
  UCM::CategorySelectionInitializer categoryInitializer =
      m_model->SetCurrentCategory(CurrentCategory->GetModelCategory());
...
void UnitConverterViewModel::BuildUnitList(
    const vector<UCM::Unit> &modelUnitList) {
  m_Units->Clear();
...
void UnitConverterViewModel::OnUnitChanged(Object ^ parameter) {
  ...
  m_model->SetCurrentUnitTypes(UnitFrom->GetModelUnit(),
                               UnitTo->GetModelUnit());
...
void UnitConverterViewModel::OnSwitchActive(Platform::Object ^ unused) {
  // this can be false if this switch occurs without the user having explicitly
  // updated any strings (for example, during deserialization). We only want to
  // try this cleanup if there's actually something to clean up.
...
String ^ UnitConverterViewModel::ConvertToLocalizedString(
  ...
  if (posOfE != wstring::npos) {
    ...
    std::wstring exponentStr(stringToLocalize.substr(
        posOfSign + 1, stringToLocalize.length() - posOfSign));

    result += ConvertToLocalizedString(significandStr, allowPartialStrings) +
              "e" + signOfE +
              ConvertToLocalizedString(exponentStr, allowPartialStrings);
  } else {
    // stringToLocalize is in en-US and has the default decimal separator, so
    // this is safe to do.
    ...
    if (hasDecimal) {
      if (allowPartialStrings) {
        // allow "in progress" strings, like "3." that occur during the
        // composition of a final number. Without this, when typing the three
        // characters in "3.2" you don't see the decimal point when typing it,
        // you only see it once you've finally typed a post-decimal digit.

        m_decimalFormatter->IsDecimalPointAlwaysDisplayed = true;
        m_currencyFormatter->IsDecimalPointAlwaysDisplayed = true;
      }

      // force post-decimal digits so that trailing zeroes entered by the user
      // aren't suddenly cut off.
      m_decimalFormatter->FractionDigits =
          static_cast<int>(stringToLocalize.length() - (posOfDecimal + 1));
      m_currencyFormatter->FractionDigits = m_currencyMaxFractionDigits;
    }

    if (IsCurrencyCurrentCategory) {
      wstring currencyResult =
          m_currencyFormatter->Format(stod(stringToLocalize))->Data();
      wstring currencyCode = m_currencyFormatter->Currency->Data();

      // CurrencyFormatter always includes LangCode or Symbol. Make it include
      // LangCode because this includes a non-breaking space. Remove the
      // LangCode.
    ...
    if (hasDecimal) {
      // Since the output from GetLocaleInfoEx() and DecimalFormatter are
      // differing for decimal string we are adding the below work-around of
      // editing the string returned by DecimalFormatter and replacing the
      // decimal separator with the one returned by GetLocaleInfoEx()
  ...
  if ((stringToLocalize.front() == L'-' && stod(stringToLocalize) == 0) ||
      resultHolder.back() == L'-') {
    if (resultHolder.back() == L'-') {
      result = ref new String(
          resultHolder.erase(resultHolder.size() - 1, 1).c_str());
    }
    result = L"-" + result;
  }
...
void UnitConverterViewModel::DisplayPasteError() {
  String ^ errorMsg = AppResourceProvider::GetInstance().GetCEngineString(
      SIDS_DOMAIN); /*SIDS_DOMAIN is for "invalid input"*/
  Value1 = errorMsg;
  Value2 = errorMsg;
  m_relocalizeStringOnSwitch = false;
}

void UnitConverterViewModel::UpdateDisplay(const wstring &from,
                                           const wstring &to) {
  ...
  if (updatedValueFrom) {
    ...
    // once we've updated the unlocalized from string, we'll potentially need to
    // clean it back up when switching between fields to eliminate dangling
    // decimal points.
...
void UnitConverterViewModel::UpdateSupplementaryResults(
    const std::vector<std::tuple<std::wstring, UnitConversionManager::Unit>>
        &suggestedValues) {
  m_cacheMutex.lock();
  ...
  m_supplementaryResultsTimer = ThreadPoolTimer::CreateTimer(
      ref new TimerElapsedHandler(
          this, &UnitConverterViewModel::SupplementaryResultsTimerTick,
          TIMER_CALLBACK_CONTEXT),
      SUPPLEMENTARY_VALUES_INTERVAL,
      ref new TimerDestroyedHandler(
          this, &UnitConverterViewModel::SupplementaryResultsTimerCancel,
          TIMER_CALLBACK_CONTEXT));
}
...
void UnitConverterViewModel::OnButtonPressed(Platform::Object ^ parameter) {
  ...
  static const vector<UCM::Command> OPERANDS = {
      UCM::Command::Zero,  UCM::Command::One,   UCM::Command::Two,
      UCM::Command::Three, UCM::Command::Four,  UCM::Command::Five,
      UCM::Command::Six,   UCM::Command::Seven, UCM::Command::Eight,
      UCM::Command::Nine};
...
void UnitConverterViewModel::OnCopyCommand(Platform::Object ^ parameter) {
  ...
  CopyPasteManager::CopyToClipboard(
      ref new Platform::String(m_valueFromUnlocalized.c_str()));
...
void UnitConverterViewModel::OnPasteCommand(Platform::Object ^ parameter) {
  ...
  CopyPasteManager::GetStringToPaste(m_Mode, NavCategory::GetGroupType(m_Mode))
      .then([this](String ^ pastedString) { OnPaste(pastedString, m_Mode); },
...
void UnitConverterViewModel::OnPropertyChanged(Platform::String ^ prop) {
  ...
  if (prop == CurrentCategoryPropertyName) {
    ...
    // Category changes will handle updating units after they've both been
    // updated. This event should only be used to update units from explicit
    // user interaction.
    ...
    UpdateValue2AutomationName();
  } else if (prop == Value1ActivePropertyName ||
             prop == Value2ActivePropertyName) {
    // if one of the values is activated, and as a result both are true, it
    // means that we're trying to switch.
  ...
  } else if (prop == CurrencySymbol1PropertyName ||
             prop == CurrencySymbol2PropertyName) {
...
String ^ UnitConverterViewModel::Serialize() {
  ...
  out << m_valueFromUnlocalized << delimiter << m_valueToUnlocalized
      << delimiter << L"[###]";
...
void UnitConverterViewModel::Deserialize(Platform::String ^ state) {
  ...
  vector<wstring> tokens =
      UCM::UnitConverter::StringToVector(serializedData, L"[###]");
  assert(tokens.size() >= 2);
  vector<wstring> viewModelData =
      UCM::UnitConverter::StringToVector(tokens[0], L"[;;;]");
...
void UnitConverterViewModel::SaveUserPreferences() {
  if (UnitsAreValid()) {
    ...
    if (!m_IsCurrencyCurrentCategory) {
      ...
      localSettings->Values->Insert(ref new String(L"UnitConverterPreferences"),
                                    ref new String(userPreferences.c_str()));
    } else {
      // Currency preferences shouldn't be saved in the same way as standard
      // converter modes because the delay loading creates a big mess of issues
      // that are better to avoid.
      localSettings->Values->Insert(
          UnitConverterResourceKeys::CurrencyUnitFromKey,
          UnitFrom->Abbreviation);
      localSettings->Values->Insert(
          UnitConverterResourceKeys::CurrencyUnitToKey, UnitTo->Abbreviation);
    }
  }
}
...
void UnitConverterViewModel::RestoreUserPreferences() {
  if (!IsCurrencyCurrentCategory) {
    ApplicationDataContainer ^ localSettings =
        ApplicationData::Current->LocalSettings;
    if (localSettings->Values->HasKey(
            ref new String(L"UnitConverterPreferences"))) {
      String ^ userPreferences =
          safe_cast<String ^>(localSettings->Values->Lookup(
              ref new String(L"UnitConverterPreferences")));
...
void UnitConverterViewModel::OnCurrencyDataLoadFinished(bool didLoad) {
  ...
  StringReference key =
      didLoad ? UnitConverterResourceKeys::CurrencyRatesUpdated
              : UnitConverterResourceKeys::CurrencyRatesUpdateFailed;
  String ^ announcement =
      AppResourceProvider::GetInstance().GetResourceString(key);
  Announcement =
      CalculatorAnnouncement::GetUpdateCurrencyRatesAnnouncement(announcement);
}

void UnitConverterViewModel::OnCurrencyTimestampUpdated(
    _In_ const wstring &timestamp, bool isWeekOld) {
...
void UnitConverterViewModel::OnNetworkBehaviorChanged(
    _In_ NetworkAccessBehavior newBehavior) {
  CurrencyDataLoadFailed = false;
  NetworkBehavior = newBehavior;
}

UnitConversionManager::Command
UnitConverterViewModel::CommandFromButtonId(NumbersAndOperatorsEnum button) {
  UCM::Command command;
...
void UnitConverterViewModel::SupplementaryResultsTimerTick(ThreadPoolTimer ^
                                                           timer) {
  timer->Cancel();
}

void UnitConverterViewModel::SupplementaryResultsTimerCancel(ThreadPoolTimer ^
                                                             timer) {
  if (!m_resettingTimer) {
...
// When UpdateDisplay is called, the ViewModel will remember the From/To
// unlocalized display values This function will announce the conversion result
// after the ValueTo/ValueFrom automation names update, only if the new
// unlocalized display values are different from the last announced values, and
// if the values are not both zero.
void UnitConverterViewModel::AnnounceConversionResult() {
  if ((m_valueFromUnlocalized != m_lastAnnouncedFrom ||
    ...
    m_lastAnnouncedConversionResult = GetLocalizedConversionResultStringFormat(
        ValueFrom, unitFrom->Name, ValueTo, unitTo->Name);

    Announcement = CalculatorAnnouncement::GetDisplayUpdatedAnnouncement(
        m_lastAnnouncedConversionResult);
  }
}

void UnitConverterViewModel::UpdateInputBlocked(
    _In_ const wstring &currencyInput) {
  // currencyInput is in en-US and has the default decimal separator, so this is
  // safe to do.
  auto posOfDecimal = currencyInput.find(L'.');
  m_isInputBlocked = false;
  if (posOfDecimal != wstring::npos && IsCurrencyCurrentCategory) {
    m_isInputBlocked =
        (posOfDecimal + static_cast<size_t>(m_currencyMaxFractionDigits) + 1 ==
         currencyInput.length());
  }
}

NumbersAndOperatorsEnum
UnitConverterViewModel::MapCharacterToButtonId(const wchar_t ch,
                                               bool &canSendNegate) {
  static_assert(NumbersAndOperatorsEnum::Zero < NumbersAndOperatorsEnum::One,
                "NumbersAndOperatorsEnum order is invalid");
  static_assert(NumbersAndOperatorsEnum::One < NumbersAndOperatorsEnum::Two,
                "NumbersAndOperatorsEnum order is invalid");
  static_assert(NumbersAndOperatorsEnum::Two < NumbersAndOperatorsEnum::Three,
                "NumbersAndOperatorsEnum order is invalid");
  static_assert(NumbersAndOperatorsEnum::Three < NumbersAndOperatorsEnum::Four,
                "NumbersAndOperatorsEnum order is invalid");
  static_assert(NumbersAndOperatorsEnum::Four < NumbersAndOperatorsEnum::Five,
                "NumbersAndOperatorsEnum order is invalid");
  static_assert(NumbersAndOperatorsEnum::Five < NumbersAndOperatorsEnum::Six,
                "NumbersAndOperatorsEnum order is invalid");
  static_assert(NumbersAndOperatorsEnum::Six < NumbersAndOperatorsEnum::Seven,
                "NumbersAndOperatorsEnum order is invalid");
  static_assert(NumbersAndOperatorsEnum::Seven < NumbersAndOperatorsEnum::Eight,
                "NumbersAndOperatorsEnum order is invalid");
  static_assert(NumbersAndOperatorsEnum::Eight < NumbersAndOperatorsEnum::Nine,
                "NumbersAndOperatorsEnum order is invalid");
  static_assert(NumbersAndOperatorsEnum::Zero < NumbersAndOperatorsEnum::Nine,
                "NumbersAndOperatorsEnum order is invalid");
...
void UnitConverterViewModel::OnPaste(String ^ stringToPaste, ViewMode mode) {
  ...
  for (auto it = stringToPaste->Begin(); it != stringToPaste->End(); it++) {
    ...
    if (NumbersAndOperatorsEnum::None != op) {
      ...
      // Negate is only allowed if it's the first legal character, which is
      // handled above.
      if (NumbersAndOperatorsEnum::Negate != op) {
...
String ^
    UnitConverterViewModel::GetLocalizedAutomationName(_In_ String ^
                                                           displayvalue,
                                                       _In_ String ^ unitname,
                                                       _In_ String ^ format) {
  String ^ valueToLocalize = displayvalue;
  if (displayvalue == ValueFrom &&
      Utils::IsLastCharacterTarget(m_valueFromUnlocalized,
                                   m_decimalSeparator)) {
    // Need to compute a second localized value for the automation
    // name that does not include the decimal separator.
    displayvalue = ConvertToLocalizedString(m_valueFromUnlocalized,
                                            false /*allowTrailingDecimal*/);
    format = m_localizedValueFromDecimalFormat;
  }

  wstring localizedResult = LocalizationStringUtil::GetLocalizedString(
      format->Data(), displayvalue->Data(), unitname->Data());
  return ref new String(localizedResult.c_str());
}

String ^ UnitConverterViewModel::GetLocalizedConversionResultStringFormat(
             _In_ String ^ fromValue, _In_ String ^ fromUnit,
             _In_ String ^ toValue, _In_ String ^ toUnit) {
  String ^ localizedString = ref new String(
      LocalizationStringUtil::GetLocalizedString(
          m_localizedConversionResultFormat->Data(), fromValue->Data(),
...
void UnitConverterViewModel::UpdateValue1AutomationName() {
  if (Unit1) {
    Value1AutomationName = GetLocalizedAutomationName(
        Value1, Unit1->AccessibleName, m_localizedValueFromFormat);
  }
}
...
void UnitConverterViewModel::UpdateValue2AutomationName() {
  if (Unit2) {
    Value2AutomationName = GetLocalizedAutomationName(
        Value2, Unit2->AccessibleName, m_localizedValueToFormat);
  }
}

void UnitConverterViewModel::OnMaxDigitsReached() {
  String ^ format = AppResourceProvider::GetInstance().GetResourceString(
      UnitConverterResourceKeys::MaxDigitsReachedFormat);
  const wstring &announcement = LocalizationStringUtil::GetLocalizedString(
      format->Data(), m_lastAnnouncedConversionResult->Data());
  Announcement = CalculatorAnnouncement::GetMaxDigitsReachedAnnouncement(
      StringReference(announcement.c_str()));
}

bool UnitConverterViewModel::UnitsAreValid() {
  return UnitFrom != nullptr && !UnitFrom->Abbreviation->IsEmpty() &&
         UnitTo != nullptr && !UnitTo->Abbreviation->IsEmpty();
}
      ...
      CONVERSION_FINALIZED_DELAY_IN_MS, [this]() {
        if (UnitsAreValid()) {
          ...
          TraceLogger::GetInstance().LogConversionResult(
              valueFrom->Data(), UnitFrom->ToString()->Data(), valueTo->Data(),
...
String ^ SupplementaryResult::GetLocalizedAutomationName() {
  auto format = AppResourceProvider::GetInstance().GetResourceString(
      "SupplementaryUnit_AutomationName");
  return ref new String(
      LocalizationStringUtil::GetLocalizedString(
          format->Data(), this->Value->Data(), this->Unit->Name->Data())
          .c_str());
}
