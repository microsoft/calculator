...
StringReference StrDateDiffResultAutomationNamePropertyName(
    L"StrDateDiffResultAutomationName");
...
StringReference
    StrDateResultAutomationNamePropertyName(L"StrDateResultAutomationName");
StringReference IsDiffInDaysPropertyName(L"IsDiffInDays");
} // namespace

DateCalculatorViewModel::DateCalculatorViewModel()
    : m_IsDateDiffMode(true), m_IsAddMode(true), m_isOutOfBound(false),
      m_DaysOffset(0), m_MonthsOffset(0), m_YearsOffset(0),
      m_StrDateDiffResult(L""), m_StrDateDiffResultAutomationName(L""),
      m_StrDateDiffResultInDays(L""), m_StrDateResult(L""),
  ...
  m_dateCalcEngine = make_shared<DateCalculationEngine>(
      localizationSettings.GetCalendarIdentifier());
  // Initialize dates of DatePicker controls to today's date
  auto calendar = ref new Calendar();
  // We force the timezone to UTC, in order to avoid being affected by Daylight
  // Saving Time when we calculate the difference between 2 dates.
  calendar->ChangeTimeZone("UTC");
  auto today = calendar->GetDateTime();

  // FromDate and ToDate should be clipped (adjusted to a consistent hour in
  // UTC)
  ...
  // Initialize the list separator delimiter appended with a space at the end,
  // e.g. ", " This will be used for date difference formatting: Y years, M
  ...
  if (calendar->DayOfWeek != trueDayOfWeek) {
    ...
    TraceLogger::GetInstance().LogDateClippedTimeDifferenceFound(
        ...
        winrt::Windows::Foundation::DateTime{
            winrt::Windows::Foundation::TimeSpan{clippedTime.UniversalTime}});
  }
}
...
void DateCalculatorViewModel::OnInputsChanged() {
  ...
  if (m_IsDateDiffMode) {
    ...
    m_dateCalcEngine->GetDateDifference(clippedFromDate, clippedToDate,
                                        m_allDateUnitsOutputFormat, &dateDiff);
    DateDiffResult = dateDiff;

    m_dateCalcEngine->GetDateDifference(clippedFromDate, clippedToDate,
                                        m_daysOutputFormat, &dateDiff);
    ...
    if (m_IsAddMode) {
      ...
      IsOutOfBound =
          !m_dateCalcEngine->AddDuration(StartDate, dateDiff, &dateTimeResult);
    } else {
      // Subtract number of Days, Months and Years from a Date
      IsOutOfBound = !m_dateCalcEngine->SubtractDuration(StartDate, dateDiff,
                                                         &dateTimeResult);
    }
...
void DateCalculatorViewModel::UpdateDisplayResult() {
  if (m_IsDateDiffMode) {
    ...
    if (m_dateDiffResultInDays.day == 0) {
      ...
      StrDateDiffResult = AppResourceProvider::GetInstance().GetResourceString(
          L"Date_SameDates");
    } else if ((m_dateDiffResult.year == 0) && (m_dateDiffResult.month == 0) &&
    ...
    if (m_isOutOfBound) {
      ...
      StrDateResult = AppResourceProvider::GetInstance().GetResourceString(
          L"Date_OutOfBoundMessage");
    } else {
...
void DateCalculatorViewModel::UpdateStrDateDiffResultAutomationName() {
  String ^ automationFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          L"Date_DifferenceResultAutomationName");
  wstring localizedAutomationName = LocalizationStringUtil::GetLocalizedString(
      automationFormat->Data(), StrDateDiffResult->Data());
...
void DateCalculatorViewModel::UpdateStrDateResultAutomationName() {
  String ^ automationFormat =
      AppResourceProvider::GetInstance().GetResourceString(
          L"Date_ResultingDateAutomationName");
  wstring localizedAutomationName = LocalizationStringUtil::GetLocalizedString(
      automationFormat->Data(), StrDateResult->Data());
  StrDateResultAutomationName = ref new String(localizedAutomationName.c_str());
}

void DateCalculatorViewModel::InitializeDateOutputFormats(_In_ String ^
                                                          calendarIdentifier) {
...
String ^ DateCalculatorViewModel::GetDateDiffStringInDays() const {
  ...
  if (m_dateDiffResultInDays.day > 1) {
    result += AppResourceProvider::GetInstance()
                  .GetResourceString(L"Date_Days")
                  ->Data();
  } else {
    result += AppResourceProvider::GetInstance()
                  .GetResourceString(L"Date_Day")
                  ->Data();
  }
