  ...
  for (iadigit = a->cdigit; iadigit > 0; iadigit--) {
    ...
    for (ibdigit = b->cdigit; ibdigit > 0; ibdigit--) {
      ...
      while (mcy || cy) {
        ...
        cy +=
            (TWO_MANTTYPE)ptrc[icdigit] + ((uint32_t)mcy & ((uint32_t)~BASEX));
...

{
  ...
  int32_t thismax = precision + g_ratio; // set a maximum number of internal
                                         // digits to shoot for in the divide.
