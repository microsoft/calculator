...

{
  ...
  if (rat_gt(*px, rat_negsmallest, precision) &&
...
//
// return if the given x is even number. The assumption here is its denominator
// is 1 and we are testing the numerator is even or not
...
void powratNumeratorDenominator(PRAT *px, PRAT y, uint32_t radix,
                                int32_t precision) {
  // Prepare rationals
  ...
  // 3. Validate the result of 2 by adding/subtracting 0.5, flooring and call
  // powratcomp with yDenom
    ...
    if (rat_gt(pxint, rat_negsmallest, precision) &&
      ...
      if (rat_gt(podd, rat_negsmallest, precision) &&
        ...
        if (sign == -1) {
          ...
          // As a first step, the numerator and denominator must be divided by 2
          // as many times as
          //     possible, so that 2/6 is allowed.
          // If the final numerator is still even, the end result should be
          // positive.
          ...
          while (IsEven(pNumerator, radix, precision) &&
                 IsEven(pDenominator, radix,
                        precision)) // both Numerator & denominator is even
          {
            divrat(&pNumerator, rat_two, precision);
            divrat(&pDenominator, rat_two, precision);
          }
          if (IsEven(pDenominator, radix,
                     precision)) // denominator is still even
          {
            fBadExponent = true;
          }
