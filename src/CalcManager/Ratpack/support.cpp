...
static int cbitsofprecision =
    RATIO_FOR_DECIMAL * DECIMAL * CALC_DECIMAL_DIGITS_DEFAULT;
...
void ChangeConstants(uint32_t radix, int32_t precision) {
  ...
  if (cbitsofprecision < (g_ratio * static_cast<int32_t>(radix) * precision)) {
    ...
    // 3248, is the max number for which calc is able to compute factorial,
    // after that it is unable to compute due to overflow. Hence restricted
    // factorial range as at most 3248.Beyond that calc will throw overflow
    // error immediately.
    INIT_AND_DUMP_RAW_RAT_IF_NULL(rat_max_fact, 3249);

    // -1000, is the min number for which calc is able to compute factorial,
    // after that it takes too long to compute.
...

{
  ...
  b->pp->sign *= -1;
  bool bret =
      (!zernum(rattmp->pp) && rattmp->pp->sign * rattmp->pq->sign == -1);
  destroyrat(rattmp);
  return (bret);
}
...

{
  if (!g_ftrueinfinite) {
    ...
    PNUMBER pp = (*px)->pp;
    PNUMBER pq = (*px)->pq;
    trim = g_ratio * (min((pp->cdigit + pp->exp), (pq->cdigit + pq->exp)) - 1) -
           precision;
    if (trim > g_ratio) {
      trim /= g_ratio;
      ...
      if (trim <= pp->exp) {
        ...
        memmove(pp->mant, &(pp->mant[trim - pp->exp]),
                sizeof(MANTTYPE) * (pp->cdigit - trim + pp->exp));
      ...
      if (trim <= pq->exp) {
        ...
        memmove(pq->mant, &(pq->mant[trim - pq->exp]),
                sizeof(MANTTYPE) * (pq->cdigit - trim + pq->exp));
