...
static constexpr wstring_view DIGITS =
...
#define CALC_INTSAFE_E_ARITHMETIC_OVERFLOW                                     \
  ((int32_t)0x80070216L) // 0x216 = 534 = ERROR_ARITHMETIC_OVERFLOW
#define CALC_ULONG_ERROR ((uint32_t)0xffffffffU)

namespace {
int32_t Calc_ULongAdd(_In_ uint32_t ulAugend, _In_ uint32_t ulAddend,
...
int32_t Calc_ULongLongToULong(_In_ uint64_t ullOperand,
                              _Out_ uint32_t *pulResult) {
  int32_t hr = CALC_INTSAFE_E_ARITHMETIC_OVERFLOW;
  *pulResult = CALC_ULONG_ERROR;
...
int32_t Calc_ULongMult(_In_ uint32_t ulMultiplicand, _In_ uint32_t ulMultiplier,
                       _Out_ uint32_t *pulResult) {
...
//
void *zmalloc(size_t a) { return calloc(a, sizeof(unsigned char)); }
...
void _dupnum(_In_ PNUMBER dest, _In_ const NUMBER *const src) {
  memcpy(dest, src,
         (int)(sizeof(NUMBER) + ((src)->cdigit) * (sizeof(MANTTYPE))));
}
...
PNUMBER numtonRadixx(_In_ PNUMBER a, uint32_t radix) {
  PNUMBER pnumret =
      i32tonum(0, BASEX); // pnumret is the number in internal form.
...
PRAT StringToRat(bool mantissaIsNegative, wstring_view mantissa,
                 bool exponentIsNegative, wstring_view exponent, uint32_t radix,
                 int32_t precision) {
...
#if defined(DEBUG)
char *statestr[] = {
    "START", "MANTS", "LZ",   "LZDP",  "LD",    "DZ",    "DD",  "DDP",
    "EXPB",  "EXPS",  "EXPD", "EXPBZ", "EXPSZ", "EXPDZ", "ERR",
};
#endif
...
wchar_t NormalizeCharDigit(wchar_t c, uint32_t radix) {
  ...
  if (size_t{radix} >= DIGITS.find(L'A') &&
      size_t{radix} <= DIGITS.find(L'Z')) {
    return towupper(c);
  }
...
PNUMBER StringToNumber(wstring_view numberString, uint32_t radix,
                       int32_t precision) {
  ...
  for (const auto &c : numberString) {
    // If the character is the decimal separator, use L'.' for the purposes of
    // the state machine.
...
//    form, where q is 1 and p is the uint32_t. Being unsigned cant take
//    negative numbers, but the full range of unsigned numbers
//
//-----------------------------------------------------------------------------
...
//
//    DESCRIPTION: Returns a number representation in the
//    base   requested of the uint32_t value passed in. Being unsigned number it
//    has no negative number and takes the full range of unsigned number
//
//-----------------------------------------------------------------------------
...
//    FUNCTION: rattoi32
//
//    ARGUMENTS: rational number in internal base, integer radix and int32_t
//    precision.
//
//    RETURN: int32_t
//
...
int32_t rattoi32(_In_ PRAT prat, uint32_t radix, int32_t precision) {
  if (rat_gt(prat, rat_max_i32, precision) ||
      rat_lt(prat, rat_min_i32, precision)) {
...
uint32_t rattoUi32(_In_ PRAT prat, uint32_t radix, int32_t precision) {
  ...
  uint32_t lret = numtoi32(
      pint->pp, BASEX); // This happens to work even if it is only signed
...
//    RETURN: Ui64
//
//    DESCRIPTION: returns the 64 bit (irrespective of which processor this is
//    running in) representation of the number input.  Assumes that the number
//    is in the internal base. Can throw exception if the number exceeds 2^64
//    Implementation by getting the HI & LO 32 bit words and concatenating them,
//    as the internal base chosen happens to be 2^32, this is easier.
//-----------------------------------------------------------------------------
...
uint64_t rattoUi64(_In_ PRAT prat, uint32_t radix, int32_t precision) {
  ...
  uint32_t lo = rattoUi32(
      pint, radix,
      precision); // wont throw exception because already hi-dword chopped off
...
int32_t numtoi32(_In_ PNUMBER pnum, uint32_t radix) {
  ...
  for (int32_t length = pnum->cdigit; length > 0 && length + expt > 0;
       length--) {
    lret *= radix;
    lret += *(pmant--);
  }
...
//
//-----------------------------------------------------------------------------
wstring NumberToString(_Inout_ PNUMBER &pnum, int format, uint32_t radix,
                       int32_t precision) {
  stripzeroesnum(pnum, precision + 2);
  int32_t length = pnum->cdigit;
  int32_t exponent =
      pnum->exp + length; // Actual number of digits to the left of decimal
  ...
  if (!zernum(pnum) &&
      (pnum->cdigit >= precision || (length - exponent > precision &&
    ...
    if (exponent > 0 || format == FMT_FLOAT) {
      ...
      round->exp =
          pnum->exp + pnum->cdigit - round->cdigit - precision - exponent;
  ...
  if (format == FMT_FLOAT) {
    // Figure out if the exponent will fill more space than the non-exponent
    // field.
...
//
//-----------------------------------------------------------------------------
wstring RatToString(_Inout_ PRAT &prat, int format, uint32_t radix,
                    int32_t precision) {
...
// Converts a PRAT to a PNUMBER and back to a PRAT, flattening/simplifying the
...
void numpowi32(_Inout_ PNUMBER *proot, int32_t power, uint32_t radix,
               int32_t precision) {
