...
static const double OPTIMALDECIMALALLOWED =
    pow(10, -1 * (OPTIMALDIGITSALLOWED - 1));
static const double MINIMUMDECIMALALLOWED =
    pow(10, -1 * (MAXIMUMDIGITSALLOWED - 1));
...
/// </summary>
/// <param name="dataLoader">An instance of the IConverterDataLoader interface
/// which we use to read in category/unit names and conversion data</param>
UnitConverter::UnitConverter(
    _In_ const shared_ptr<IConverterDataLoader> &dataLoader)
...
/// <param name="dataLoader">An instance of the IConverterDataLoader interface
/// which we use to read in category/unit names and conversion data</param>
/// <param name="currencyDataLoader">An instance of the IConverterDataLoader
/// interface, specialized for loading currency data from an internet
/// service</param>
UnitConverter::UnitConverter(
    _In_ const shared_ptr<IConverterDataLoader> &dataLoader,
    _In_ const shared_ptr<IConverterDataLoader> &currencyDataLoader) {
...
void UnitConverter::Initialize() { m_dataLoader->LoadData(); }
...
UnitConverter::SetCurrentCategory(const Category &input) {
  if (m_currencyDataLoader != nullptr &&
      m_currencyDataLoader->SupportsCategory(input)) {
...
/// </summary>
Category UnitConverter::GetCurrentCategory() { return m_currentCategory; }
...
void UnitConverter::SetCurrentUnitTypes(const Unit &fromType,
                                        const Unit &toType) {
  if (!CheckLoad()) {
    return;
  }
...
/// We use this to handle cases where the front-end may choose to trim more
/// digits than we have been storing internally, in which case appending will
/// not function as expected without the use of this parameter.
/// </param>
void UnitConverter::SwitchActive(const wstring &newValue) {
  ...
  if (m_currencyDataLoader != nullptr && m_vmCurrencyCallback != nullptr) {
    shared_ptr<ICurrencyConverterDataLoader> currencyDataLoader =
        GetCurrencyConverterDataLoader();
    const pair<wstring, wstring> currencyRatios =
        currencyDataLoader->GetCurrencyRatioEquality(m_fromType, m_toType);

    m_vmCurrencyCallback->CurrencyRatiosCallback(currencyRatios.first,
                                                 currencyRatios.second);
  }
}

wstring UnitConverter::CategoryToString(const Category &c,
                                        const wchar_t *delimiter) {
  wstringstream out(wstringstream::out);
  out << Quote(std::to_wstring(c.id)) << delimiter
      << Quote(std::to_wstring(c.supportsNegative)) << delimiter
      << Quote(c.name) << delimiter;
  return out.str();
}

vector<wstring> UnitConverter::StringToVector(const wstring &w,
                                              const wchar_t *delimiter,
                                              bool addRemainder) {
  ...
  while (delimiterIndex != w.npos) {
    serializedTokens.push_back(
        w.substr(startIndex, delimiterIndex - startIndex));
...
wstring UnitConverter::UnitToString(const Unit &u, const wchar_t *delimiter) {
  wstringstream out(wstringstream::out);
  out << Quote(std::to_wstring(u.id)) << delimiter << Quote(u.name) << delimiter
      << Quote(u.abbreviation) << delimiter
      << std::to_wstring(u.isConversionSource) << delimiter
      << std::to_wstring(u.isConversionTarget) << delimiter
      << std::to_wstring(u.isWhimsical) << delimiter;
  return out.str();
}
...
wstring UnitConverter::ConversionDataToString(ConversionData d,
                                              const wchar_t *delimiter) {
  wstringstream out(wstringstream::out);
  out.precision(32);
  out << fixed << d.ratio;
  ...
  TrimString(ratio);
  TrimString(offset);
  out << Quote(ratio) << delimiter << Quote(offset) << delimiter
      << std::to_wstring(d.offsetFirst) << delimiter;
  return out.str();
}
...
wstring UnitConverter::Serialize() {
  ...
  out << std::to_wstring(m_currentHasDecimal) << delimiter
      << std::to_wstring(m_returnHasDecimal) << delimiter
      << std::to_wstring(m_switchedActive) << delimiter;
  ...
  for (const auto &cur : m_ratioMap) {
    ...
    for (const auto &curConversion : cur.second) {
      ...
      unitToUnitToDoubleString
          << ConversionDataToString(curConversion.second, delimiter) << ":,";
    }
    unitToUnitToDoubleString << "["
                             << "]";
  }
...
/// </summary>
/// <param name="serializedData">wstring holding the serialized data. If it does
/// not have expected number of parameters, we will ignore it</param>
void UnitConverter::DeSerialize(const wstring &serializedData) {
  ...
  for (wstring token : ratioMapTokens) {
    ...
    vector<wstring> ratioMapList =
        StringToVector(ratioMapComponentTokens[1], L",");
    for (wstring subtoken : ratioMapList) {
      vector<wstring> ratioMapSubComponentTokens =
          StringToVector(subtoken, L":");
      assert(ratioMapSubComponentTokens.size() ==
             EXPECTEDMAPCOMPONENTTOKENCOUNT);
      Unit subkey = StringToUnit(ratioMapSubComponentTokens[0]);
      ConversionData conversion =
          StringToConversionData(ratioMapSubComponentTokens[1]);
...
/// De-Serializes the data in the converter from a string
/// </summary>
/// <param name="userPreferences">wstring holding the serialized data. If it
/// does not have expected number of parameters, we will ignore it</param>
...
/// <summary>
/// Serializes the Category and Associated Units in the converter and returns it
/// as a string
/// </summary>
...
/// <summary>
/// Sanitizes the input string, escape quoting any symbols we rely on for our
/// delimiters, and returns the sanitized string.
/// </summary>
...
/// <summary>
/// Unsanitizes the sanitized input string, returning it to its original
/// contents before we had quoted it.
/// </summary>
...
/// </summary>
/// <param name="command">Command enum representing the command that was
/// entered</param>
void UnitConverter::SendCommand(Command command) {
  ...
  if ((m_currentHasDecimal &&
       m_currentDisplay.size() - 1 >= MAXIMUMDIGITSALLOWED) ||
      (!m_currentHasDecimal &&
       m_currentDisplay.size() >= MAXIMUMDIGITSALLOWED)) {
    clearBack = true;
  }
  ...
  case Command::Backspace:
    clearFront = false;
    clearBack = false;
    if ((m_currentDisplay.front() != '-' && m_currentDisplay.size() > 1) ||
        m_currentDisplay.size() > 2) {
...
/// </summary>
/// <param name="newCallback">instance of IDisplayCallback interface that
/// receives our update calls</param>
void UnitConverter::SetViewModelCallback(
    _In_ const shared_ptr<IUnitConverterVMCallback> &newCallback) {
...
void UnitConverter::SetViewModelCurrencyCallback(
    _In_ const shared_ptr<IViewModelCurrencyCallback> &newCallback) {
...
shared_ptr<ICurrencyConverterDataLoader>
UnitConverter::GetCurrencyConverterDataLoader() {
  return dynamic_pointer_cast<ICurrencyConverterDataLoader>(
      m_currencyDataLoader);
}

/// <summary>
/// Converts a double value into another unit type, currently by multiplying by
/// the given double ratio
/// </summary>
...
/// <summary>
/// Calculates the suggested values for the current display value and returns
/// them as a vector
/// </summary>
vector<tuple<wstring, Unit>> UnitConverter::CalculateSuggested() {
  ...
  // Calculate converted values for every other unit type in this category,
  ...
  // Sort the resulting list by absolute magnitude, breaking ties by choosing
  // the positive value
  ...
  // Now that the list is sorted, iterate over it and populate the return vector
  // with properly rounded and formatted return strings
...
void UnitConverter::Reset() {
  ...
  for (const Category &category : m_categories) {
    shared_ptr<IConverterDataLoader> activeDataLoader =
        GetDataLoaderForCategory(category);
    if (activeDataLoader == nullptr) {
      // The data loader is different depending on the category, e.g. currency
      // data loader is different from the static data loader. If there is no
      // data loader for this category, continue.
      continue;
    }
    ...
    // Just because the units are empty, doesn't mean the user can't select this
    // category, we just want to make sure we don't let an unready category be
    // the default.
    if (!units.empty()) {
      for (Unit u : units) {
...
/// </summary>
shared_ptr<IConverterDataLoader>
UnitConverter::GetDataLoaderForCategory(const Category &category) {
  if (m_currencyDataLoader != nullptr &&
      m_currencyDataLoader->SupportsCategory(category)) {
...
void UnitConverter::Calculate() {
  unordered_map<Unit, ConversionData, UnitHash> conversionTable =
      m_ratioMap[m_fromType];
  double returnValue = stod(m_currentDisplay);
  if (AnyUnitIsEmpty() || (conversionTable[m_toType].ratio == 1.0 &&
                           conversionTable[m_toType].offset == 0.0)) {
    m_returnDisplay = m_currentDisplay;
    ...
    if (numPreDecimal > MAXIMUMDIGITSALLOWED ||
        (returnValue != 0 && abs(returnValue) < MINIMUMDECIMALALLOWED)) {
      wstringstream out(wstringstream::out);
      ...
      if (m_currentDisplay.size() <= OPTIMALDIGITSALLOWED &&
          abs(returnValue) >= OPTIMALDECIMALALLOWED) {
        returnString = RoundSignificant(
            returnValue,
            OPTIMALDIGITSALLOWED - min(numPreDecimal, OPTIMALDIGITSALLOWED));
      } else {
        returnString = RoundSignificant(
            returnValue,
            MAXIMUMDIGITSALLOWED - min(numPreDecimal, MAXIMUMDIGITSALLOWED));
      }
...
/// <param name="numSignificant">int number of significant digits to round
/// to</param>
...
void UnitConverter::UpdateCurrencySymbols() {
  if (m_currencyDataLoader != nullptr && m_vmCurrencyCallback != nullptr) {
    shared_ptr<ICurrencyConverterDataLoader> currencyDataLoader =
        GetCurrencyConverterDataLoader();
    const pair<wstring, wstring> currencySymbols =
        currencyDataLoader->GetCurrencySymbols(m_fromType, m_toType);
    const pair<wstring, wstring> currencyRatios =
        currencyDataLoader->GetCurrencyRatioEquality(m_fromType, m_toType);

    m_vmCurrencyCallback->CurrencySymbolsCallback(currencySymbols.first,
                                                  currencySymbols.second);
    m_vmCurrencyCallback->CurrencyRatiosCallback(currencyRatios.first,
                                                 currencyRatios.second);
  }
}
