...
unsigned int CalculatorHistory::AddToHistory(
  ...
  // Prefixing and suffixing the special Unicode markers to ensure that the
  // expression in the history doesn't get broken for RTL languages
  spHistoryItem->historyItemVector.expression =
      L'\u202d' + generatedExpression + L'\u202c';
...
unsigned int
CalculatorHistory::AddItem(_In_ shared_ptr<HISTORYITEM> const &spHistoryItem) {
...
CalculatorHistory::GetHistoryItem(_In_ unsigned int uIdx) {
  assert(uIdx >= 0 && uIdx < m_historyItems.size());
  return m_historyItems.at(uIdx);
}

CalculatorHistory::~CalculatorHistory(void) { ClearHistory(); }

void CalculatorHistory::ClearHistory() { m_historyItems.clear(); }
