...
CHistoryCollector::CHistoryCollector(
    ...
    std::shared_ptr<IHistoryDisplay> pHistoryDisplay, wchar_t decimalSymbol)
    : m_pHistoryDisplay(pHistoryDisplay), m_pCalcDisplay(pCalcDisplay),
...
void CHistoryCollector::AddOpndToHistory(wstring_view numStr,
                                         Rational const &rat,
                                         bool fRepetition) {
  std::shared_ptr<CalculatorVector<int>> commands =
      std::make_shared<CalculatorVector<int>>();
  // Check for negate
  ...
  auto operandCommand =
      std::make_shared<COpndCommand>(commands, fNegative, fDecimal, fSciFmt);
...
void CHistoryCollector::RemoveLastOpndFromHistory() {
  ...
  SetExpressionDisplay();
  m_lastOpStartIndex = -1;
  // This will not restore the m_lastBinOpStartIndex, as it isn't possible to
  // remove that also later
}
...
// This is expected to be called when a binary op in the last say 1+2+ is
// changing to another one say 1+2* (+ changed to *) It needs to know by this
// change a Precedence inversion happened. i.e. previous op was lower or equal
// to its previous op, but the new one isn't. (Eg. 1*2* to 1*2^). It can add
// explicit brackets to ensure the precedence is inverted. (Eg. (1*2) ^)
...
void CHistoryCollector::AddOpenBraceToHistory() {
  ...
  int ichOpndStart =
      IchAddSzToEquationSz(CCalcEngine::OpCodeToString(IDC_OPENP), -1);
...
void CHistoryCollector::EnclosePrecInversionBrackets() {
  ...
  int ichStart =
      (m_curOperandIndex > 0) ? m_operandIndices[m_curOperandIndex - 1] : 0;
...
// AddUnaryOpToHistory
//
// This is does the postfix to prefix translation of the input and adds the text
// to the history. Eg. doing 2 + 4 (sqrt), this routine will ensure the last
// sqrt call unary operator, actually goes back in history and wraps 4 in
// sqrt(4)
//
void CHistoryCollector::AddUnaryOpToHistory(int nOpCode, bool fInv,
  ...
  } else // all the other unary ops
  {
    ...
    if (IDC_SIGN == nOpCode) {
      ...
      case IDC_SIN:
        command =
            fInv ? static_cast<int>(CalculationManager::Command::CommandASIN)
                 : IDC_SIN;
        spExpressionCommand = std::make_shared<CUnaryCommand>(
            static_cast<int>(angleOpCode), command);
        break;
      case IDC_COS:
        command =
            fInv ? static_cast<int>(CalculationManager::Command::CommandACOS)
                 : IDC_COS;
        spExpressionCommand = std::make_shared<CUnaryCommand>(
            static_cast<int>(angleOpCode), command);
        break;
      case IDC_TAN:
        command =
            fInv ? static_cast<int>(CalculationManager::Command::CommandATAN)
                 : IDC_TAN;
        spExpressionCommand = std::make_shared<CUnaryCommand>(
            static_cast<int>(angleOpCode), command);
        break;
      case IDC_SINH:
        command =
            fInv ? static_cast<int>(CalculationManager::Command::CommandASINH)
                 : IDC_SINH;
        spExpressionCommand = std::make_shared<CUnaryCommand>(command);
        break;
      case IDC_COSH:
        command =
            fInv ? static_cast<int>(CalculationManager::Command::CommandACOSH)
                 : IDC_COSH;
        spExpressionCommand = std::make_shared<CUnaryCommand>(command);
        break;
      case IDC_TANH:
        command =
            fInv ? static_cast<int>(CalculationManager::Command::CommandATANH)
                 : IDC_TANH;
        spExpressionCommand = std::make_shared<CUnaryCommand>(command);
        break;
      case IDC_LN:
        command =
            fInv ? static_cast<int>(CalculationManager::Command::CommandPOWE)
                 : IDC_LN;
        spExpressionCommand = std::make_shared<CUnaryCommand>(command);
        break;
      default:
    ...
    wstring operandStr{
        CCalcEngine::OpCodeToUnaryString(nOpCode, fInv, angletype)};
    if (!m_bLastOpndBrace) // The opnd is already covered in braces. No need for
                           // additional braces around it
    {
  ...
  SetExpressionDisplay();
  m_bLastOpndBrace = false;
  // m_lastOpStartIndex remains the same as last opnd is just replaced by
  // unaryop(lastopnd)
  m_lastBinOpStartIndex = -1;
}

// Called after = with the result of the equation
// Responsible for clearing the top line of current running history display, as
// well as adding yet another element to history of equations
void CHistoryCollector::CompleteHistoryLine(wstring_view numStr) {
  if (nullptr != m_pCalcDisplay) {
    m_pCalcDisplay->SetExpressionDisplay(
        std::make_shared<CalculatorVector<std::pair<std::wstring, int>>>(),
        std::make_shared<
            CalculatorVector<std::shared_ptr<IExpressionCommand>>>());
  }

  if (nullptr != m_pHistoryDisplay) {
    unsigned int addedItemIndex =
        m_pHistoryDisplay->AddToHistory(m_spTokens, m_spCommands, numStr);
...
// Adds the given string psz to the globally maintained current equation string
// at the end.
//  Also returns the 0 based index in the string just added. Can throw out of
//  memory error
int CHistoryCollector::IchAddSzToEquationSz(wstring_view str,
                                            int icommandIndex) {
  if (m_spTokens == nullptr) {
    m_spTokens =
        std::make_shared<CalculatorVector<std::pair<std::wstring, int>>>();
...
// Inserts a given string into the global m_pszEquation at the given index ich
// taking care of reallocations etc.
void CHistoryCollector::InsertSzInEquationSz(wstring_view str,
                                             int icommandIndex, int ich) {
  if (FAILED(m_spTokens->InsertAt(
          ich, std::make_pair(wstring(str), icommandIndex)))) {
    throw(CALC_E_OUTOFMEMORY);
  }
}
...
int CHistoryCollector::AddCommand(
    _In_ const std::shared_ptr<IExpressionCommand> &spCommand) {
  if (m_spCommands == nullptr) {
    m_spCommands = std::make_shared<
        CalculatorVector<std::shared_ptr<IExpressionCommand>>>();
  }
...
void CHistoryCollector::UpdateHistoryExpression(uint32_t radix,
                                                int32_t precision) {
  if (m_spTokens == nullptr) {
    return;
  }
  ...
  for (unsigned int i = 0; i < size; ++i) {
    ...
    if (commandPosition != -1) {
      ...
      if (expCommand != nullptr &&
          CalculationManager::CommandType::OperandCommand ==
              expCommand->GetCommandType()) {
        std::shared_ptr<COpndCommand> opndCommand =
            std::static_pointer_cast<COpndCommand>(expCommand);
...
std::shared_ptr<CalculatorVector<int>>
CHistoryCollector::GetOperandCommandsFromString(wstring_view numStr) {
  std::shared_ptr<CalculatorVector<int>> commands =
      std::make_shared<CalculatorVector<int>>();
  // Check for negate
  bool fNegative = (numStr[0] == L'-');
