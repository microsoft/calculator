...
// Truncates if too big, makes it a non negative - the number in rat. Doesn't do
// anything if not in INT mode
CalcEngine::Rational
CCalcEngine::TruncateNumForIntMath(CalcEngine::Rational const &rat) {
  if (!m_fIntegerMode) {
    return rat;
  }
  ...
  // Can be converting a dec negative number to Hex/Oct/Bin rep. Use 2's
  // complement form Check the range.
  if (result < 0) {
...
void CCalcEngine::DisplayNum(void) {
  ...
  //  called.
  //
  if (m_bRecord || gldPrevious.value != m_currentVal ||
      gldPrevious.precision != m_precision || gldPrevious.radix != m_radix ||
      gldPrevious.nFE != (int)m_nFE || gldPrevious.bUseSep != true ||
    ...
    if (m_bRecord) {
      ...
      // If we're in Programmer mode, perform integer truncation so e.g. 5 / 2 *
      // 2 results in 4, not 5.
      if (m_fIntegerMode) {
    ...
    // Displayed number can go through transformation. So copy it after
    // transformation
    gldPrevious.value = m_currentVal;

    if ((m_radix == 10) &&
        IsNumberInvalid(m_numberString, MAX_EXPONENT, m_precision, m_radix)) {
...
int CCalcEngine::IsNumberInvalid(const wstring &numberString, int iMaxExp,
                                 int iMaxMantissa, uint32_t radix) const {
  int iError = 0;
  ...
  if (radix == 10) {
    ...
    wregex rx(wstring{c_decPreSepStr} + m_decimalSeparator +
              wstring{c_decPostSepStr});
    wsmatch matches;
...
\****************************************************************************/
vector<uint32_t>
CCalcEngine::DigitGroupingStringToGroupingVector(wstring_view groupingString) {
  ...
  for (const wchar_t *itr = groupingString.data(); *itr != L'\0'; ++itr) {
    ...
    // The loop will then increment us to the next character, which should be a
    // number.
    if (next && (static_cast<size_t>(next - groupingString.data()) <
                 groupingString.length())) {
      itr = next;
    }
  }
...
wstring CCalcEngine::GroupDigitsPerRadix(wstring_view numberString,
                                         uint32_t radix) {
  ...
  switch (radix) {
  case 10:
    return GroupDigits(wstring{m_groupSeparator}, m_decGrouping, numberString,
                       (L'-' == numberString[0]));
  case 8:
...
*
\***************************************************************************/
wstring CCalcEngine::GroupDigits(wstring_view delimiter,
                                 vector<uint32_t> const &grouping,
                                 wstring_view displayString,
                                 bool isNumNegative) {
  ...
  // Create an iterator that points to the end of the portion of the number
  // subject to grouping (i.e. left of the decimal)
  ...
  // Mark the 'end' of the string as either rend() or rend()-1 if there is a
  // negative sign We exclude the sign here because we don't want to end up with
  // e.g. "-,123,456" Then, iterate from back to front, adding group delimiters
  // as needed.
  ...
  while (ritr != reverse_end) {
    ...
    if (currGrouping != 0 && (groupingSize % currGrouping) == 0 &&
        ritr != reverse_end) {
      ...
      if (groupItr != grouping.end()) {
        ...
        // A 0 in another position is a group. So, "3;0;0" means "group 3, then
        // group 0 repeatedly" This could be expressed as just "3" but
        // GetLocaleInfo is returning 3;0;0 in some cases instead.
  ...
  // Add the right (fractional or exponential) part of the number to the final
  // string.
  if (hasDecimal) {
