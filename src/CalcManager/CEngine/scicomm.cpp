...
// NPrecedenceOfOp
//
// returns a virtual number for precedence for the operator. We expect binary
// operator only, otherwise the lowest number 0 is returned. Higher the number,
// higher the precedence of the operator.
...
// HandleErrorCommand
//
// When it is discovered by the state machine that at this point the input is
// not valid (eg. "1+)"), we want to proceed as though this input never occurred
// and may be some feedback to user like Beep. The rest of input can then
// continue by just ignoring this command.
...
void CCalcEngine::ProcessCommandWorker(OpCode wParam) {
  ...
  if (m_bRecord) {
    if (IsOpInRange(wParam, IDC_AND, IDC_MMINUS) ||
        ...
        (IDC_INV == wParam) || (IDC_SIGN == wParam && 10 != m_radix)) {
      m_bRecord = false;
  ...
  if (IsDigitOpCode(wParam)) {
    ...
    if (!m_input.TryAddDigit(iValue, m_radix, m_fIntegerMode,
                             m_maxDecimalValueStrings[m_numwidth],
                             m_dwWordBitWidth, m_cIntDigitsSav)) {
      HandleErrorCommand(wParam);
  ...
  if (IsBinOpCode(wParam)) {
    ...
    if (IsBinOpCode(m_nLastCom)) {
      int nPrev;
      bool fPrecInvToHigher = false; // Is Precedence Inversion from lower to
                                     // higher precedence happening ??

      m_nOpCode = (int)wParam;

      // Check to see if by changing this binop, a Precedence inversion is
      // happening. Eg. 1 * 2  + and + is getting changed to ^. The previous
      // precedence rules would have already computed 1*2, so we will put
      // additional brackets to cover for precedence inversion and it will
      // become (1 * 2) ^ Here * is m_nPrevOpCode, m_currentVal is 2  (by 1*2),
      // m_nLastCom is +, m_nOpCode is ^
      if (m_fPrecedence && 0 != m_nPrevOpCode) {
        ...
        if (nx <= nPrev && ni > nPrev) // condition for Precedence Inversion
        {
          ...
          m_nPrevOpCode = 0; // Once the precedence inversion has put additional
                             // brackets, its no longer required
        }
      }
    ...
    if (!m_HistoryCollector.FOpndAddedToHistory()) {
      // if the prev command was ) or unop then it is already in history as a
      // opnd form (...)
    ...
    DoPrecedenceCheckAgain:
      ...
      if ((nx > ni) && m_fPrecedence) {
        if (m_precedenceOpCount < MAXPRECDEPTH) {
          ...
          m_HistoryCollector
              .PushLastOpndStart(); // Eg. 1 + 2  *, Need to remember the start
                                    // of 2 to do Precedence inversion if need
                                    // to
        } else {
        ...
        if ((m_precedenceOpCount != 0) &&
            (m_nPrecOp[m_precedenceOpCount - 1])) {
          m_precedenceOpCount--;
          ...
          // Precedence Inversion Higher to lower can happen which needs
          // explicit enclosure of brackets Eg.  1 + 2 * Or 3 Or.  We would have
          // pushed 1+ before, and now last + forces 2 Or 3 to be evaluated
          // because last Or is less or equal to first + (after 1). But we see
          // that 1+ is in stack and we evaluated to 2 Or 3 This is precedence
          // inversion happened because of operator changed in between. We put
          // extra brackets like 1 + (2 Or 3)
          if (ni <= nx) {
  ...
  if (IsUnaryOpCode(wParam) || (wParam == IDC_DEGREES)) {
    ...
    if ((wParam == IDC_SIN) || (wParam == IDC_COS) || (wParam == IDC_TAN) ||
    ...
    if (wParam == IDC_PERCENT) {
      CheckAndAddLastBinOpToHistory();
      m_HistoryCollector.AddOpndToHistory(
          m_numberString, m_currentVal,
    ...
    if (m_bInv &&
        ((wParam == IDC_CHOP) || (wParam == IDC_SIN) || (wParam == IDC_COS) ||
         ...
         (wParam == IDC_DEGREES) || (wParam == IDC_SINH) ||
         (wParam == IDC_COSH) || (wParam == IDC_TANH))) {
      m_bInv = false;
    }
  ...
  {
    if (!m_bChangeOp) {
      // A special goody we are doing to preserve the history, if all was done
      // was serious of unary operations last
    ...
    m_bChangeOp = false;
    m_precedenceOpCount = m_nTempCom = m_nLastCom = m_nOpCode =
        m_openParenCount = 0;
    m_nPrevOpCode = 0;
    m_bNoPrevEqu = true;
  ...
  case IDC_EQU:
    while (m_openParenCount > 0) {
      // when m_bError is set and m_ParNum is non-zero it goes into infinite
      // loop
      if (m_bError) {
        break;
      }
      // automatic closing of all the parenthesis to get a meaningful result as
      // well as ensure data integrity
      m_nTempCom = m_nLastCom; // Put back this last saved command to the prev
                               // state so ) can be handled properly
      ProcessCommand(IDC_CLOSEP);
      m_nLastCom = m_nTempCom;  // Actually this is IDC_CLOSEP
      m_nTempCom = (int)wParam; // put back in the state where last op seen was
                                // IDC_CLOSEP, and current op is IDC_EQU
    }

    if (!m_bNoPrevEqu) {
      // It is possible now unary op changed the num in screen, but still
      // m_lastVal hasn't changed.
      m_lastVal = m_currentVal;
    }
    ...
    do {
      ...
      if (m_nOpCode) /* Is there a valid operation around?        */
      {
        ...
        /* Otherwise let m_currentVal=m_holdVal.  This keeps m_currentVal
         * constant */
  ...
  case IDC_CLOSEP:
    ...
    if ((m_openParenCount >= MAXPRECDEPTH && nx) ||
        (!m_openParenCount && !nx) ||
        ((m_precedenceOpCount >= MAXPRECDEPTH &&
          m_nPrecOp[m_precedenceOpCount - 1] != 0))) {
    ...
    if (nx) {
      ...
      m_lastVal = 0;
      if (IsBinOpCode(m_nLastCom)) {
        // We want 1 + ( to start as 1 + (0. Any number you type replaces 0. But
        // if it is 1 + 3 (, it is treated as 1 + (3
        m_currentVal = 0;
      }
      m_nTempCom = 0;
      m_nOpCode = 0;
      ...
      // Last thing keyed in was an operator. Lets do the op on a duplicate of
      // the last entry.
      if (IsBinOpCode(m_nLastCom)) {
        m_currentVal = m_lastVal;
      }
      ...
      // Now process the precedence stack till we get to an opcode which is
      // zero.
      for (m_nOpCode = m_nPrecOp[--m_precedenceOpCount]; m_nOpCode;
           m_nOpCode = m_nPrecOp[--m_precedenceOpCount]) {
        // Precedence Inversion check
        ni = NPrecedenceOfOp(m_nPrevOpCode);
        nx = NPrecedenceOfOp(m_nOpCode);
      ...
      // Now get back the operation and opcode at the beginning of this
      // parenthesis pair
    ...
    if (nullptr != m_pCalcDisplay) {
      m_pCalcDisplay->SetParenDisplayText(
          m_openParenCount ? to_wstring(m_openParenCount) : L"");
    }
  ...
  case IDC_SIGN: {
    if (m_bRecord) {
      if (m_input.TryToggleSign(m_fIntegerMode,
                                m_maxDecimalValueStrings[m_numwidth])) {
        DisplayNum();
      } else {
        HandleErrorCommand(wParam);
  ...
  case IDC_MPLUS: {
    ...
    m_memoryValue = make_unique<Rational>(TruncateNumForIntMath(
        result)); // Memory should follow the current int mode
  ...
  case IDC_MCLEAR:
    m_memoryValue = make_unique<Rational>(
        wParam == IDC_STORE ? TruncateNumForIntMath(m_currentVal) : 0);
    break;
...
// CheckAndAddLastBinOpToHistory
//
//  This is a very confusing helper routine to add the last entered binary
//  operator to the history. This is expected to
// leave the history with <exp> <binop> state. It can really add the last
// entered binary op, or it can actually remove the last operand from history.
// This happens because you can 'type' or 'compute' over last operand in some
// cases, thereby effectively removing only it from the equation but still
// keeping the previous portion of the equation. Eg. 1 + 4 sqrt 5. The last 5
// will remove sqrt(4) as it is not used anymore to participate in 1 + 5 If you
// are messing with this, test cases like this CE, statistical functions, ( & MR
// buttons
void CCalcEngine::CheckAndAddLastBinOpToHistory(bool addToHistory) {
  if (m_bChangeOp) {
    if (m_HistoryCollector.FOpndAddedToHistory()) {
      // if last time opnd was added but the last command was not a binary
      // operator, then it must have come from commands which add the operand,
      // like unary operator. So history at this is showing 1 + sqrt(4) but in
      // reality the sqrt(4) is getting replaced by new number (may be unary op,
      // or MR or SUM etc.) So erase the last operand
      m_HistoryCollector.RemoveLastOpndFromHistory();
    }

  } else if (m_HistoryCollector.FOpndAddedToHistory() && !m_bError) {
    // Corner case, where opnd is already in history but still a new opnd
    // starting (1 + 4 sqrt 5). This is yet another special casing of previous
    // case under if (m_bChangeOp), but this time we can do better than just
    // removing it Let us make a current value =. So in case of 4 SQRT (or a
    // equation under braces) and then a new equation is started, we can just
    // form a useful equation of sqrt(4) = 2 and continue a new equation from
    // now on. But no point in doing this for things like MR, SUM etc. All you
    // will get is 5 = 5 kind of no useful equation.
    if ((IsUnaryOpCode(m_nLastCom) || IDC_SIGN == m_nLastCom ||
         IDC_CLOSEP == m_nLastCom) &&
        0 == m_openParenCount) {
      if (addToHistory) {
        m_HistoryCollector.CompleteHistoryLine(
            GroupDigitsPerRadix(m_numberString, m_radix));
      }
    } else {
      m_HistoryCollector.RemoveLastOpndFromHistory();
    }
...
// change the display area from a static text to an editbox, which has the focus
// can make Magnifier (Accessibility tool) work
...
// since unary operators button names aren't exactly friendly for history
// purpose, we have this separate table to get its localized name and for its
// Inv function if it exists.
typedef struct {
  int idsFunc;    // index of string for the unary op function. Can be NULL, in
                  // which case it same as button name
  int idsFuncInv; // index of string for Inv of unary op. Can be NULL, in case
                  // it is same as idsFunc
  bool fDontUseInExpEval; // true if this cant be used in reverse direction as
                          // well, ie. during expression evaluation
} UFNE;
...
static const UFNE rgUfne[] = {
    ...
    /* IDC_COM   */ {0, 0, true},
    /* IDC_SIN    */ {IDS_SIND, IDS_ASIND, false}, // default in this table is
                                                   // degrees for sin,cos & tan
...
wstring_view CCalcEngine::OpCodeToUnaryString(int nOpCode, bool fInv,
...
//
// Sets the Angle Mode for special unary op IDC's which are used to index to the
// table rgUfne and returns the equivalent plain IDC for trigonometric function.
// If it isn't a trigonometric function returns the passed in idc itself.
...
int CCalcEngine::GetCurrentRadix() { return m_radix; }

wstring CCalcEngine::GetCurrentResultForRadix(uint32_t radix,
                                              int32_t precision) {
  Rational rat =
      (m_bRecord ? m_input.ToRational(m_radix, m_precision) : m_currentVal);
...
wstring CCalcEngine::GetStringForDisplay(Rational const &rat, uint32_t radix) {
  ...
  if (!m_fIntegerMode) {
    ...
    try {
      ...
      if ((radix == 10) && fMsb) {
        // If high bit is set, then get the decimal number in negative 2's
        // complement form.
