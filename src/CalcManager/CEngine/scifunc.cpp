...
CalcEngine::Rational
CCalcEngine::SciCalcFunctions(CalcEngine::Rational const &rat, uint32_t op) {
  Rational result{};
  try {
    switch (op) {
    case IDC_CHOP:
    ...
    case IDC_DEGREES:
      ...
      // so setting the IDC_INV command first and then performing 'dms'
      // operation as global variables m_bInv, m_bRecord are set properly
      // through ProcessCommand(IDC_INV)
      [[fallthrough]];
    case IDC_DMS: {
      if (!m_fIntegerMode) {
