...
int CParentheses::GetCommand() const { return m_command; }
...
void CBinaryCommand::SetCommand(int command) { m_command = command; }

int CBinaryCommand::GetCommand() const { return m_command; }
...
COpndCommand::COpndCommand(shared_ptr<CalculatorVector<int>> const &commands,
                           bool fNegative, bool fDecimal, bool fSciFmt)
    : m_commands(commands), m_fNegative(fNegative), m_fSciFmt(fSciFmt),
      m_fDecimal(fDecimal), m_fInitialized(false), m_value{} {}
...
void COpndCommand::SetCommands(
    shared_ptr<CalculatorVector<int>> const &commands) {
  m_commands = commands;
}
...
bool COpndCommand::IsNegative() const { return m_fNegative; }

bool COpndCommand::IsSciFmt() const { return m_fSciFmt; }

bool COpndCommand::IsDecimalPresent() const { return m_fDecimal; }

CalculationManager::CommandType COpndCommand::GetCommandType() const {
  return CalculationManager::CommandType::OperandCommand;
}

void COpndCommand::ClearAllAndAppendCommand(
    CalculationManager::Command command) {
  m_commands->Clear();
