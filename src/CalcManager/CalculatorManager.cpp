...
#define MEMORY_COMMAND_TO_UNSIGNED_CHAR(c)                                     \
  __pragma(warning(push))                                                      \
      __pragma(warning(disable : 4309)) static_cast<unsigned char>(c)          \
          __pragma(warning(pop))

namespace CalculationManager {
CalculatorManager::CalculatorManager(_In_ ICalcDisplay *displayCallback,
                                     _In_ IResourceProvider *resourceProvider)
    : m_displayCallback(displayCallback), m_currentCalculatorEngine(nullptr),
      m_resourceProvider(resourceProvider), m_inHistoryItemLoadMode(false),
...
void CalculatorManager::SetPrimaryDisplay(_In_ const wstring &displayString,
                                          _In_ bool isError) {
...
void CalculatorManager::DisplayPasteError() {
  m_currentCalculatorEngine->DisplayError(
      CALC_E_DOMAIN /*code for "Invalid input" error*/);
}
...
/// </summary>
/// <param name="expressionString">wstring representing expression to be
/// displayed</param>
...
/// Passed in string representations of memorized numbers get passed to the
/// client
/// </summary>
/// <param name="memorizedNumber">vector containing wstring values of memorized
/// numbers</param>
void CalculatorManager::SetMemorizedNumbers(
    _In_ const vector<wstring> &memorizedNumbers) {
...
/// </summary>
/// <param name="parenthesisCount">string containing the parenthesis
/// count</param>
...
void CalculatorManager::SetStandardMode() {
  if (!m_standardCalculatorEngine) {
    m_standardCalculatorEngine =
        make_unique<CCalcEngine>(false /* Respect Order of Operations */,
  ...
  m_currentCalculatorEngine->ChangePrecision(
      static_cast<int>(CalculatorPrecision::StandardModePrecision));
  UpdateMaxIntDigits();
...
void CalculatorManager::SetScientificMode() {
  if (!m_scientificCalculatorEngine) {
    m_scientificCalculatorEngine = make_unique<CCalcEngine>(
        true /* Respect Order of Operations */, false /* Set to Integer Mode */,
  ...
  m_currentCalculatorEngine->ProcessCommand(IDC_CLEAR);
  m_currentCalculatorEngine->ChangePrecision(
      static_cast<int>(CalculatorPrecision::ScientificModePrecision));
...
void CalculatorManager::SetProgrammerMode() {
  if (!m_programmerCalculatorEngine) {
    m_programmerCalculatorEngine = make_unique<CCalcEngine>(
        true /* Respect Order of Operations */, true /* Set to Integer Mode */,
        m_resourceProvider, this, nullptr);
  }
  ...
  m_currentCalculatorEngine->ProcessCommand(IDC_CLEAR);
  m_currentCalculatorEngine->ChangePrecision(
      static_cast<int>(CalculatorPrecision::ProgrammerModePrecision));
}
...
void CalculatorManager::SendCommand(_In_ Command command) {
  // When the expression line is cleared, we save the current state, which
  // includes, primary display, memory, and degree mode
  if (command == Command::CommandCLEAR || command == Command::CommandEQU ||
      command == Command::ModeBasic || command == Command::ModeScientific ||
  ...
  if (command == Command::CommandDEG || command == Command::CommandRAD ||
      command == Command::CommandGRAD) {
    m_currentDegreeMode = command;
  }

  if (command != Command::CommandFE) {
    m_savedCommands.push_back(MapCommandForSerialize(
        command)); // Save the commands in the m_savedCommands
  }

  switch (command) {
  case Command::CommandASIN:
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandINV));
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandSIN));
    break;
  case Command::CommandACOS:
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandINV));
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandCOS));
    break;
  case Command::CommandATAN:
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandINV));
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandTAN));
    break;
  case Command::CommandPOWE:
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandINV));
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandLN));
    break;
  case Command::CommandASINH:
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandINV));
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandSINH));
    break;
  case Command::CommandACOSH:
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandINV));
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandCOSH));
    break;
  case Command::CommandATANH:
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandINV));
    m_currentCalculatorEngine->ProcessCommand(
        static_cast<OpCode>(Command::CommandTANH));
    break;
...
/// Since some Commands are higher than 255, they are saved after subtracting
/// 255 The smallest Command is CommandSIGN = 80, thus, subtracted value does
/// not overlap with other values.
/// </summary>
/// <param name="command">Enum Command</command>
...
/// The command that is smaller than 80, CommandSIGN, can be converted back to
/// original value by adding 255.
/// </summary>
/// <param name="command">unsigned char value represent the saved
/// command</command>
unsigned int
...
/// <summary>
/// Return saved degree mode which is saved when last time the expression was
/// cleared.
/// </summary>
...
/// <summary>
/// Return serialized primary display that is saved when the expression line was
/// cleared.
/// </summary>
...
/// </summary>
/// <param name = "serializedPrimaryDisplay">Serialized Rational of primary
/// display</param>
void CalculatorManager::DeSerializePrimaryDisplay(
    const vector<long> &serializedPrimaryDisplay) {
  if (serializedPrimaryDisplay.empty()) {
    return;
  }
  m_persistedPrimaryValue =
      DeSerializeRational(serializedPrimaryDisplay.begin());
...
void CalculatorManager::SerializeMemory() {
  ...
  for (auto const &memoryItem : m_memorizedNumbers) {
    ...
    m_serializedMemory.insert(m_serializedMemory.end(), serialMem.begin(),
                              serialMem.end());
  }
}
...
void CalculatorManager::DeSerializeMemory(
    const vector<long> &serializedMemory) {
  ...
  while (itr != serializedMemory.end()) {
    ...
    auto lengthMemoryItem = (2 * SERIALIZED_NUMBER_MINSIZE) +
                            memoryItem.P().Mantissa().size() +
                            memoryItem.Q().Mantissa().size();
...
void CalculatorManager::DeSerializeCommands(
    _In_ const vector<unsigned char> &serializedData) {
  m_savedCommands.clear();

  for (auto commandItr = serializedData.begin();
       commandItr != serializedData.end(); ++commandItr) {
    if (*commandItr >=
            MEMORY_COMMAND_TO_UNSIGNED_CHAR(MemoryCommand::MemorizeNumber) &&
        *commandItr <= MEMORY_COMMAND_TO_UNSIGNED_CHAR(
                           MemoryCommand::MemorizedNumberClearAll)) {
      // MemoryCommands(which have values above 255) are pushed on
      // m_savedCommands upon casting to unsigned char. SerializeCommands uses
      // m_savedCommands, which is then used in DeSerializeCommands. Hence, a
      // simple cast to MemoryCommand is not sufficient.
      MemoryCommand memoryCommand =
          static_cast<MemoryCommand>(*commandItr + UCHAR_MAX + 1);
      ...
      case MemoryCommand::MemorizedNumberLoad:
        if (commandItr + 1 == serializedData.end()) {
          throw out_of_range(
              "Expecting index of memory, data ended prematurely");
        }
      ...
      this->SendCommand(
          static_cast<Command>(MapCommandForDeSerialize(*commandItr)));
    }
  }
}
...
/// </summary>
/// <param name="indexOfMemory">Index of the target memory</param>
void CalculatorManager::MemorizedNumberSubtract(
    _In_ unsigned int indexOfMemory) {
...
void CalculatorManager::MemorizedNumberClearAll() {
  m_savedCommands.push_back(
      MEMORY_COMMAND_TO_UNSIGNED_CHAR(MemoryCommand::MemorizedNumberClearAll));
...
/// <summary>
/// Helper function that selects a memory from the vector and set it to
/// CCalcEngine Saved RAT number needs to be copied and passed in, as
/// CCalcEngine destroyed the passed in RAT
/// </summary>
/// <param name="indexOfMemory">Index of the target memory</param>
...
/// </summary>
/// <param name="indexOfMemory">Index of the target memory</param>
void CalculatorManager::MemorizedNumberChanged(
    _In_ unsigned int indexOfMemory) {
  if (m_currentCalculatorEngine->FInErrorState()) {
    return;
  }
...
void CalculatorManager::SaveMemoryCommand(_In_ MemoryCommand command,
                                          _In_ unsigned int indexOfMemory) {
  m_savedCommands.push_back(MEMORY_COMMAND_TO_UNSIGNED_CHAR(command));
  if (indexOfMemory > UCHAR_MAX) {
    throw invalid_argument("Unexpected value. IndexOfMemory is bigger than the "
...
vector<shared_ptr<HISTORYITEM>> const &
CalculatorManager::GetHistoryItems(_In_ CALCULATOR_MODE mode) {
  return (mode == CM_STD) ? m_pStdHistory->GetHistory()
                          : m_pSciHistory->GetHistory();
}

shared_ptr<HISTORYITEM> const &
CalculatorManager::GetHistoryItem(_In_ unsigned int uIdx) {
...
void CalculatorManager::SetMemorizedNumbersString() {
  ...
  for (auto const &memoryItem : m_memorizedNumbers) {
    ...
    wstring stringValue =
        m_currentCalculatorEngine->GetStringForDisplay(memoryItem, radix);

    if (!stringValue.empty()) {
      resultVector.push_back(
          m_currentCalculatorEngine->GroupDigitsPerRadix(stringValue, radix));
    }
  }
...
void CalculatorManager::SetHistory(
    _In_ CALCULATOR_MODE eMode,
    _In_ vector<shared_ptr<HISTORYITEM>> const &history) {
...
wstring CalculatorManager::GetResultForRadix(uint32_t radix,
                                             int32_t precision) {
  return m_currentCalculatorEngine
             ? m_currentCalculatorEngine->GetCurrentResultForRadix(radix,
                                                                   precision)
             : L"";
}
...
wchar_t CalculatorManager::DecimalSeparator() {
  return m_currentCalculatorEngine
             ...
             : m_resourceProvider->GetCEngineString(L"sDecimal")[0];
}
...
void CalculatorManager::SetInHistoryItemLoadMode(
    _In_ bool isHistoryItemLoadMode) {
...
vector<long> CalculatorManager::SerializeRational(Rational const &rat) {
  ...
  serializedRational.insert(serializedRational.end(), serialP.begin(),
                            serialP.end());

  auto serialQ = SerializeNumber(rat.Q());
  serializedRational.insert(serializedRational.end(), serialQ.begin(),
                            serialQ.end());
...
/// </summary>
Rational
CalculatorManager::DeSerializeRational(vector<long>::const_iterator itr) {
  auto p = DeSerializeNumber(itr);
  auto q =
      DeSerializeNumber(itr + SERIALIZED_NUMBER_MINSIZE + p.Mantissa().size());
