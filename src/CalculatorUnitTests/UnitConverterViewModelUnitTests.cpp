...
String ^
    AddUnicodeLTRMarkers(wstring str) {
...
TEST_CLASS(SupplementaryResultsViewModelTests){
  public :
      // Test all binding values:
...
VM::Unit ^ vmunit = ref new VM::Unit(unit);
VM::SupplementaryResult ^ vmsupp =
    ref new VM::SupplementaryResult(ref new String(value.c_str()), vmunit);
...
TEST_METHOD(TestGetUnitNameReturnsCorrectValue) {
  ...
  VM::Unit ^ vmunit = ref new VM::Unit(unit);
  VM::SupplementaryResult ^ vmsupp =
      ref new VM::SupplementaryResult(L"", vmunit);
...
TEST_METHOD(TestGetIsWhimsicalReturnsCorrectValue) {
  ...
  VM::SupplementaryResult ^ vmsupp =
      ref new VM::SupplementaryResult(L"", vmUnit);
  VM::SupplementaryResult ^ vmsuppW =
      ref new VM::SupplementaryResult(L"", vmUnitW);
...
UnitConverterMock::UnitConverterMock()
    : m_initCallCount(0), m_getCategoriesCallCount(0),
      m_setCurrentCategoryCallCount(0), m_setCurUnitTypesCallCount(0),
      m_switchActiveCallCount(0), m_sendCommandCallCount(0),
      m_setVMCallbackCallCount(0), m_serializeCallCount(0),
      m_deSerializeCallCount(0) {}

// IUnitConverter

void UnitConverterMock::Initialize() { m_initCallCount++; }
...
UCM::CategorySelectionInitializer
UnitConverterMock::SetCurrentCategory(const UCM::Category &input) {
...
UCM::Category UnitConverterMock::GetCurrentCategory() { return m_curCategory; }

void UnitConverterMock::SetCurrentUnitTypes(const UCM::Unit &fromType,
                                            const UCM::Unit &toType) {
...
std::wstring UnitConverterMock::SaveUserPreferences() { return L"TEST"; };

void UnitConverterMock::RestoreUserPreferences(
    _In_ const std::wstring & /*userPreferences*/){};
...
void UnitConverterMock::SetViewModelCallback(
    const shared_ptr<UCM::IUnitConverterVMCallback> &newCallback) {
...
public:
  ...
  virtual property bool IsActive {
    ...
    void set(bool value) { m_active = value; }
  }
};

TEST_CLASS(UnitConverterDataLoaderTests){
  public : TEST_METHOD(FuncUnitConverterAllUnitCombinations){
      VM::UnitConverterViewModel ^ vm = ref new UnitConverterViewModel(
          make_shared<UnitConversionManager::UnitConverter>(
...
double epsilon =
    0.1; // Could be more precise like 0.001 except atm to pascal conversion
...
for (unsigned int k = 0; k < categoryList->Size; k++) {
  ...
  for (unsigned int i = 0; i < unitList->Size; i++) {
    ...
    for (unsigned int j = 0; j < unitList->Size; j++) {
      ...
      String ^ expectedResult = m_resLoader->GetString(
          ref new String((unit1Name + L"-" + unit2Name).c_str()));
      ...
      if (expectedResult != nullptr) {
        ...
        double actualConversion =
            GetDoubleFromWstring(GetStringValue(vm->Value1)->Data());
        double diff = abs(expectedConversion - actualConversion);

        // Assert for diff less than epsilonth fraction of expected conversion
        // result
...
TEST_CLASS(UnitConverterViewModelTests){
  public :
      ...
      TEST_METHOD(TestUnitConverterCtorSetsUpCorrectActiveValue){
...
TEST_METHOD(TestUnitSelectionChangeUpdatesModel) {
  shared_ptr<UnitConverterMock> mock = make_shared<UnitConverterMock>();
  VM::UnitConverterViewModel vm(mock);
  vm.Unit1 = vm.Units->GetAt(
      1); // Change from u4 to u5
...
TEST_METHOD(TestCategorySelectionChangeUpdatesUnits) {
  ...
  VM::UnitConverterViewModel vm(mock);
  vm.CurrentCategory =
      vm.Categories->GetAt(2); // Change from cat1 to cat3
                               // Counts will be 2 here since the first call
                               // should have happened during init
...
TEST_METHOD(TestOnValueGotFocusActivatesControl) {
  ...
  // vm.OnValueGotFocus(AsActivatable(activatable)); // try again, starting with
  // true VERIFY_IS_TRUE(activatable->IsActive);
}
...
TEST_METHOD(
    TestDisplayValueUpdatesAfterSwitchingActiveTwiceUpdateTheRightDisplay) {
  shared_ptr<UnitConverterMock> mock = make_shared<UnitConverterMock>();
  VM::UnitConverterViewModel vm(mock);
  const WCHAR *vFrom = L"1", *vTo = L"234";
  vm.UpdateDisplay(vFrom, vTo);
...
TEST_METHOD(
    TestCategoryChangeAfterSwitchingActiveTwiceUpdatesDisplayCorrectly) {
  shared_ptr<UnitConverterMock> mock = make_shared<UnitConverterMock>();
  VM::UnitConverterViewModel vm(mock);
  const WCHAR *vFrom = L"1", *vTo = L"234";
  vm.UpdateDisplay(vFrom, vTo);
...
TEST_METHOD(TestSuggestedValuesCallbackUpdatesSupplementaryResults) {
  ...
  VERIFY_IS_TRUE((AddUnicodeLTRMarkers(L"1")) ==
                 vm.SupplementaryResults->GetAt(0)->Value);
  VERIFY_IS_TRUE(UNIT1 ==
                 vm.SupplementaryResults->GetAt(0)->Unit->GetModelUnit());
  VERIFY_IS_TRUE((AddUnicodeLTRMarkers(L"2")) ==
                 vm.SupplementaryResults->GetAt(1)->Value);
  VERIFY_IS_TRUE(UNIT2 ==
                 vm.SupplementaryResults->GetAt(1)->Unit->GetModelUnit());
  VERIFY_IS_TRUE((AddUnicodeLTRMarkers(L"3")) ==
                 vm.SupplementaryResults->GetAt(2)->Value);
  VERIFY_IS_TRUE(UNIT3 ==
                 vm.SupplementaryResults->GetAt(2)->Unit->GetModelUnit());
}
...
TEST_METHOD(TestCategoryChangeImmediatelyUpdatesSupplementaryResults) {
  ...
  VERIFY_ARE_EQUAL(
      (UINT)3,
      vm.SupplementaryResults
          ->Size); // Verify we're in the state we expect as a pre condition
...
TEST_METHOD(TestCategoryChangeImmediatelyUpdatesSupplementaryResultsWhimsy) {
  ...
  VERIFY_IS_TRUE(
      vm.SupplementaryResults->GetAt(3)->Unit->GetModelUnit().isWhimsical);
  VERIFY_IS_FALSE(
      vm.SupplementaryResults->GetAt(0)->Unit->GetModelUnit().isWhimsical);
}
...
TEST_METHOD(TestSupplementaryResultsWhimsicalUnits) {
  ...
  // Last item
  VERIFY_IS_TRUE(vm.SupplementaryResults->GetAt(0)->Unit->GetModelUnit() ==
                 UNITWHIMSY);
}

// Test deserialization
TEST_METHOD(TestUnitConverterViewModelDeserialization) {
  String ^ serializedTest =
      L"0[;;;]0[;;;]0[;;;]1[;;;]1.5[;;;]25[;;;]1.5[;;;]25[;;;][###][###]";
  shared_ptr<UnitConverterMock> mock = make_shared<UnitConverterMock>();
  VM::UnitConverterViewModel vm(mock);
...
TEST_METHOD(TestOnPaste) {
  ...
  // Call count is being set to 1 because we send 'CE' command as the first call
  // in OnPaste method of the ViewModel
  UINT callCount = 1;
  ...
  // Test all valid characters. Verify that two commands are sent for each
  // character
  vm.OnPaste("0", mode);
  callCount += 2;
  VERIFY_ARE_EQUAL(callCount, mock->m_sendCommandCallCount);
...
TEST_METHOD(TestDecimalFormattingLogic) {
  // verify that a dangling decimal is left alone until the user switches active
  // fields, and that trailing zeroes entered by the user remain in the display
  // even after a switch.
  ...
  VERIFY_IS_TRUE(vm.Value1 ==
                 AddUnicodeLTRMarkers(L"3")); // dangling decimal now removed
  VERIFY_IS_TRUE(vm.Value2 == AddUnicodeLTRMarkers(L"2.50"));
  vm.SwitchActive->Execute(nullptr);
  VERIFY_IS_TRUE(vm.Value1 == AddUnicodeLTRMarkers(L"3"));
  VERIFY_IS_TRUE(vm.Value2 == AddUnicodeLTRMarkers(L"2.50"));
...
TEST_METHOD(TestUnitsListBuildsFromEmptyModelUnitList) {
  ...
  VERIFY_ARE_EQUAL((UCM::EMPTY_UNIT).id,
                   (vm.Units->GetAt(0)->GetModelUnit()).id);
}
