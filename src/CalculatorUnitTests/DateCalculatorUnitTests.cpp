...
TEST_CLASS(DateCalculatorUnitTests){
  public :
      TEST_CLASS_INITIALIZE(TestClassSetup){/* Test Case Data */
...
datetimeBoundSubtract[0].dateDiff =
    dateDifference[2]; // on subtract date[0] not used
/*OutofBound*/ datetimeBoundSubtract[1].startDate = date[14];
datetimeBoundSubtract[1].endDate = date[0];
datetimeBoundSubtract[1].dateDiff =
    dateDifference[11]; // on subtract date[0] not used
...
/* Duration Between Two Date Tests -- Timediff obtained after calculation should
 * be checked to be identical */
...
TEST_METHOD(TestAddOob) {
  ...
  //    // Add Duration
  //    bool isValid =
  //    m_DateCalcEngine.AddDuration(DateUtils::SystemTimeToDateTime(datetimeBoundAdd[testIndex].startDate),
...
TEST_METHOD(TestSubtractOob) {
  for (int testIndex = 0; testIndex < c_numSubtractOobDate; testIndex++) {
    ...
    bool isValid = m_DateCalcEngine.SubtractDuration(
        DateUtils::SystemTimeToDateTime(
            datetimeBoundSubtract[testIndex].startDate),
...
TEST_METHOD(TestAddition) {
  ...
  //    // Add Duration
  //    bool isValid =
  //    m_DateCalcEngine.AddDuration(DateUtils::SystemTimeToDateTime(datetimeAddCase[testIndex].startDate),
  ...
  //        datetimeAddCase[testIndex].endDate.wDayOfWeek)
  //    {
  //        isValid = false;
  //    }
...
TEST_METHOD(TestSubtraction) {
  ...
  //    // Subtract Duration
  //    bool isValid =
  //    m_DateCalcEngine.SubtractDuration(DateUtils::SystemTimeToDateTime(datetimeSubtractCase[testIndex].startDate),
  ...
  //        || systemTime.wDayOfWeek !=
  //        datetimeSubtractCase[testIndex].endDate.wDayOfWeek)
  //    {
  //        isValid = false;
  //    }
...
TEST_METHOD(DateCalcViewModelDateDiffDaylightSavingTimeTest) {
  ...
  // 29.02.2008
  viewModel->FromDate =
      DateUtils::SystemTimeToDateTime(datetimeDifftest[5].startDate);
  // 31.03.2008
  viewModel->ToDate =
      DateUtils::SystemTimeToDateTime(datetimeDifftest[5].endDate);

  //// Assert for the result
  VERIFY_IS_FALSE(viewModel->IsDiffInDays);
  VERIFY_ARE_EQUAL(StringReference(L"31 days"),
                   viewModel->StrDateDiffResultInDays);
  VERIFY_ARE_EQUAL(StringReference(L"1 month, 2 days"),
                   viewModel->StrDateDiffResult);
...
TEST_METHOD(DateCalcViewModelAddTest) {
  ...
  //     The date returned by DateUtils::GetLongDate can be a different string
  //     than expected
  //         based on the values of date[7].  This is because date[7] is in UTC
  //         but GetLongDate doesn't format according to UTC.  If it did, the
  //         test would still be incorrect because the ViewModel is not
  //         necessarily in UTC.
  //
  //     The DateTime value assigned to StartDate after the conversion
  //     SystemTimeToDateTime is not
  //         the same DateTime value as if the user were to select the same date
  //         from the CalendarDatePicker. This means testing a specific date
  //         here, is *not* the same as selecting that date in the app.
  ...
  // viewModel->StartDate =
  // DateUtils::SystemTimeToDateTime(datetimeAddCase[0].startDate);
  ...
  //// Assert for the result
  // VERIFY_ARE_EQUAL(DateUtils::GetLongDate(date[7]),
...
TEST_METHOD(DateCalcViewModelSubtractTest) {
  ...
  // viewModel->StartDate =
  // DateUtils::SystemTimeToDateTime(datetimeSubtractCase[0].startDate);
...
TEST_METHOD(DateCalcViewModelAddOobTest) {
  ...
  //    // Assert for the result
  //    VERIFY_ARE_EQUAL(StringReference(L"Date out of Bound"),
  //    viewModel->StrDateResult);
  //}
}
...
TEST_METHOD(DateCalcViewModelSubtractOobTest) {
  ...
  for (int testIndex = 0; testIndex < c_numSubtractOobDate; testIndex++) {
    viewModel->StartDate = DateUtils::SystemTimeToDateTime(
        datetimeBoundSubtract[testIndex].startDate);
...
TEST_METHOD(DateCalcViewModelDateDiffIgnoreSignTest) {
  ...
  VERIFY_IS_FALSE(viewModel->IsDiffInDays);
  VERIFY_ARE_EQUAL(StringReference(L"305 days"),
                   viewModel->StrDateDiffResultInDays);
...
TEST_METHOD(DateCalcViewModelDateDiffTest) {
  // TODO - MSFT 10331900, fix this test
  // The last VERIFY checks with expected value "8398 years, 11 months, 4 weeks,
  // 2 days" The viewmodel result is something like "8398 years, 12 months,
  // 6568892 weeks, 1 day", which shows there is a problem with the viewmodel's
  // reduction algorithm.
  ...
  // viewModel->FromDate =
  // DateUtils::SystemTimeToDateTime(datetimeDifftest[0].startDate);
  // viewModel->ToDate =
  // DateUtils::SystemTimeToDateTime(datetimeDifftest[0].endDate);

  //// Assert for the result
  // VERIFY_IS_FALSE(viewModel->IsDiffInDays);
  // VERIFY_ARE_EQUAL(StringReference(L"3067670 days"),
  // viewModel->StrDateDiffResultInDays);
  // VERIFY_ARE_EQUAL(StringReference(L"8398 years, 11 months, 4 weeks, 2
  // days"), viewModel->StrDateDiffResult);
}
...
TEST_METHOD(DateCalcViewModelAddSubtractResultAutomationNameTest) {
  ...
  wstring components[] = {L"dayofweek.full", L"month.full", L"year.full",
                          L"day"};

  for (const wstring &component : components) {
    auto formatter =
        ref new DateTimeFormatter(ref new String(component.c_str()));
    wstring expectedValue = formatter->Format(startDate)->Data();
    wstring message = L"Verifying " + component + L" is present in the result";
    VERIFY_IS_TRUE(actualValue.find(expectedValue) != wstring::npos,
                   message.c_str());
  }
}
}
