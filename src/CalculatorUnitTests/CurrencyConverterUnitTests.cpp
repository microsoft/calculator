...
public:
  MockCurrencyHttpClientWithResult(String ^ staticResponse,
...
class DataLoadedCallback
    : public UnitConversionManager::IViewModelCurrencyCallback {
public:
  ...
  void CurrencySymbolsCallback(_In_ const wstring & /*fromSymbol*/,
                               _In_ const wstring & /*toSymbol*/) override {}
  void
  CurrencyRatiosCallback(_In_ const wstring & /*ratioEquality*/,
                         _In_ const wstring & /*accRatioEquality*/) override {}
  void CurrencyTimestampCallback(_In_ const std::wstring & /*timestamp*/,
                                 bool /*isWeekOldData*/) override {}
...
const UCM::Category CURRENCY_CATEGORY = {
    NavCategory::Serialize(ViewMode::Currency), L"Currency",
    false /*supportsNegative*/};

unique_ptr<CurrencyDataLoader>
MakeLoaderWithResults(String ^ staticResponse, String ^ allRatiosResponse) {
  auto client = make_unique<MockCurrencyHttpClientWithResult>(
      staticResponse, allRatiosResponse);
  client->SetSourceCurrencyCode(StringReference(DefaultCurrencyCode.data()));
  return make_unique<CurrencyDataLoader>(move(client));
}

String ^
    SerializeContent(const vector<String ^> &data) {
      String ^ result = L"";
...
bool DeleteFileFromLocalCacheFolder(String ^ filename) {
  try {
    ...
    IAsyncOperation<StorageFile ^> ^ fileOperation =
...
bool DeleteCurrencyCacheFiles() {
  try {
    bool deletedStaticData = DeleteFileFromLocalCacheFolder(
        CurrencyDataLoaderConstants::StaticDataFilename);
    bool deletedAllRatiosData = DeleteFileFromLocalCacheFolder(
        CurrencyDataLoaderConstants::AllRatiosDataFilename);
...
void StandardCacheSetup() {
  ...
  InsertToLocalSettings(CurrencyDataLoaderConstants::CacheLangcodeKey,
                        StringReference(sc_Language_EN));
  ...
  VERIFY_IS_TRUE(WriteToFileInLocalCacheFolder(
      CurrencyDataLoaderConstants::AllRatiosDataFilename,
      CurrencyHttpClient::GetRawAllRatiosDataResponse()));
}

TEST_CLASS(CurrencyConverterLoadTests){
  public : TEST_METHOD_INITIALIZE(DeleteCacheFiles){DeleteCurrencyCacheFiles();
} // namespace CalculatorUnitTests
...
TEST_METHOD(LoadFromCache_Fail_OlderThanADay) {
  ...
  DateTime now = Utils::GetUniversalSystemTime();
  DateTime dayOld;
  dayOld.UniversalTime =
      now.UniversalTime - CurrencyDataLoaderConstants::DayDuration - 1;
...
TEST_METHOD(LoadFromCache_Fail_ResponseLanguageChanged) {
  ...
  // Tests always use en-US as response language. Insert a different lang-code
  // to fail the test.
  InsertToLocalSettings(CurrencyDataLoaderConstants::CacheLangcodeKey,
                        L"ar-SA");
...
TEST_METHOD(LoadFromWeb_Fail_WebException) {
  CurrencyDataLoader loader{
      make_unique<MockCurrencyHttpClientThrowsException>()};
...
TEST_METHOD(LoadFromWeb_Success) {
  ...
  String ^ allRatiosResponse =
      CurrencyHttpClient::GetRawAllRatiosDataResponse();
  unique_ptr<CurrencyDataLoader> loader =
      MakeLoaderWithResults(staticResponse, allRatiosResponse);
...
TEST_METHOD(Load_Success_LoadedFromCache) {
  ...
  loader.SetViewModelCallback(
      make_shared<DataLoadedCallback>(data_loaded_event));
...
TEST_CLASS(CurrencyConverterUnitTests){
    const UCM::Unit GetUnit(const vector<UCM::Unit> &unitList,
                            const wstring &target){return *find_if(
        begin(unitList), end(unitList),
        [&target](const UCM::Unit &u) { return u.abbreviation == target; });
}
...
TEST_METHOD(Loaded_LoadOrderedRatios) {
  ...
  unordered_map<UCM::Unit, UCM::ConversionData, UCM::UnitHash> ratios =
      loader.LoadOrderedRatios(usdUnit);
...
TEST_METHOD(Loaded_GetCurrencySymbols_Valid) {
  ...
  const pair<wstring, wstring> symbols =
      loader.GetCurrencySymbols(usdUnit, eurUnit);
...
TEST_METHOD(Loaded_GetCurrencySymbols_Invalid) {
  ...
  pair<wstring, wstring> symbols =
      loader.GetCurrencySymbols(fakeUnit1, fakeUnit2);

  VERIFY_ARE_EQUAL(wstring(L""), wstring(symbols.first.c_str()));
  VERIFY_ARE_EQUAL(wstring(L""), wstring(symbols.second.c_str()));

  // Verify that when only one unit is valid, both symbols return as empty
  // string.
  vector<UCM::Unit> unitList = loader.LoadOrderedUnits(CURRENCY_CATEGORY);
  VERIFY_ARE_EQUAL(size_t{2}, unitList.size());
...
TEST_METHOD(Loaded_GetCurrencyRatioEquality_Valid) {
  ...
  const pair<wstring, wstring> ratio =
      loader.GetCurrencyRatioEquality(usdUnit, eurUnit);

  VERIFY_ARE_EQUAL(wstring(L"1 USD = 0.9205 EUR"), ratio.first);
  VERIFY_ARE_EQUAL(wstring(L"1 United States Dollar = 0.9205 Europe Euro"),
                   ratio.second);
}
...
TEST_METHOD(Loaded_GetCurrencyRatioEquality_Invalid) {
  ...
  pair<wstring, wstring> ratio =
      loader.GetCurrencyRatioEquality(fakeUnit1, fakeUnit2);
